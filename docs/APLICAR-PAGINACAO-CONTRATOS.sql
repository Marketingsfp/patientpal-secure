-- ===================================================================
-- APLICAR NO SQL EDITOR DO LOVABLE
-- Paginacao da listagem de contratos (Cartao Beneficios > Vendas)
--
-- O QUE RESOLVE
-- A tela so recebia os 500 contratos mais recentes, e os filtros
-- (periodo, convenio, vendedor, situacao...) rodavam em cima desse
-- recorte. Por isso o filtro "Ultimos 30 dias" dizia
-- "1-22 de 22 contratos" quando existiam 29 que atendiam ao filtro.
--
-- O QUE MUDA
-- Acrescenta o parametro _offset, para a tela pedir a base em paginas
-- ate acabar. A consulta em si nao muda: mesma ordenacao, mesmos
-- campos, mesma regra de permissao. Nenhum dado e alterado.
--
-- SEGURANCA
-- E somente definicao de funcao: nao mexe em nenhuma tabela, nao
-- altera nem apaga registro nenhum. Pode rodar com o sistema em uso.
--
-- SE VOCE NAO RODAR: nada quebra. A tela continua como esta hoje,
-- mostrando so a primeira pagina.
-- ===================================================================
...
-- ===================================================================
-- CONFERENCIA (somente leitura) — rode depois de aplicar.
-- A primeira linha deve trazer 4 argumentos, terminando em _offset.
-- ===================================================================
SELECT p.oid::regprocedure AS assinatura_da_funcao
  FROM pg_proc p
  JOIN pg_namespace n ON n.oid = p.pronamespace
 WHERE n.nspname = 'public' AND p.proname = 'buscar_contratos';
