-- FASE 3: classificador central de período (alinha o banco às mesmas regras do código)
...
begin
  ...
  -- Versões oficiais aplicáveis ao escopo real do evento.
  -- Calendário de unidade só vale quando a unidade do evento é conhecida e igual.
  -- Calendário geral (unidade_id nulo) vale por estar explicitamente definido como geral.
  create temp table if not exists _tmp_ver_ignore(x int); -- no-op para compatibilidade
  with aplicaveis as (
    select v.*
  ...
  -- 1) exceções por data prevalecem
...
-- Mantém o retorno curto ('dentro' | 'fora' | 'nao_classificavel') usado pelas métricas,
-- agora delegando para o classificador central.
...
set search_path = public
as $$
  select case public.nina_classificar_atendimento_detalhe(p_clinica, p_unidade, p_em, p_fuso) ->> 'classificacao'
