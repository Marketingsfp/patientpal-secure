CREATE OR REPLACE FUNCTION public.buscar_pacientes_global(_clinica_ids uuid[], _termo text, _limite integer DEFAULT 20)
 RETURNS TABLE(id uuid, nome text, cpf text, telefone text, data_nascimento date, clinica_id uuid, codigo_prontuario text, codigo_prontuario_anterior text, numero_pasta text, email text, associado_convenio text, ultima_consulta date, cadastro_incompleto boolean, match_score integer, match_reason text)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF v_is_num THEN
    -- CPF exato — usa idx_pacientes_clinica_cpf_digits (btree indexado)
    ...
    -- Prontuário exato (comparação direta, sem regex; catch mais amplo em texto)
    INSERT INTO _busca_cand
    ...
    -- Telefone — usa idx_pacientes_clinica_tel_digits / tel2_digits (btree por expressão)
    ...
    -- CPF parcial — prefix match usa o btree (clinica_id, cpf_digits)
  ...
  conv AS (
    SELECT DISTINCT ON (ca.paciente_id)
           ca.paciente_id,
     ...
     WHERE ca.paciente_id IN (SELECT b.pid FROM best b)
       AND ca.status = 'ativo'
     ORDER BY ca.paciente_id, ca.created_at DESC
  ),
  ult AS (
  ...
  SELECT
    ...
    LEFT JOIN conv cn ON cn.paciente_id = b.pid
