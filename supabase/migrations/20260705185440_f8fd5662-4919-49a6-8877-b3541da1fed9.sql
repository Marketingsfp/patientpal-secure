...
DECLARE
  ...
  v_is_phone boolean := v_dlen BETWEEN 8 AND 11;
  ...
  WITH pac AS (
    SELECT
      ...
      (CASE
        ...
        WHEN p.nome ILIKE v_like THEN 35
        ELSE 5
    ...
    WHERE p.clinica_id = ANY(v_scope)
      ...
      AND (
        -- CPF exato (btree idx_pacientes_clinica_cpf_digits)
        (v_is_cpf AND p.cpf_digits = v_digits)
        -- Telefone exato (btree funcional idx_pacientes_clinica_tel_digits / tel2)
        OR (v_is_phone AND (
              regexp_replace(coalesce(p.telefone,''),  '\D','','g') = v_digits
           OR regexp_replace(coalesce(p.telefone2,''), '\D','','g') = v_digits
        ))
        -- Dígitos curtos: prefixo (btree range scan) — evita LIKE '%x%'
        OR (v_is_short_digits AND (
              p.cpf_digits LIKE v_digits || '%'
           OR regexp_replace(coalesce(p.telefone,''),  '\D','','g') LIKE v_digits || '%'
           OR regexp_replace(coalesce(p.telefone2,''), '\D','','g') LIKE v_digits || '%'
        ...
        -- Nome: prefixo (btree lower(nome)) OU contains via trigram (gin idx_pacientes_nome_trgm)
        OR (NOT v_has_digits AND (
              lower(p.nome) LIKE v_prefix
           OR p.nome ILIKE v_like
        ))
      )
