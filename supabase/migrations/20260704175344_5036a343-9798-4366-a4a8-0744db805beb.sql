...
BEGIN
  ...
  RETURN QUERY
  WITH candidates AS (
    -- CPF exato (11 dígitos)
    SELECT p.id, 100 AS score, 'CPF exato'::text AS reason
      FROM public.pacientes p
     WHERE p.ativo AND p.clinica_id = ANY(v_allowed)
       AND v_is_num AND length(v_digits) = 11
    ...
    SELECT p.id, 95, 'Prontuário exato'
      FROM public.pacientes p
     WHERE p.ativo AND p.clinica_id = ANY(v_allowed)
       AND v_is_num AND p.codigo_prontuario = v_digits
    UNION ALL
    -- Código antigo exato
    SELECT p.id, 90, 'Código antigo exato'
      FROM public.pacientes p
     WHERE p.ativo AND p.clinica_id = ANY(v_allowed)
       AND v_is_num AND p.codigo_prontuario_anterior = v_digits
    UNION ALL
    -- Pasta exata
    SELECT p.id, 88, 'Pasta exata'
      FROM public.pacientes p
     WHERE p.ativo AND p.clinica_id = ANY(v_allowed)
       AND v_is_num AND p.numero_pasta = v_digits
    UNION ALL
    -- Telefone exato ou por sufixo
    SELECT p.id, 85, 'Telefone'
      FROM public.pacientes p
     WHERE p.ativo AND p.clinica_id = ANY(v_allowed)
       AND v_is_num AND length(v_digits) BETWEEN 8 AND 11
       AND (p.telefone LIKE '%' || v_digits || '%'
            OR p.telefone2 LIKE '%' || v_digits || '%')
    UNION ALL
    -- Data de nascimento
    SELECT p.id, 82, 'Data de nascimento'
      FROM public.pacientes p
     WHERE p.ativo AND p.clinica_id = ANY(v_allowed)
       AND v_data IS NOT NULL AND p.data_nascimento = v_data
    UNION ALL
    -- Nome: começa com (índice text_pattern_ops)
    ...
    SELECT p.id, 40, 'Nome contém'
     ...
     WHERE p.ativo AND p.clinica_id = ANY(v_allowed)
       ...
       AND upper(public.strip_accents(p.nome)) LIKE '%' || v_norm || '%'
    UNION ALL
    -- CPF parcial (trgm em cpf_digits)
    SELECT p.id, 20, 'Parcial'
      FROM public.pacientes p
     WHERE p.ativo AND p.clinica_id = ANY(v_allowed)
       AND v_is_num AND length(v_digits) BETWEEN 3 AND 10
