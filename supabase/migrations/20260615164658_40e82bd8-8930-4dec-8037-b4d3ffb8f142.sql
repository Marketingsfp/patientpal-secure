...
BEGIN
  ...
  WITH g AS (
    SELECT id, created_at, codigo_prontuario_anterior,
           ROW_NUMBER() OVER (
             ...
             ORDER BY (codigo_prontuario_anterior IS NOT NULL) DESC, (cpf IS NOT NULL) DESC, created_at, id
           ) AS rn,
           FIRST_VALUE(id) OVER (
             PARTITION BY upper(public.strip_accents(trim(nome))), data_nascimento
             ORDER BY (codigo_prontuario_anterior IS NOT NULL) DESC, (cpf IS NOT NULL) DESC, created_at, id
  ...
  FROM (
    ...
    ORDER BY m.winner_id,
      ((lo.telefone IS NOT NULL)::int + (lo.email IS NOT NULL)::int + (lo.cep IS NOT NULL)::int) DESC,
      lo.created_at
  ) src
