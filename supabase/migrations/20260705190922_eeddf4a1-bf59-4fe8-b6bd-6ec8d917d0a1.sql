...
BEGIN
  ...
  RETURN QUERY
  WITH pac_raw AS (
    (SELECT p.id AS pid, p.nome, p.cpf, p.telefone, p.numero_pasta, p.codigo_prontuario,
  ...
  pac_dedup AS (
    SELECT DISTINCT ON (pid) pid, nome, cpf, telefone, numero_pasta, codigo_prontuario,
      data_nascimento, clinica_id, ativo, created_at, raw_score
    FROM pac_raw
    ORDER BY pid, raw_score DESC
  ),
  pac AS (
    SELECT
      'paciente'::text AS tipo, p.pid AS id, p.nome::text AS titulo,
