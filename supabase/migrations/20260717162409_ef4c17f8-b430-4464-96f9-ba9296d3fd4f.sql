...
-- Drop older overloads (without _valor_mensal) to remove ambiguity
DROP FUNCTION IF EXISTS public.renovar_contrato_extensao(uuid, text, jsonb);
DROP FUNCTION IF EXISTS public.renovar_contrato_troca_plano(uuid, uuid, text, boolean, jsonb);

-- Recreate renovar_contrato_extensao WITHOUT the non-existent "descricao" column
    ...
    FOR v_dep IN SELECT * FROM jsonb_array_elements(_dependentes) LOOP
      ...
      ELSE
        INSERT INTO public.contrato_dependentes (
          ...
          _contrato_id, v_contrato.clinica_id,
...
-- Recreate renovar_contrato_troca_plano WITHOUT "descricao"
  ...
  ELSE
    ...
    FOR v_dep IN SELECT * FROM jsonb_array_elements(_dependentes) LOOP
      ...
      INSERT INTO public.contrato_dependentes (
        ...
        v_novo_id, v_contrato.clinica_id,
