CREATE OR REPLACE FUNCTION public.renovar_contrato_extensao(_contrato_id uuid, _observacao text DEFAULT NULL::text, _dependentes jsonb DEFAULT '[]'::jsonb, _valor_mensal numeric DEFAULT NULL::numeric)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
      ...
      ELSE
        ...
        IF COALESCE((v_dep->>'cobrar_taxa_inclusao')::boolean, false) AND v_taxa_inclusao > 0 THEN
          ...
          INSERT INTO public.contrato_mensalidades (
            ...
            _contrato_id, v_contrato.clinica_id, v_menor_neg, CURRENT_DATE,
...
CREATE OR REPLACE FUNCTION public.renovar_contrato_troca_plano(_contrato_id uuid, _convenio_novo_id uuid, _observacao text DEFAULT NULL::text, _cobrar_taxa_adesao boolean DEFAULT true, _dependentes jsonb DEFAULT '[]'::jsonb, _valor_mensal numeric DEFAULT NULL::numeric)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  IF jsonb_typeof(_dependentes) <> 'array' OR jsonb_array_length(_dependentes) = 0 THEN
    ...
    SELECT v_novo_id, paciente_id, paciente_nome, parentesco, tipo, CURRENT_DATE, true
      FROM public.contrato_dependentes
     WHERE contrato_id = _contrato_id AND ativo = true;
  ELSE
    v_menor_neg := 0;
    FOR v_dep IN SELECT * FROM jsonb_array_elements(_dependentes) LOOP
      IF NOT COALESCE((v_dep->>'manter')::boolean, true) THEN CONTINUE; END IF;
      IF NULLIF(v_dep->>'paciente_id','') IS NULL THEN CONTINUE; END IF;
      ...
      IF NULLIF(v_dep->>'id','') IS NULL
        ...
        INSERT INTO public.contrato_mensalidades (
          ...
          v_novo_id, v_contrato.clinica_id, v_menor_neg, v_data_inicio,
          v_taxa_inclusao, 'pendente',
          'Taxa de inclusão de dependente — ' || COALESCE(v_dep->>'paciente_nome','')
        );
      END IF;
