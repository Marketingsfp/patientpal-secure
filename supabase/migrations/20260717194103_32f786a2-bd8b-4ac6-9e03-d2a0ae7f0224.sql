DROP FUNCTION IF EXISTS public.renovar_contrato_extensao(uuid, text, jsonb, numeric);
DROP FUNCTION IF EXISTS public.renovar_contrato_extensao(uuid, text, jsonb, numeric, date);
DROP FUNCTION IF EXISTS public.renovar_contrato_troca_plano(uuid, uuid, text, boolean, jsonb, numeric);
DROP FUNCTION IF EXISTS public.renovar_contrato_troca_plano(uuid, uuid, text, boolean, jsonb, numeric, date);
...
BEGIN
  ...
  INSERT INTO public.contrato_renovacoes (
    ...
    CASE WHEN _data_renovacao IS NOT NULL AND _data_renovacao <> CURRENT_DATE
         THEN COALESCE(_observacao || E'\n', '') || '[Renovação registrada com data informada: ' || to_char(v_data_ren, 'DD/MM/YYYY') || ']'
...
CREATE OR REPLACE FUNCTION public.renovar_contrato_troca_plano(
  ...
  _observacao text DEFAULT NULL,
  _cobrar_taxa_adesao boolean DEFAULT false,
  _dependentes jsonb DEFAULT '[]'::jsonb,
  _valor_mensal numeric DEFAULT NULL,
  _data_renovacao date DEFAULT NULL
)
