...
-- Fix: coalesce NULL → 0 no valor final (compat com JS `?? 0`)
...
begin
  ...
  base as (
    select f.ag_id, f.paciente_id, f.paciente_nome, f.proc_nome, f.inicio, f.medico_nome,
           proc.procedimento_id, proc.base_dinheiro, proc.base_cartao
    from fila f
    left join lateral (
      ...
      where pp.clinica_id = _clinica_id and upper(pp.nome) = upper(f.proc_nome)
      limit 1
    ) proc on true
  ),
  conv as (
    ...
    cross join lateral (
      ...
      where cc.clinica_id = _clinica_id and cc.status = 'ativo'
        and cc.paciente_id = b.paciente_id and cc.convenio_id is not null
      order by cc.created_at asc nulls last limit 1
    ) ct
