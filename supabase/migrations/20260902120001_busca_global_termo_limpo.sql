-- Parte 2 da correcao da busca colada: buscar_pacientes_global.
--
-- Ela alimenta o campo de busca de paciente (agenda, prontuario, financeiro e
-- mais 20 telas) e a lista de Clientes V2. Compara com
-- LIKE '%' || termo || '%' em cima do termo INTEIRO, entao um espaco a mais no
-- meio derruba o resultado para zero.
--
...
-- definicao que esta em producao hoje (md5 573c301f5ce1e0ad133e8f90f69b59b7).
--
-- Observacao: buscar_pacientes (a lista antiga de Clientes) NAO precisa da
-- mesma correcao -- ela ja quebra o nome em palavras com
-- regexp_split_to_array(v_norm, '\s+') e junta com '%', o que absorve espaco
-- repetido, NBSP e tabulacao por conta propria.
...
-- CREATE OR REPLACE preserva as permissoes existentes; repetidas aqui para
-- garantir que uma reaplicacao nunca reabra a funcao ao visitante anonimo.
