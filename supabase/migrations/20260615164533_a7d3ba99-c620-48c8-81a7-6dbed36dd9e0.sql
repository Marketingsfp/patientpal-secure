...
BEGIN
  ...
  WITH g AS (
    SELECT id, created_at, codigo_prontuario_anterior,
           ROW_NUMBER() OVER (
  ...
  -- Preencher campos vazios do vencedor (evitando colidir CPF com outro paciente fora do grupo)
  UPDATE public.pacientes p SET
    cpf = CASE
      WHEN p.cpf IS NOT NULL THEN p.cpf
      WHEN src.cpf IS NULL THEN NULL
      WHEN EXISTS (
        SELECT 1 FROM public.pacientes p2
        WHERE p2.clinica_id = _clin AND p2.cpf = src.cpf AND p2.id <> p.id
          AND NOT EXISTS (SELECT 1 FROM _merge_map mm WHERE mm.loser_id = p2.id)
      ) THEN NULL
      ELSE src.cpf
    END,
