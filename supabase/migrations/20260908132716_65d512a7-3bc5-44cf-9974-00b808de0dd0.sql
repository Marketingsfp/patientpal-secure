-- Quadro de avaliação do pool: quem é elegível agora e, para quem não é, por quê.
-- Usada tanto pela auditoria da atribuição quanto pela tela de diagnóstico.
...
AS $function$
  WITH base AS (
    SELECT
      ...
      public.has_module_access(p.user_id, _clinica_id, 'telefonia', 'read') AS permission_telefonia,
      EXISTS (
    ...
    SELECT b.*,
      CASE
        WHEN NOT b.permission_telefonia THEN 'missing_telefonia_permission'
        ...
        WHEN NOT b.presenca_recente     THEN 'presenca_desatualizada'
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
           'user_id', a.user_id,
           'permission_telefonia', a.permission_telefonia,
...
-- Auditoria completa da atribuição automática.
...
BEGIN
  ...
  -- Quadro de candidatos avaliados NESTE instante (para auditoria).
  ...
  -- Evento de handoff que originou esta atribuição (quando existir).
  ...
  VALUES
    (_clinica_id, _conversa_id, 'ASSUMIDA', _escolhido, _departamento_id,
     ...
     jsonb_build_object(
       ...
       -- compatibilidade com os campos já usados por telas existentes
