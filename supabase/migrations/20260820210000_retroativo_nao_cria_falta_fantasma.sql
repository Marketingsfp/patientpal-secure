-- ---------------------------------------------------------------------------
-- Lançamento retroativo em caixa já fechado deixa de inventar "Falta em caixa"
--
-- Quando um recebimento é gravado com data de um dia cujo caixa já foi
-- fechado, `fn_registrar_lancamento_e_caixa` somava o valor apenas no
-- `valor_fechamento_calculado` e recalculava
--
--     diferenca = informado - (calculado + valor)
--
-- O `valor_fechamento_informado` — o que a operadora contou e digitou no
-- fechamento — ficava intocado. Resultado: toda vez que alguém lançava um
-- atendimento esquecido com a data de ontem, o caixa de ontem, já conferido e
-- zerado, passava a acusar uma FALTA exatamente do tamanho do lançamento.
--
-- Medido na base em 20/08/2026: 34 sessões fechadas receberam retroativo e em
-- 22 delas a diferença registrada era exatamente o simétrico da soma dos
-- retroativos — falta 100% fantasma. Exemplos: Nicole em 19/08 fechou em
-- R$ 7.985,95 batendo e, depois de três retroativos (R$ 120 + R$ 80 + R$ 120)
-- lançados no dia seguinte, o caixa passou a mostrar "Falta em caixa
-- R$ 320,00"; Mayara em 23/07 ganhou uma falta de R$ 882,00 vinda de dois
-- retroativos de R$ 462,00 e R$ 420,00.
--
-- A conferência é um ato humano com hora marcada: a operadora contou a gaveta
-- e comparou com o que o sistema mostrava NAQUELE momento. Um valor digitado
-- depois não pode reescrever esse veredito. Por isso o retroativo passa a ser
-- somado nos DOIS lados — calculado e informado — preservando a diferença que
-- o fechamento registrou. A observação de auditoria continua sendo gravada,
-- então o valor acrescentado depois permanece rastreável.
--
-- A correção do histórico (as 34 sessões já marcadas) está na migração
-- seguinte, 20260820210500_restaura_conferencia_dos_caixas_afetados.sql.
-- ---------------------------------------------------------------------------
    ...
    ELSE
      ...
      BEGIN
        ...
        ELSIF v_sess_status = 'fechado'::public.caixa_sessao_status THEN
          -- O valor entra nos DOIS lados: o dia passa a valer o que realmente
          -- foi recebido, e a diferença continua sendo a que a operadora
          -- fechou (arredondada para matar resíduo de ponto flutuante).
          UPDATE public.caixa_sessoes
          SET valor_fechamento_calculado = COALESCE(v_sess_calc, 0) + v_valor_mov,
              valor_fechamento_informado = COALESCE(v_sess_infor, 0) + v_valor_mov,
              diferenca = round(COALESCE(v_sess_infor, 0) - COALESCE(v_sess_calc, 0), 2),
