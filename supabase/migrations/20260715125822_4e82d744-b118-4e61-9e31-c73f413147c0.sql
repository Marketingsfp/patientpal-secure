...
begin
  ...
  -- do caixa para o paciente, não uma retirada operacional. Também trata
  -- linhas antigas cadastradas como 'sangria' com descrição "Estorno —" no
  -- exists() para não duplicar.
  insert into caixa_movimentos (sessao_id, clinica_id, user_id, tipo, valor, descricao, forma_pagamento, lancamento_id)
  select r.sessao_id, r.clinica_id, r.user_id, 'estorno', r.valor,
  ...
  where r.lancamento_id = v_lanc.id and r.tipo = 'recebimento'
    and not exists (
      ...
      where s.lancamento_id = v_lanc.id
        ...
        and (
          s.tipo = 'estorno'
          or (s.tipo = 'sangria' and lower(coalesce(s.descricao, '')) like 'estorno%')
        )
    );
