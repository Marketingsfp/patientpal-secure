-- Parte 4 da correcao da busca colada: buscar_contratos.
--
-- A busca da tela de Contratos tolerava espaco repetido por acidente (quebra o
-- nome em palavras com regexp_split_to_array e junta com '%'), mas nao trata
-- espaco duro (NBSP) nem caractere de largura zero -- justamente o que vem ao
-- colar de pagina web e de PDF. Nesses casos o contrato "nao aparece".
--
...
-- definicao aplicada em 02/09/2026 (migration 20260902160000), inclusive a
-- ordem de prontuario codigo_prontuario -> numero_pasta -> herdado.
...
AS $function$
DECLARE
  -- Limpa o texto colado antes de comparar: espaco duro (NBSP) e caractere de
  -- largura zero nao sao removidos pelo regexp_split_to_array mais abaixo.
