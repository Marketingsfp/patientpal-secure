...
BEGIN
  ...
  SELECT p.user_id
   ...
   WHERE p.clinica_id = _clinica_id
     ...
     -- e mesmo que o perfil dele também tenha Telefonia.
     AND NOT public.atend_usuario_e_admin(p.user_id, _clinica_id)
     -- FASE 2 — Telefonia é elegibilidade: só entra no pool quem tem a
     -- permissão marcada em Cadastros › Perfis. Fail-closed por definição da
     -- has_module_access (perfil sem a permissão = não elegível).
...
COMMENT ON FUNCTION public.atend_auto_assign_conversa(uuid, uuid, uuid, text) IS
  'Distribuicao automatica: candidatos precisam estar ONLINE, sem pausa aberta, nao-admin e com a permissao de modulo "telefonia" (Cadastros > Perfis). Demais criterios (setor, menor carga, desempate) inalterados.';