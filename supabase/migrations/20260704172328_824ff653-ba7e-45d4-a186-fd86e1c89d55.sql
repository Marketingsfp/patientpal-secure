-- P1-CAIXA-001: otimização da RPC fila_caixa_hoje
-- Ver plano no chat: substitui o LEFT JOIN LATERAL upper(nome)=upper(nome)
-- (que causava seq-scan repetido de procedimentos = statement timeout de 3s
-- em clínicas com ~1600 agendamentos no dia) por um hash join único usando
-- CTE pre-normalizada de procedimentos. Também garante índice funcional
-- em upper(nome) para os poucos casos remanescentes.

create index if not exists idx_procedimentos_clinica_upnome
  on public.procedimentos (clinica_id, upper(nome));
...
begin
  ...
  -- Pre-agrega procedimentos DA CLÍNICA por nome uppercased (dedup via distinct on).
  procs_norm as (
