...
begin
  ...
  procs_norm as (
    select distinct on (upper(pp.nome))
           upper(pp.nome) as up_nome, pp.id as procedimento_id,
    ...
    from public.procedimentos pp where pp.clinica_id = _clinica_id
  ...
  base as (
    ...
    from fila f left join procs_norm pn on pn.up_nome = upper(f.proc_nome)
  ),
  conv as (
    select distinct on (b.ag_id) b.ag_id, cc.id as contrato_id, cc.convenio_id
    from base b
    join public.contratos_assinatura cc
      on cc.clinica_id = _clinica_id and cc.status = 'ativo'
     and cc.paciente_id = b.paciente_id and cc.convenio_id is not null
  ...
  mens_pagas as (
    ...
    from conv cv
    left join public.contrato_mensalidades m on m.contrato_id = cv.contrato_id and m.status = 'pago'
    group by cv.ag_id
  ),
  fixo as (
    select b.ag_id, v.valor_dinheiro::numeric as v
    from base b join conv cv on cv.ag_id = b.ag_id
  ...
  regra_pick as (
    ...
    cross join lateral (
      ...
      order by
        ...
        (case when rr.procedimento_id is not null then 0 when rr.tipo is null then 1
  ...
  calc as (
    select b.ag_id, b.paciente_id, b.paciente_nome, b.proc_nome, b.inicio, b.medico_nome,
           ...
           case when fx.v is not null then 'valor convênio' when rpv.sufixo is not null then rpv.sufixo end as sufixo,
           case when fx.v is not null then 'convenio_valor_fixo'
                when rpv.dinheiro is not null then 'convenio_regra' else 'particular' end as origem
    from base b
