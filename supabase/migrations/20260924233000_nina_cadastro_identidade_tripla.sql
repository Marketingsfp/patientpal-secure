-- Cadastro da Nina: nome completo, nascimento e telefone em conjunto.
-- Homônimos são filtrados pelo telefone antes da decisão de ambiguidade.
-- Preserva dados históricos, assinatura e permissões da RPC.
...
BEGIN
  ...
  END IF;
  -- Ambiente vem da conversa persistida, nunca de argumentos do modelo.
  v_teste := coalesce(v_conv.is_teste, false);
  IF v_teste THEN v_cpf := NULL; END IF;
  ...
  ELSE
    ...
    IF v_cpf IS NOT NULL THEN
      SELECT array_agg(id) INTO v_ids FROM public.pacientes
        WHERE clinica_id = _clinica_id AND cpf_digits = v_cpf
          AND is_mock_data = v_teste AND teste = v_teste;
    ...
    IF v_id IS NULL THEN
      SELECT array_agg(id) INTO v_ids FROM public.pacientes
        WHERE clinica_id = _clinica_id AND is_mock_data = v_teste AND teste = v_teste
      ...
      IF cardinality(v_ids) > 0 THEN
        -- Telefone sozinho não identifica: filtre apenas candidatos com
        -- nome e nascimento compatíveis, incluindo cadastro incompleto.
        SELECT array_agg(id) INTO v_ids FROM public.pacientes
          WHERE id = ANY(v_ids) AND clinica_id = _clinica_id
            AND is_mock_data = v_teste AND teste = v_teste
            AND (public.normalizar_telefone(telefone) = v_tel
              OR public.normalizar_telefone(telefone2) = v_tel);
        IF cardinality(v_ids) > 1 THEN
          RETURN jsonb_build_object('ok', false, 'erro', 'PATIENT_AMBIGUOUS');
        END IF;
        IF coalesce(cardinality(v_ids), 0) = 0 THEN
          -- Existe possível cadastro, mas não é seguro vincular ou duplicar.
  ...
  IF v_id IS NOT NULL THEN
    SELECT * INTO v_pac FROM public.pacientes
      WHERE id = v_id AND clinica_id = _clinica_id AND is_mock_data = v_teste AND teste = v_teste FOR UPDATE;
    IF NOT FOUND OR NOT v_pac.ativo
      ...
      RETURN jsonb_build_object('ok', false, 'erro', 'PATIENT_DATA_MISMATCH');
    END IF;
    -- CPF opcional também não dispensa o cruzamento com o telefone.
    -- Vínculos já confirmados podem manter seu cadastro e completar campos.
    IF NOT coalesce(v_conv.identidade_confirmada AND v_conv.contato_paciente_id = v_id, false)
      AND public.normalizar_telefone(v_pac.telefone) IS DISTINCT FROM v_tel
    ...
    END IF;
  ELSE
    INSERT INTO public.pacientes (clinica_id, nome, data_nascimento, telefone, cpf, sexo, ativo, is_mock_data, teste)
      VALUES (_clinica_id, v_nome, _data_nascimento, v_tel, v_cpf, 'nao_informar', true, v_teste, v_teste)
  ...
  INSERT INTO public.audit_log (clinica_id, table_name, record_id, action, dados_depois)
    VALUES (_clinica_id, 'pacientes', v_id, 'NINA_CADASTRO_RESOLVIDO',
      jsonb_build_object('conversa_id', _conversa_id, 'criado', v_criado,
        'campos_completados', v_completados, 'origem', CASE WHEN v_teste THEN 'nina_homologacao' ELSE 'nina_whatsapp' END));
  RETURN jsonb_build_object('ok', true, 'paciente_id', v_id, 'criado', v_criado, 'teste', v_teste,
