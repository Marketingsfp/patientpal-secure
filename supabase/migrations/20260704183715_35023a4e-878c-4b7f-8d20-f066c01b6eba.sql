...
BEGIN
  ...
  IF v_is_num THEN
    IF length(v_digits) = 11 THEN
      ...
      SELECT p.id, 100, 'CPF exato'
       ...
       WHERE p.ativo
         ...
         AND regexp_replace(coalesce(p.cpf_digits, p.cpf, ''), '\D', '', 'g') = v_digits
       LIMIT v_limite;
    END IF;
    ...
    SELECT p.id, 95, 'Prontuário exato'
     ...
     WHERE p.ativo
       AND p.clinica_id = ANY(v_allowed)
       AND regexp_replace(coalesce(p.codigo_prontuario, ''), '\D', '', 'g') = v_digits
     LIMIT v_limite;
    ...
    SELECT p.id, 90, 'Código antigo exato'
     ...
     WHERE p.ativo
       AND p.clinica_id = ANY(v_allowed)
       AND regexp_replace(coalesce(p.codigo_prontuario_anterior, ''), '\D', '', 'g') = v_digits
     LIMIT v_limite;
    ...
    SELECT p.id, 88, 'Pasta exata'
     ...
     WHERE p.ativo
       AND p.clinica_id = ANY(v_allowed)
       AND regexp_replace(coalesce(p.numero_pasta, ''), '\D', '', 'g') = v_digits
     LIMIT v_limite;
      ...
      SELECT p.id, 85, 'Telefone'
       ...
       WHERE p.ativo
         ...
         AND (
           ...
           OR regexp_replace(coalesce(p.responsavel_telefone, ''), '\D', '', 'g') LIKE '%' || v_digits || '%'
         )
       LIMIT v_limite;
    END IF;
    ...
    IF length(v_digits) BETWEEN 3 AND 10 THEN
      ...
      SELECT p.id, 20, 'CPF parcial'
       ...
       WHERE p.ativo
         AND p.clinica_id = ANY(v_allowed)
         AND regexp_replace(coalesce(p.cpf_digits, p.cpf, ''), '\D', '', 'g') LIKE v_digits || '%'
       LIMIT v_limite;
    END IF;
  END IF;
...
GRANT EXECUTE ON FUNCTION public.buscar_pacientes_global(uuid[], text, integer) TO service_role;