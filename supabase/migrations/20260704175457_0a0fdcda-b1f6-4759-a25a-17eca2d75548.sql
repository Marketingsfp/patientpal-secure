...
BEGIN
  ...
  WITH candidates AS (
    ...
    SELECT p.id, 100 AS score, 'CPF exato'::text AS reason
     ...
     WHERE v_is_num AND length(v_digits) = 11
       AND p.ativo AND p.clinica_id = ANY(v_allowed)
       AND p.cpf_digits = v_digits
    UNION ALL
    SELECT p.id, 95, 'Prontuário exato'
      FROM public.pacientes p
     WHERE v_is_num AND p.ativo AND p.clinica_id = ANY(v_allowed)
    ...
    SELECT p.id, 85, 'Telefone'
      FROM public.pacientes p
     WHERE v_is_num AND length(v_digits) BETWEEN 8 AND 11
       AND p.ativo AND p.clinica_id = ANY(v_allowed)
    ...
    SELECT p.id, 20, 'Parcial'
      FROM public.pacientes p
     WHERE v_is_num AND length(v_digits) BETWEEN 3 AND 10
       AND p.ativo AND p.clinica_id = ANY(v_allowed)
