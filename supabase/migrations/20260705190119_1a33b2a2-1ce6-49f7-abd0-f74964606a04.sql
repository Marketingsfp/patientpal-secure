...
BEGIN
  ...
  WITH pac_raw AS (
    -- Ramo CPF exato
    SELECT p.id, p.nome, p.cpf, p.telefone, p.telefone2, p.numero_pasta, p.codigo_prontuario,
           p.data_nascimento, p.clinica_id, p.ativo, p.created_at,
           100::numeric AS raw_score
    ...
    UNION ALL
    -- Ramo dígitos curtos por prefixo (CPF/telefones/prontuário/pasta)
    SELECT p.id, p.nome, p.cpf, p.telefone, p.telefone2, p.numero_pasta, p.codigo_prontuario,
           p.data_nascimento, p.clinica_id, p.ativo, p.created_at,
    ...
    UNION ALL
    -- Ramo Nome (trigram): apenas pacientes ativos
    SELECT p.id, p.nome, p.cpf, p.telefone, p.telefone2, p.numero_pasta, p.codigo_prontuario,
           p.data_nascimento, p.clinica_id, p.ativo, p.created_at,
           (CASE
  ...
  pac AS (
    SELECT
      ...
      (max(p.raw_score) * (CASE WHEN bool_or(p.ativo) THEN 1.0 ELSE 0.7 END))::numeric AS score,
      min(p.created_at) AS criado_em
    FROM pac_raw p
    GROUP BY p.id, p.nome, p.cpf, p.telefone, p.numero_pasta, p.data_nascimento, p.clinica_id, p.codigo_prontuario
    ORDER BY score DESC, criado_em DESC LIMIT _limite
  ),
  orc AS (
