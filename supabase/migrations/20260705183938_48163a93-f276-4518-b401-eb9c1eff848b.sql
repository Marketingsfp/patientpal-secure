...
CREATE INDEX IF NOT EXISTS idx_pacientes_clinica_lower_nome
  ON public.pacientes (clinica_id, lower(nome));
CREATE INDEX IF NOT EXISTS idx_pacientes_clinica_cpf_digits
  ON public.pacientes (clinica_id, cpf_digits);
CREATE INDEX IF NOT EXISTS idx_pacientes_clinica_numero_pasta
  ON public.pacientes (clinica_id, numero_pasta);
CREATE INDEX IF NOT EXISTS idx_pacientes_clinica_cod_prontuario
  ON public.pacientes (clinica_id, codigo_prontuario);
CREATE INDEX IF NOT EXISTS idx_pacientes_clinica_tel_digits
  ON public.pacientes (clinica_id, (regexp_replace(coalesce(telefone,''),  '\D','','g')));
CREATE INDEX IF NOT EXISTS idx_pacientes_clinica_tel2_digits
  ON public.pacientes (clinica_id, (regexp_replace(coalesce(telefone2,''), '\D','','g')));
...
DECLARE
  ...
  v_is_digits boolean := length(v_digits) > 0 AND v_digits = regexp_replace(v_termo,'\s','','g');
  v_scope uuid[];
  ...
  pac AS (
    SELECT
      ...
      (CASE WHEN p.numero_pasta IS NOT NULL AND p.numero_pasta <> ''
        ...
        || CASE WHEN p.data_nascimento IS NOT NULL
                  THEN to_char(p.data_nascimento,'DD/MM/YYYY') ELSE '' END
      )::text AS hint,
      jsonb_build_object(
      ...
      (CASE
        WHEN v_is_digits AND length(v_digits) = 11 AND p.cpf_digits = v_digits THEN 100
        WHEN v_is_digits AND length(v_digits) >= 8
             AND (regexp_replace(coalesce(p.telefone,''),  '\D','','g') = v_digits
               OR regexp_replace(coalesce(p.telefone2,''), '\D','','g') = v_digits) THEN 95
        WHEN v_is_digits AND (p.numero_pasta = _termo OR p.codigo_prontuario = _termo) THEN 90
        WHEN lower(p.nome) = v_termo THEN 85
        WHEN lower(p.nome) LIKE v_prefix THEN 65
        WHEN v_is_digits AND (
    ...
    WHERE p.clinica_id = ANY(v_scope)
      ...
      AND (
        ...
        OR (v_is_digits AND length(v_digits) >= 3 AND (
             p.cpf_digits LIKE '%'||v_digits||'%'
          OR regexp_replace(coalesce(p.telefone,''),  '\D','','g') LIKE '%'||v_digits||'%'
          OR regexp_replace(coalesce(p.telefone2,''), '\D','','g') LIKE '%'||v_digits||'%'
          OR p.numero_pasta = _termo
          OR p.codigo_prontuario = _termo
        ))
      )
