-- ===========================================================================
-- Painel Executivo — atendimento realizado so ate hoje (10/09/2026)
--
-- Sao duas funcoes trocadas por CREATE OR REPLACE; nao apaga nem altera nenhum
-- dado. O corpo e o mesmo de 20260901130000_painel_executivo_correcoes.sql
-- (conferido contra o banco de producao em 10/09/2026), com UMA mudanca em
-- cada funcao: a definicao de "realizado".
--
-- O QUE ESTAVA ERRADO
-- -------------------
-- Desde 01/09 um agendamento conta como atendido quando esta marcado como
-- realizado, tem horario de execucao ou ja tem recebimento confirmado. Faltava
-- olhar a data: a recepcao cobra adiantado, e o paciente que pagou hoje uma
-- consulta marcada para daqui a duas semanas ja entrava em "Atendimentos
-- realizados". Em 10/09/2026 eram 43 agendamentos de 11/09 a 25/09 contados
-- como realizados sem terem acontecido.
--
-- Passa a valer: so conta como realizado o agendamento cujo dia (no fuso da
-- clinica) e hoje ou anterior. O dia de hoje entra inteiro — quem pagou de
-- manha a consulta das 15h esta na clinica esperando a vez. O agendamento
-- futuro pago continua sendo contado como agendado e confirmado; so deixa de
-- inflar realizados, comparecimento, tempo medio e o ranking por medico, e
-- volta a contar sozinho quando o dia dele chegar.
-- ===========================================================================
...
--
-- Mesmo corte de data aplicado a Producao: Compareceram, Taxa de
-- comparecimento, No-show, Tempo medio e o ranking por medico saem todos da
-- mesma definicao de "realizado".
-- ---------------------------------------------------------------------------
...
DECLARE
  v_resultado jsonb;
  -- Dia de hoje no fuso da clinica: agendamento depois dele nao conta como
  -- realizado, mesmo que ja esteja pago.
  v_hoje date := (now() AT TIME ZONE 'America/Sao_Paulo')::date;
BEGIN
  IF NOT public.is_member(auth.uid(), p_clinica) THEN
    RETURN NULL;
  ...
  base AS (
    SELECT a.*,
           ...
           coalesce(a.paciente_id::text, a.id::text) || '|'
             || (a.inicio AT TIME ZONE 'America/Sao_Paulo')::date::text AS chave_lab,
           -- Mesma definicao de dashboard_blocos_periodo, inclusive o corte
           -- no dia de hoje.
           ((a.status = 'realizado' OR a.executado_em IS NOT NULL OR a.pago)
            AND (a.inicio AT TIME ZONE 'America/Sao_Paulo')::date <= v_hoje) AS realizado,
           CASE
