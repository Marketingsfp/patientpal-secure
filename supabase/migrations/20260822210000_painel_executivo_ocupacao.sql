-- Painel Executivo — Ocupacao passa a considerar so quem atendeu no periodo.
--
-- ============================== O QUE MUDA AQUI ============================
-- Regrava `painel_executivo_periodo` alterando UMA coisa: quais medicos entram
-- na capacidade que serve de denominador da Ocupacao. Todo o resto segue igual.
--
-- Antes:  capacidade = agenda de TODOS os medicos com disponibilidade ativa
-- Agora:  capacidade = agenda so dos medicos que tiveram atendimento no periodo
--
-- Motivo, medido em 22/08/2026 no periodo de 24/07 a 22/08: 109 medicos tem
-- agenda cadastrada, mas apenas 51 deles atenderam alguem. Os parados
-- respondiam por 57% da capacidade (794.690 min no total contra 341.300 min de
-- quem trabalha), e puxavam a Ocupacao para 35,2% quando a agenda de quem
-- atende estava 82,0% cheia.
--
-- A leitura muda de proposito: o indicador deixa de responder "quanto da
-- estrutura cadastrada estamos usando" e passa a responder "quao cheia esta a
-- agenda de quem trabalha". A ociosidade dos cadastros parados sera tratada
-- separadamente, como limpeza de base — decisao do dono da clinica em
-- 22/08/2026.
--
-- "Movimento" aqui e ter pelo menos um agendamento nao cancelado no periodo, o
-- mesmo criterio que ja vale para os minutos agendados do numerador.
--
-- ------------------- documentacao da migracao anterior ---------------------
-- Painel Executivo — troca o divisor do Ticket Medio.
--
-- ============================== O QUE MUDA AQUI ============================
-- Regrava `painel_executivo_periodo` (criada em 20260822160000) alterando UMA
-- coisa: o divisor do Ticket Medio. Todo o resto da funcao segue identico.
--
-- Antes:  recebimentos / clientes atendidos  (agendamento marcado "realizado")
-- Agora:  recebimentos / atendimentos que geraram recebimento no periodo
--
-- Motivo, medido em 22/08/2026: no dia, 123 atendimentos ja tinham gerado
-- recebimento, mas so 12 estavam marcados como "realizado" — a recepcao da
-- baixa no status mais tarde. Com o divisor antigo o card mostrava R$ 1.245,70
-- quando o ticket real do dia era R$ 123,40. A distorcao nao e so do dia
-- corrente: nos 30 dias anteriores foram 2.672 atendimentos com recebimento
-- contra 1.593 marcados como realizados.
--
-- O card "Clientes atendidos" NAO muda — ele mede outra coisa, quantos
-- atendimentos a equipe ja deu baixa, e continua util assim. O que muda e so
-- de onde o Ticket Medio tira o divisor, para o indicador parar de depender do
-- momento em que alguem aperta o botao.
--
-- Efeito: o Ticket Medio deixa de fechar na mao com os dois cards vizinhos do
-- painel. Em troca, ele fica estavel ao longo do dia. Decisao do dono da
-- clinica em 22/08/2026.
--
-- ------------------- documentacao original da funcao ----------------------
...
-- 4. Ticket medio = recebimentos do periodo / atendimentos que geraram
--    recebimento no periodo (agendamentos distintos com receita confirmada).
--    Ja passou por duas versoes antes desta: era a media de
--    `fin_atendimentos.valor_total`, tabela que a clinica parou de alimentar em
--    29/07/2026 (dava o "R$ 3,00" da tela), e depois recebimentos / clientes
--    atendidos, que inflava porque o status "realizado" e atualizado com
--    atraso. Ver o cabecalho desta migracao.
...
-- 7. Ocupacao = minutos agendados / capacidade dos medicos que atenderam no
--    periodo. Medico com agenda cadastrada e nenhum atendimento fica de fora
--    dos dois lados da conta. Ver o cabecalho desta migracao.
...
BEGIN
  ...
  -- Medicos que efetivamente trabalharam no periodo. Ver o cabecalho desta
  -- migracao: so a agenda deles entra na capacidade.
  medicos_com_movimento AS (
    SELECT DISTINCT b.medico_id
    FROM base b
  ...
  capacidade AS (
    ...
    JOIN public.medico_disponibilidades d
     ...
     AND d.ativo
     AND d.medico_id IN (SELECT medico_id FROM medicos_com_movimento)
  ...
  SELECT jsonb_build_object(
    ...
    'financeiro', jsonb_build_object(
      ...
      -- marcados como "realizado" — ver o cabecalho desta migracao
