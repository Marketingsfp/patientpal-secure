  ...
  WITH pac AS (
    SELECT
      ...
      (CASE
        ...
        WHEN upper(public.strip_accents(p.nome)) LIKE v_norm_like THEN 35
        ELSE 5
    ...
    WHERE p.clinica_id = ANY(v_scope)
      ...
      AND (
        ...
        -- Nome: prefixo (btree idx_pacientes_nome_norm) e contém (gin trgm idx_pacientes_nome_norm_trgm)
        OR (NOT v_has_digits AND upper(public.strip_accents(p.nome)) LIKE v_norm_like)
      )
