-- ============================================================================
-- Relatório de Sessões: modo "Movimento do período"
-- ============================================================================
-- O relatório nasceu respondendo UMA pergunta: "onde cada paciente está hoje"
-- (posição). Nesse modo a data é referência, não janela — tratamento em
-- andamento aparece de qualquer data, senão a busca ativa nunca acharia quem
-- sumiu.
--
-- O financeiro e a coordenação precisam de OUTRA pergunta: "quantas sessões e
-- manutenções foram realizadas neste mês fechado", para conferir produção e
-- bater metas. Aí a data é janela de verdade.
--
-- As duas não cabem na mesma consulta sem mentir em alguma coluna, então o
-- modo entra como parâmetro. O modo 'posicao' é exatamente o que já estava
-- funcionando e não foi tocado.
--
-- O parâmetro tem DEFAULT 'posicao' de propósito: enquanto o site publicado
-- ainda for a versão antiga, que chama a função com três argumentos, a chamada
-- continua resolvendo e o relatório não quebra entre o SQL e o Publish.
-- ============================================================================

-- A versão de 3 argumentos sai de cena para não ficarem duas funções
-- concorrendo pelo mesmo nome no PostgREST. O DEFAULT acima é o que mantém a
-- chamada antiga funcionando.
drop function if exists public.fn_relatorio_sessoes(uuid, date, date);
