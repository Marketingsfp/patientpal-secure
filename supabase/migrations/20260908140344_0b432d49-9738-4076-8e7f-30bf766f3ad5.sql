  ...
  SELECT EXISTS (
    SELECT 1
     ...
     WHERE m.user_id = _user_id
       ...
       AND m.ativo = true
       AND m.role::text = 'telefonia'
  );
$function$;

GRANT EXECUTE ON FUNCTION public.atend_tem_perfil_telefonia(uuid, uuid) TO authenticated, service_role;
...
AS $function$
  WITH base AS (
    SELECT
      ...
      public.atend_tem_perfil_telefonia(p.user_id, _clinica_id) AS permission_telefonia,
      EXISTS (
