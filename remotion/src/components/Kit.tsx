...
export const Rise: React.FC<{
  ...
  return (
    <div
      style={{ opacity: s, transform: `translateY(${interpolate(s, [0, 1], [y, 0])}px)`, ...style }}
    >
      {children}
    </div>
  );
...
export const Backdrop: React.FC<{ tone?: number }> = ({ tone = 0 }) => {
  ...
  return (
    <AbsoluteFill
      style={{
        background: `radial-gradient(120% 90% at ${18 + tone * 12}% 8%, #17342C 0%, ${C.bg} 45%, ${C.bgDeep} 100%)`,
      }}
    >
      <div
        style={{
...
export const StepTag: React.FC<{ n: string; label: string; delay?: number; font: string }> = ({
  n,
  label,
  delay = 0,
  font,
  ...
  <Rise delay={delay} y={26}>
    <div style={{ display: "flex", alignItems: "center", gap: 16, fontFamily: font }}>
      ...
      <div
        style={{ color: C.creamDim, fontSize: 24, letterSpacing: 4, textTransform: "uppercase" }}
      >
        {label}
      </div>
    </div>
  ...
  return (
    <div
      style={{
        ...
        background: "#0B1A16EE",
        border: `1px solid ${C.greenSoft}44`,
        borderRadius: 26,
...
export const Cursor: React.FC<{
  ...
  const pulse = interpolate(frame, [c, c + 18], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
  return (
    ...
    >
      ...
      >
        <path
          d="M4 2 L4 20 L9 15.5 L12.5 22 L15.5 20.5 L12 14.5 L19 14 Z"
          fill={C.cream}
          stroke={C.ink}
          strokeWidth="1.2"
        />
