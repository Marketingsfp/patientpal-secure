    ...
    <AbsoluteFill>
      ...
      <AbsoluteFill style={{ padding: "90px 140px" }}>
        ...
        <Panel delay={12} style={{ maxWidth: 1520 }}>
          {CAMPOS.map(([k, v], i) => {
            const op = interpolate(frame, [16 + i * 9, 28 + i * 9], [0, 1], {
        ...
        <Rise delay={82} y={24}>
          <div style={{ fontFamily: body, color: C.creamDim, fontSize: 28, maxWidth: 1400 }}>
            O <span style={{ color: C.clay, fontWeight: 700 }}>mês de referência</span> é o que
            manda: ele define a parcela 1 e o vencimento das seguintes.
          </div>
        </Rise>
      </AbsoluteFill>
    </AbsoluteFill>
