...
export const AgendaSceneFiltro: React.FC = () => {
  ...
  const cx = interpolate(f, [0, 18, 50, 70, 96, 115], [1500, 360, 360, 880, 1020, 1020], {
    extrapolateRight: "clamp",
  });
  const cy = interpolate(f, [0, 18, 50, 70, 96, 115], [800, 250, 250, 250, 460, 460], {
    extrapolateRight: "clamp",
  });
  ...
  return (
    <Frame title="Agendas" step="Filtros — profissional e data">
      <div
        style={{ display: "flex", gap: 16, opacity: s, transform: `translateY(${(1 - s) * 10}px)` }}
      >
        {/* Filtro profissional */}
        <div style={{ position: "relative" }}>
          <div
            style={{
              ...
              gap: 16,
              boxShadow: openMedico
                ? "0 10px 24px rgba(29,78,216,.18)"
                : "0 2px 8px rgba(0,0,0,.04)",
            }}
          >
            <span style={{ fontSize: 22, fontWeight: 600, color: C.ink, fontFamily: dm }}>
              ...
              {MEDICOS.map((m, i) => (
                <div
                  ...
                  style={{
                    ...
                    fontSize: 20,
                    color: C.ink,
                    background: i === selectedMedico ? C.primarySoft : "transparent",
                    fontWeight: i === selectedMedico ? 600 : 400,
                  }}
                >
                  {m}
                </div>
        ...
        <div style={{ position: "relative" }}>
          <div
            style={{
              ...
              alignItems: "center",
              gap: 14,
              boxShadow: openCal ? "0 10px 24px rgba(29,78,216,.18)" : "0 2px 8px rgba(0,0,0,.04)",
            }}
          >
            <svg
              width="22"
            ...
            <span style={{ fontSize: 22, fontWeight: 600, color: C.ink, fontFamily: dm }}>
              {String(selectedDay).padStart(2, "0")} de maio · 2025
            </span>
          </div>
          {openCal && (
            <div
            ...
            >
              ...
              >
                {["D", "S", "T", "Q", "Q", "S", "S"].map((d) => (
                  <div key={d} style={{ textAlign: "center", color: C.sub, padding: 4 }}>
                    {d}
                  </div>
                ))}
      ...
      >
        ...
        >
          <div style={{ fontSize: 24, fontWeight: 700, fontFamily: dm }}>
            {MEDICOS[selectedMedico]} · {String(selectedDay).padStart(2, "0")}/05
          </div>
        ...
        {["08:00", "08:30", "09:00", "09:30", "10:00", "10:30"].map((h, i) => (
          ...
          >
            <div style={{ fontSize: 20, fontWeight: 700, fontFamily: dm, color: C.primary }}>
              {h}
            </div>
            <div style={{ fontSize: 19, color: C.ink }}>
              {selectedDay === 22 && i < 3
                ? ["Pedro Costa", "João Ribeiro", "Helena Vargas"][i]
                : selectedDay === 17 && i === 0
                  ? "Lucas Andrade"
                  : "—"}
            </div>
