...
export const SceneFluxo: React.FC = () => {
  ...
  return (
    <Frame title="Fluxo do paciente" step="Etapa 4 — Após o pagamento: Triagem → Atendimento">
        ...
        {cols.map((c) => (
          <div key={c.id} style={{ background: "#fff", border: `1px solid ${C.line}`, borderRadius: 16, padding: 18, minHeight: 480 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ background: c.c, color: c.t, padding: "8px 14px", borderRadius: 999, fontSize: 18, fontWeight: 600, fontFamily: dm }}>{c.l}</div>
              <div style={{ color: C.sub, fontSize: 16 }}>1</div>
            </div>
            <div style={{ marginTop: 16, opacity: 0.35 }}>
              <div style={{ border: `2px dashed ${C.line}`, borderRadius: 12, padding: 22, textAlign: "center", color: C.sub, fontSize: 16 }}>vazio</div>
            </div>
          </div>
        ))}
      </div>
      ...
      {/* moving ticket card */}
      <div style={{
        position: "absolute", left: ticketX + 18, top: 90 - lift,
        width: 300, background: "#fff", borderRadius: 14, padding: 14,
        boxShadow: "0 20px 40px rgba(0,0,0,.18)", border: `2px solid ${C.primary}`,
      }}>
