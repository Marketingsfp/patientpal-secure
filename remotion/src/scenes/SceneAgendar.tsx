...
export const SceneAgendar: React.FC = () => {
  ...
  const cx = interpolate(f, [0, 30, 55, 85, 105], [1500, 880, 880, 1380, 1380], { extrapolateRight: "clamp" });
  const cy = interpolate(f, [0, 30, 55, 85, 105], [800, 380, 380, 720, 720], { extrapolateRight: "clamp" });
    ...
    <Frame title="Consulta rápida" step="Etapa 2 — Por senha (horário como referência)">
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 24, opacity: s, transform: `translateY(${(1 - s) * 12}px)` }}>
        ...
        <div style={{ background: "#fff", borderRadius: 16, padding: 24, border: `1px solid ${C.line}` }}>
          ...
          <div style={{ marginTop: 24, display: "flex", justifyContent: "flex-end", gap: 12 }}>
            ...
            <div style={{ padding: "14px 22px", borderRadius: 12, background: C.accent, color: "#fff", fontSize: 20, fontWeight: 600, fontFamily: dm, boxShadow: "0 10px 24px rgba(16,185,129,.35)" }}>Confirmar agendamento</div>
          </div>
        </div>
      </div>
      ...
      {confirmed && (
        <div style={{
          ...
          opacity: confSpring, background: C.accent, color: "#fff",
