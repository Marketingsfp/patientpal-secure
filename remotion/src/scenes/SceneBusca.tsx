...
export const SceneBusca: React.FC = () => {
  ...
  const typedChars = Math.max(
    0,
    Math.min(TEXT.length, Math.floor(interpolate(f, [15, 60], [0, TEXT.length + 1]))),
  );
  ...
  const cx = interpolate(f, [0, 25, 60, 95, 130], [1200, 720, 720, 760, 760], {
    extrapolateRight: "clamp",
  });
  const cy = interpolate(f, [0, 25, 60, 95, 130], [700, 240, 240, 430, 430], {
    extrapolateRight: "clamp",
  });
      ...
      {showResult && (
        <div
          style={{
            ...
            transform: `translateY(${(1 - resultSpring) * 16}px)`,
          }}
        >
          {[
            ...
            <div
              ...
              style={{
                background: "#fff",
                border: `1px solid ${i === 0 ? C.primary : C.line}`,
                borderRadius: 16,
                padding: 22,
                display: "flex",
                alignItems: "center",
                gap: 16,
                boxShadow: i === 0 ? "0 10px 30px rgba(29,78,216,.18)" : "none",
              }}
            >
              <div
                style={{
                  ...
                  height: 54,
                  borderRadius: 999,
                  background: i === 0 ? C.primarySoft : "#EEF1F7",
                  color: C.primary,
                  display: "flex",
                  alignItems: "center",
      ...
      {f > 90 && (
        <div
          style={{
            ...
            border: `1px solid ${C.line}`,
            opacity: cardSpring,
            transform: `translateY(${(1 - cardSpring) * 16}px)`,
          }}
        >
          <div style={{ fontSize: 20, color: C.sub, fontFamily: dm, marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            {[
              ...
              <div
                key={i}
                style={{
                  background: p.on ? C.primarySoft : "#F2F4F9",
                  border: `1px solid ${p.on ? C.primary : C.line}`,
                  padding: "14px 18px",
                  borderRadius: 12,
                  display: "flex",
