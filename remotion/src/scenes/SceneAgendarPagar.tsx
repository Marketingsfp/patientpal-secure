    ...
    <Frame title="Consulta rápida" step="Etapa 2 — Agendar e pagar (mesma tela)">
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 24, opacity: s, transform: `translateY(${(1 - s) * 12}px)` }}>
        ...
        <div style={{ background: "#fff", borderRadius: 16, padding: 24, border: `1px solid ${C.line}` }}>
          ...
          <div style={{ fontSize: 26, fontWeight: 700, marginTop: 4, fontFamily: dm }}>Maria Souza</div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8 }}>
            ...
            <span style={{ color: C.primary, fontSize: 22, fontWeight: 700, fontFamily: dm }}>R$ 250,00</span>
          </div>

          {confirmed && (
            <div style={{
              marginTop: 22, padding: "14px 16px", borderRadius: 12,
              background: "rgba(16,185,129,.12)", border: `1px solid ${C.accent}`,
              display: "flex", alignItems: "center", gap: 12, opacity: Math.min(1, confSpring),
            }}>
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={C.accent} strokeWidth="3"><path d="M5 12l5 5L20 7"/></svg>
              <div>
                <div style={{ fontSize: 18, fontWeight: 700, color: C.accent, fontFamily: dm }}>Agendamento confirmado</div>
        ...
        <div style={{ background: "#fff", borderRadius: 16, padding: 24, border: `1px solid ${C.line}`, position: "relative", minHeight: 520 }}>
          {!showPay && (
            <>
                ...
                {slots.map((t, i) => {
                  ...
                  return (
                    <div key={t} style={{
                      ...
                      <div style={{ fontSize: 14, marginTop: 2, opacity: selected ? 0.85 : 0.6, fontFamily: dm }}>ref. {t}</div>
                    </div>
                  );
                })}
              </div>
              <div style={{ marginTop: 24, display: "flex", justifyContent: "flex-end", gap: 12 }}>
                <div style={{ padding: "14px 22px", borderRadius: 12, border: `1px solid ${C.line}`, fontSize: 20 }}>Cancelar</div>
                <div style={{
                  padding: "14px 22px", borderRadius: 12,
                  background: confirmed ? C.accent : C.accent, color: "#fff",
                  fontSize: 20, fontWeight: 600, fontFamily: dm,
                  boxShadow: "0 10px 24px rgba(16,185,129,.35)",
                }}>{confirmed ? "Confirmado ✓" : "Confirmar e pagar"}</div>
              </div>
            </>
          )}
          ...
          {showPay && (
            <div style={{ opacity: payIn, transform: `translateY(${(1 - payIn) * 14}px)` }}>
              <div style={{ fontSize: 20, color: C.sub, fontFamily: dm, marginBottom: 14 }}>Pagamento · sem trocar de tela</div>
              ...
              <div style={{ marginTop: 22 }}>
                ...
                <div style={{ marginTop: 12, color: processing ? C.primary : done ? C.accent : C.sub, fontSize: 18, fontWeight: 600, fontFamily: dm }}>
                  {processing ? "Processando pagamento…" : done ? "Pagamento aprovado · R$ 250,00 via PIX" : "Selecione a forma de pagamento"}
                </div>
              </div>
            </div>
          )}
      ...
      {confirmed && !showPay && (
        <div style={{
          ...
          padding: "22px 36px", borderRadius: 999, fontSize: 28, fontWeight: 700, fontFamily: dm,
