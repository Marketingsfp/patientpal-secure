...
export function useClientesV2Flag() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const prefs = (data?.preferencias_ui ?? {}) as { flags?: { clientes_v2?: boolean } };
      if (alive) { setEnabled(Boolean(prefs.flags?.clientes_v2)); setLoading(false); }
    })();
    const onChange = (e: Event) => {
      const ce = e as CustomEvent<{ clientes_v2: boolean }>;
      if (ce.detail) setEnabled(Boolean(ce.detail.clientes_v2));
    };
  ...
  const set = useCallback(async (v: boolean) => {
    ...
    const flags = { ...((prev.flags as object) ?? {}), clientes_v2: v };
