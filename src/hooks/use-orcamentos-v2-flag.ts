...
export function useOrcamentosV2Flag() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const prefs = (data?.preferencias_ui ?? {}) as { flags?: { orcamentos_v2?: boolean } };
      if (alive) { setEnabled(Boolean(prefs.flags?.orcamentos_v2)); setLoading(false); }
    })();
    const onChange = (e: Event) => {
      const ce = e as CustomEvent<{ orcamentos_v2: boolean }>;
      if (ce.detail) setEnabled(Boolean(ce.detail.orcamentos_v2));
    };
  ...
  const set = useCallback(async (v: boolean) => {
    ...
    const flags = { ...((prev.flags as object) ?? {}), orcamentos_v2: v };
