/**
 * VALIDAÇÃO A–I — reinício de sessão da homologação só pelo botão.
 *
 ...
 *    escreve sessão/telefone virtual nem emite reset de memória.
 *
...
function semear() {
  ...
  for (let i = 1; i <= 2; i++) {
    ...
    conversas.push({
      ...
      nina_fluxo_estado: { intencao: "agendar", paciente_nome: "João", especialidade: "neurologista" },
    ...
    mensagens.push(
      { id: `m-${i}-1`, clinica_id: CLINICA, conversa_id: `conv-${i}`, direction: "in", body: "ola bom dia" },
      { id: `m-${i}-2`, clinica_id: CLINICA, conversa_id: `conv-${i}`, direction: "out", body: "Bom dia!" },
    );
    batches.push({ id: `b-${i}`, clinica_id: CLINICA, conversa_id: `conv-${i}`, status: "PROCESSING", processed_at: null });
    locks.push({ id: `lk-${i}`, clinica_id: CLINICA, chave: `${CLINICA}:55001${i}00003`, liberado_em: null, expira_em: "x" });
...
// ============================================================== A, B, C, H
describe("A/B/C/H — nenhum caminho automático reinicia sessão", () => {
  it("A — só a rotina do botão escreve sessão e telefone virtual", () => {
    ...
    const alvos = [
      ...
      "src/lib/nina/carga.functions.ts",
      "src/lib/nina/carga-preflight.server.ts",
  ...
  it("H — Terra, cenários, carga e correção assistida são recusados no servidor", async () => {
    const { resetarLeadTeste, ResetManualObrigatorioError } = await import(
      "@/lib/nina/teste-console.server"
    );
    const origens = ["terra", "cenario_inicio", "cenario_fim", "carga_preflight", "correcao_reteste"];
...
describe("E/I — botão manual: alcance, auditoria e continuidade", () => {
  ...
  it("I — contexto ativo antes do reset; contexto zerado depois", async () => {
    ...
    const estadoAtivo = JSON.stringify({ leads, conversasAbertas: conversas.filter((c) => c.status === "open" && c.id === "conv-1") });
