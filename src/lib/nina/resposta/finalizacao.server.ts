...
function guardar(chave: string, valor: RespostaFinalizada): RespostaFinalizada {
...
export async function finalizarResposta(
  ...
  const jaFeita = finalizados.get(pedido.chaveTurno);
  ...
  return guardar(pedido.chaveTurno, finalizada);
}
