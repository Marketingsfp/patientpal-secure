...
export function AtendInbox() {
  ...
  const [filtroStatus, setFiltroStatus] = useState<"all" | "active" | "waiting" | "closed" | "bot_attending">("all");
  ...
  const transferir = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await transferirFn({ data: { clinicaId, conversaId: sel.id, userId: userId || null, departamentoId, motivo } });
  ...
  const statusBadge = (s: string) => {
    if (s === "active") return <Badge className="bg-emerald-500/15 text-emerald-600 hover:bg-emerald-500/20">Ativa</Badge>;
    if (s === "waiting") return <Badge className="bg-amber-500/15 text-amber-600 hover:bg-amber-500/20">Em espera</Badge>;
      ...
      <Card className="col-span-12 md:col-span-4 lg:col-span-3 flex flex-col overflow-hidden">
        <CardHeader className="py-3 space-y-2">
          ...
          <div className="relative">
            ...
            <Input value={busca} onChange={(e) => setBusca(e.target.value)} className="pl-7 h-8 text-sm" placeholder="Buscar nome, telefone, protocolo…" />
          </div>
        ...
        <div className="flex-1 overflow-auto border-t">
          ...
          {convs.map((c) => (
            ...
            >
              <div className="flex items-center gap-2">
                <span className="font-medium text-sm truncate flex-1">{c.contato_nome || c.contato_telefone || "—"}</span>
                {c.unread_count > 0 && (
                  <Badge className="bg-emerald-500 text-white text-xs px-1.5 py-0">{c.unread_count}</Badge>
                )}
              </div>
              <div className="flex items-center gap-1.5 mt-1">
                {statusBadge(c.status)}
                {c.protocol_number && <code className="text-[10px] text-muted-foreground">#{c.protocol_number}</code>}
              </div>
              <div className="text-xs text-muted-foreground truncate mt-1">{c.ultima_msg_preview || "—"}</div>
              <div className="text-[10px] text-muted-foreground mt-0.5">{fmtData(c.ultima_msg_em)}</div>
            </button>
          ))}
        </div>
      </Card>
        ...
        {!sel ? (
          ...
          <>
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground flex items-center gap-2">
                    ...
                    {sel.protocol_number && <> · <code>#{sel.protocol_number}</code></>}
                    {sel.sla_first_response_seg != null && <> · 1ª resp: {fmtSeg(sel.sla_first_response_seg)}</>}
                  </p>
                </div>
            ...
            <div className="flex-1 overflow-auto p-4 space-y-2 bg-muted/20">
              {msgs.length === 0 && <p className="text-sm text-muted-foreground text-center">Sem mensagens.</p>}
              {msgs.map((m) => {
                const out = m.direction === "out";
                return (
                  <div key={m.id} className={`flex ${out ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[75%] rounded-2xl px-3 py-2 text-sm shadow-sm break-words ${
                      ...
                      <div className={`text-[10px] mt-1 ${out ? "text-emerald-50" : "text-muted-foreground"}`}>
            ...
            <>
              {contato.paciente ? (
                <section>
                  ...
                  <div className="text-xs text-muted-foreground space-y-0.5 mt-1">
                    ...
                    {contato.paciente.cidade && <div>📍 {contato.paciente.cidade}/{contato.paciente.estado}</div>}
                  </div>
                </section>
              ) : (
                <section>
                  <div className="text-xs text-muted-foreground">Não vinculado a paciente cadastrado.</div>
                </section>
              )}
                ...
                <section>
                  ...
                  {contato.agendamentos.map((a: any) => (
                    <div key={a.id} className="text-xs border rounded p-2 mb-1">
                      ...
                      <div className="text-muted-foreground">{fmtData(a.inicio)} · {a.status}</div>
                    </div>
                  ))}
                </section>
              )}
                ...
                <section>
                  ...
                  {contato.contratos.map((c: any) => (
                    <div key={c.id} className="text-xs border rounded p-2 mb-1">
                      ...
                      <div className="text-muted-foreground">{c.status} · {fmtData(c.data_inicio)}</div>
                    </div>
                  ))}
                </section>
              )}
              ...
              <section>
                ...
                <div className="space-y-1.5">
                  ...
                  {notas.map((n: any) => (
                    <div key={n.id} className="text-xs bg-amber-500/10 border border-amber-500/20 rounded p-2">
                      <div className="whitespace-pre-wrap">{n.conteudo}</div>
                      <div className="text-[10px] text-muted-foreground mt-1">{fmtData(n.created_at)}</div>
                    </div>
                  ))}
                </div>
                <div className="flex gap-1 mt-2">
                  <Textarea value={novaNota} onChange={(e) => setNovaNota(e.target.value)} rows={2} className="text-xs" placeholder="Nota interna (não vai para o paciente)…" />
                  <Button size="sm" variant="outline" onClick={adicionarNota} disabled={!novaNota.trim()}>
              ...
              {contato.atribuido_nome && (
                <section className="text-xs text-muted-foreground">
                  Atribuída a <span className="font-medium text-foreground">{contato.atribuido_nome}</span>
                </section>
              )}
            </>
          )}
      ...
      <Dialog open={transferOpen} onOpenChange={setTransferOpen}>
        <DialogContent>
          ...
          <form onSubmit={transferir} className="space-y-3">
            ...
            <div><Label>Motivo</Label><Input name="motivo" maxLength={200} /></div>
            <DialogFooter>
      ...
      <Dialog open={fecharOpen} onOpenChange={setFecharOpen}>
        <DialogContent>
          ...
          <form onSubmit={fechar} className="space-y-3">
            <div><Label>Motivo</Label><Input name="motivo" maxLength={120} placeholder="Resolvido, sem resposta, etc." /></div>
            <div><Label>Resumo do atendimento</Label><Textarea name="resumo" rows={4} maxLength={2000} /></div>
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setFecharOpen(false)}>Cancelar</Button>
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Stat label="Alertas SLA" value={alertas.length} tone="text-destructive" icon={AlertTriangle} />
      </div>

      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="flex items-center gap-2"><Eye className="h-5 w-5" /> Conversas em tempo real</CardTitle>
        ...
        <CardContent className="p-0">
          <div className="overflow-auto max-h-[60vh]">
            <table className="w-full text-sm">
              ...
              <tbody>
                {rows.length === 0 && (
                  <tr><td colSpan={7} className="p-4 text-center text-muted-foreground">Nenhuma conversa ativa.</td></tr>
                )}
                {rows.map((r) => {
                  const espMin = r.aguardando_desde ? (Date.now() - new Date(r.aguardando_desde).getTime()) / 60000 : null;
                  ...
                  return (
                    <tr key={r.id} className={`border-t ${alerta ? "bg-destructive/5" : ""}`}>
                      ...
                      <td className="p-2">
                        <Badge variant={r.status === "waiting" ? "secondary" : "outline"} className={r.status === "active" ? "bg-emerald-500/15 text-emerald-600" : ""}>
                          {r.status}
                        </Badge>
                      </td>
                      <td className="p-2">
                        {r.agente_nome ?? <span className="text-muted-foreground">—</span>}
                        {r.agente_em_pausa && <Badge variant="outline" className="ml-1 text-[10px]">em pausa</Badge>}
                      </td>
                      <td className="p-2 text-muted-foreground">{r.departamento_nome ?? "—"}</td>
                      <td className="p-2">
                        {espMin != null ? (
                          <span className={alerta ? "text-destructive font-medium" : ""}>{Math.round(espMin)}min</span>
                        ) : "—"}
                      </td>
                      <td className="p-2">{fmtSeg(r.sla_first_response_seg)}</td>
                      <td className="p-2 text-xs text-muted-foreground max-w-[280px] truncate">{r.ultima_msg_preview}</td>
                    </tr>
                  );
                })}
              </tbody>
...
function Stat({ label, value, tone, icon: Icon }: { label: string; value: any; tone?: string; icon: any }) {
  return (
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
            ...
            <Stat label="Ativas" value={data?.totais?.ativas ?? "—"} tone="text-emerald-500" icon={Users} />
            <Stat label="Em espera" value={data?.totais?.espera ?? "—"} tone="text-amber-500" icon={Clock} />
      ...
      <Card>
        ...
        <CardContent className="p-0 overflow-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {(data?.agentes ?? []).length === 0 && <tr><td colSpan={4} className="p-4 text-center text-muted-foreground">Sem dados.</td></tr>}
      ...
      <Card>
        ...
        <CardContent className="p-0 overflow-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {(data?.departamentos ?? []).length === 0 && <tr><td colSpan={3} className="p-4 text-center text-muted-foreground">Sem dados.</td></tr>}
...
export function AtendRoteamento() {
  ...
  const handleSalvar = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!clinicaId) return;
    const fd = new FormData(e.currentTarget);
    const pal = palavras.split(",").map((p) => p.trim()).filter(Boolean);
    try {
      await salvar({ data: {
        clinicaId, id: edit?.id,
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle className="flex items-center gap-2"><ArrowRightLeft className="h-5 w-5" /> Roteamento automático</CardTitle>
        <Button size="sm" onClick={abrirNovo}><Plus className="h-4 w-4 mr-1" /> Nova regra</Button>
      </CardHeader>
      <CardContent className="space-y-2">
        {rows.length === 0 && (
          <p className="text-sm text-muted-foreground">
            Nenhuma regra. Regras direcionam novas conversas a departamentos com base em canal, palavras-chave ou horário.
          </p>
        )}
        {rows.map((r) => (
          <div key={r.id} className="flex items-start justify-between rounded-lg border p-3 gap-3">
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <Badge variant="outline" className="text-xs">ordem {r.ordem}</Badge>
                {!r.ativo && <Badge variant="outline">Inativa</Badge>}
                {r.canal && <Badge variant="secondary" className="text-xs">{r.canal}</Badge>}
                <Badge variant="outline" className="text-xs">→ {deptoNome(r.departamento_id)}</Badge>
              </div>
              <div className="text-xs text-muted-foreground mt-1 space-y-0.5">
                {(r.palavras_chave ?? []).length > 0 && <div>Palavras: {(r.palavras_chave ?? []).join(", ")}</div>}
                {(r.horario_inicio || r.horario_fim) && <div>Horário: {r.horario_inicio ?? "00:00"} – {r.horario_fim ?? "23:59"}</div>}
                <div>Dias: {(r.dias_semana ?? []).map((d: number) => DIAS.find((x) => x.v === d)?.l).join(", ")}</div>
        ...
        <DialogContent className="max-w-xl">
          ...
          <form onSubmit={handleSalvar} className="space-y-3">
            ...
            <div>
              ...
              <Input value={palavras} onChange={(e) => setPalavras(e.target.value)} placeholder="agendamento, marcar, consulta" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><Label>Horário início</Label><Input name="horario_inicio" type="time" defaultValue={edit?.horario_inicio ?? ""} /></div>
              <div><Label>Horário fim</Label><Input name="horario_fim" type="time" defaultValue={edit?.horario_fim ?? ""} /></div>
            </div>
            <div>
              <Label>Dias da semana</Label>
              <div className="flex flex-wrap gap-1 mt-1">
                {DIAS.map((d) => (
                  <Button key={d.v} type="button" size="sm" variant={diasSel.includes(d.v) ? "default" : "outline"} onClick={() => toggleDia(d.v)}>
                    {d.l}
                  </Button>
                ))}
              </div>
            </div>
            <div><Label>Mensagem automática (opcional)</Label><Textarea name="mensagem_auto" defaultValue={edit?.mensagem_auto ?? ""} maxLength={1000} rows={2} placeholder="Olá! Em instantes um atendente vai te responder." /></div>
