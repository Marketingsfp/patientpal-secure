...
export function PlanosPage({ modulo = "planos" }: { modulo?: string } = {}) {
  ...
  const updEdit = (patch: Partial<Plano>) => setEditing((e) => (e ? { ...e, ...patch } : e));
      ...
      <Dialog open={!!editing} onOpenChange={(v) => !v && setEditing(null)}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          {editing ? (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <div>
                ...
                <CurrencyInput
                  value={String(editing.valor_mensal ?? "")}
                  onChange={(v) => updEdit({ valor_mensal: Number(v) || 0 })}
                />
              </div>
              <div>
                <Label>Taxa de adesão (R$)</Label>
                <CurrencyInput
                  value={String(editing.taxa_adesao ?? "")}
                  onChange={(v) => updEdit({ taxa_adesao: Number(v) || 0 })}
                />
              </div>
              <div>
                ...
                <Input
                  type="number"
                  value={editing.num_parcelas}
                  onChange={(e) => updEdit({ num_parcelas: Number(e.target.value) })}
                />
              </div>
              <div>
                ...
                <Input
                  type="number"
                  value={editing.max_dependentes}
                  onChange={(e) => updEdit({ max_dependentes: Number(e.target.value) })}
                />
              </div>
              <div>
                <Label>Máx. agregados</Label>
                <Input
                  type="number"
                  value={editing.max_agregados}
                  onChange={(e) => updEdit({ max_agregados: Number(e.target.value) })}
                />
              </div>
              <div>
                ...
                <Input
                  type="number"
                  value={editing.fidelidade_meses}
                  onChange={(e) => updEdit({ fidelidade_meses: Number(e.target.value) })}
                />
              </div>
              <div>
                ...
                <Input
                  type="number"
                  value={editing.vigencia_meses}
                  onChange={(e) => updEdit({ vigencia_meses: Number(e.target.value) })}
                />
              </div>
              <div className="col-span-full">
                ...
                <Textarea
                  rows={3}
                  value={editing.descricao_beneficios ?? ""}
                  onChange={(e) => updEdit({ descricao_beneficios: e.target.value })}
                />
              </div>
              <div className="col-span-full">
                <Label>
                  Modelo do contrato (use{" "}
                  {`{{VALOR_MENSAL}}, {{PACIENTE_NOME}}, {{DEPENDENTES}}, {{CLINICA_NOME}}`} etc.)
                </Label>
                <Textarea
                  rows={10}
                  value={editing.template_contrato ?? ""}
                  onChange={(e) => updEdit({ template_contrato: e.target.value })}
                />
              </div>
            </div>
          ) : null}
          <DialogFooter>
            ...
            {editing && podeEscrever ? (
              <Button onClick={() => salvar(editing)}>
