...
export function PacienteQuickActions({ pacienteId, clinicaId }: Props) {
  ...
  async function buscarCep() {
    ...
    try {
      ...
      if (j?.erro) { toast.info("CEP não encontrado."); return; }
      setData(d => ({
        ...d,
  ...
  if (loading) {
    return <div className="text-xs text-muted-foreground flex items-center gap-1"><Loader2 className="h-3 w-3 animate-spin"/> Carregando dados…</div>;
  }
  ...
  return (
    <div className="rounded-md border bg-muted/30 p-2 space-y-2">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <Button type="button" size="sm" variant="outline" onClick={() => setFotoOpen(true)} title="Tirar foto do paciente">
          {fotoPreviewUrl ? (
...
function EnderecoDialog({
  open, onClose, data, setData, onSave, onBuscarCep,
}: {
  open: boolean; onClose: () => void; data: PacData;
  setData: React.Dispatch<React.SetStateAction<PacData>>;
  onSave: () => Promise<void>; onBuscarCep: () => Promise<void>;
}) {
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) onClose(); }}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><MapPin className="h-5 w-5"/> Endereço do paciente</DialogTitle>
          <DialogDescription>Atualize CEP, rua, número, bairro e demais dados. Use o botão buscar para preencher pelo CEP.</DialogDescription>
        </DialogHeader>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
          <div className="sm:col-span-1 space-y-1">
            <Label className="text-xs">CEP</Label>
            <div className="flex gap-1">
              <Input value={data.cep ?? ""} onChange={(e) => setData(d => ({ ...d, cep: e.target.value }))} onBlur={onBuscarCep}/>
            </div>
          </div>
          <div className="sm:col-span-2 space-y-1">
            <Label className="text-xs">Logradouro</Label>
            <Input value={data.logradouro ?? ""} onChange={(e) => setData(d => ({ ...d, logradouro: e.target.value }))}/>
          </div>
          <div className="sm:col-span-1 space-y-1">
            <Label className="text-xs">Número</Label>
            <Input value={data.numero ?? ""} onChange={(e) => setData(d => ({ ...d, numero: e.target.value }))}/>
          </div>
          <div className="sm:col-span-2 space-y-1">
            <Label className="text-xs">Complemento</Label>
            <Input value={data.complemento ?? ""} onChange={(e) => setData(d => ({ ...d, complemento: e.target.value }))}/>
          </div>
          <div className="sm:col-span-2 space-y-1">
            <Label className="text-xs">Bairro</Label>
            <Input value={data.bairro ?? ""} onChange={(e) => setData(d => ({ ...d, bairro: e.target.value }))}/>
          </div>
          <div className="sm:col-span-1 space-y-1">
            <Label className="text-xs">UF</Label>
            <Input value={data.estado ?? ""} maxLength={2} onChange={(e) => setData(d => ({ ...d, estado: e.target.value.toUpperCase() }))}/>
          </div>
          <div className="sm:col-span-3 space-y-1">
            <Label className="text-xs">Cidade</Label>
            <Input value={data.cidade ?? ""} onChange={(e) => setData(d => ({ ...d, cidade: e.target.value }))}/>
          </div>
        </div>
        <DialogFooter>
          <Button variant="ghost" onClick={onClose}><X className="h-4 w-4 mr-1"/>Cancelar</Button>
          <Button onClick={() => void onSave()}><Save className="h-4 w-4 mr-1"/>Salvar</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
function FotoDialog({
  open, onClose, pacienteId, clinicaId, atualUrl, onSaved,
}: {
  open: boolean; onClose: () => void; pacienteId: string; clinicaId: string;
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) { stop(); onClose(); } }}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Camera className="h-5 w-5"/> Foto do paciente</DialogTitle>
          <DialogDescription>{preview ? "Confirme ou tire outra foto." : "Posicione o paciente na câmera e capture."}</DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter className="gap-2">
          ...
          {preview ? (
            <>
              <Button variant="outline" onClick={() => setPreview(null)}>Tirar outra</Button>
              <Button onClick={() => void salvar()} disabled={busy}>
                {busy ? <Loader2 className="h-4 w-4 mr-1 animate-spin"/> : <Check className="h-4 w-4 mr-1"/>} Salvar foto
              </Button>
            </>
          ) : (
            <Button onClick={() => void capturar()}><Camera className="h-4 w-4 mr-1"/> Capturar</Button>
          )}
        </DialogFooter>
      </DialogContent>
    </Dialog>
