...
export function ConferenciaPage({ produto }: { produto: ProdutoCartao }) {
  ...
  const carregar = async () => {
    ...
    // Só os convênios DESTE produto (Benefícios ou Terapêutico).
    ...
    const idsDoProduto = new Set(convenios.keys());
    const contratosDoProduto = contratos.filter((c) =>
      contratoDoProduto(produto, idsDoProduto, c.convenio_id),
    );
    ...
    const diagnosticadas = contratosDoProduto.map((c) => ({
      contrato: c,
