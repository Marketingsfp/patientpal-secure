...
export function RelatoriosPage({ produto }: { produto: ProdutoCartao }) {
  ...
  const load = async () => {
    ...
    const [cs, ps, ds] = await Promise.all([
      supabase
        ...
        .select(
          "id, numero, paciente_id, paciente_nome, convenio_id, valor_mensal, taxa_adesao, status, data_inicio, assinado_em",
        )
        .eq("clinica_id", cid)
      ...
      supabase
        .from("cb_convenios")
        .select("id, nome, modalidade, valor_mensal")
        .eq("clinica_id", cid)
        .eq("produto", produto),
    ...
    // Só o que é deste produto (Benefícios ou Terapêutico).
    const idsDoProduto = new Set(((ps.data ?? []) as Array<{ id: string }>).map((p) => p.id));
    const doProduto = (c: Contrato) => contratoDoProduto(produto, idsDoProduto, c.convenio_id);
    const cList = ((cs.data ?? []) as Contrato[]).filter(doProduto);
    ...
    const allCsRes = await supabase
      ...
      .eq("clinica_id", cid)
      .limit(10000);
    const allCList = ((allCsRes.data ?? []) as Contrato[]).filter(doProduto);
    const allCIds = allCList.map((c) => c.id);
    const allDepsRes = allCIds.length
      ? await supabase
          .from("contrato_dependentes")
          .select("id, contrato_id, paciente_id, paciente_nome, tipo, ativo")
          .in("contrato_id", allCIds)
          .limit(20000)
      : { data: [] as Dep[] };
    const allMensRes = allCIds.length
      ? await supabase
          .from("contrato_mensalidades")
          .select("id, contrato_id, numero_parcela, valor, status, pago_em, vencimento")
          .in("contrato_id", allCIds)
          .eq("status", "pago")
          .limit(50000)
      : { data: [] as Mens[] };
  ...
  const stats = useMemo(() => {
    ...
    const pagasPeriodo = mens.filter(
    ...
    const porPlanoAll = planos
      .map((p) => {
        ...
        const receitaPaga = allMens
          .filter((m) => csIds.has(m.contrato_id))
          .reduce((s, m) => s + Number(m.valor), 0);
    ...
    const consultasTitulares = contratos
      .map((c) => ({
    ...
    const consultasDependentes = deps
      .map((d) => ({
    ...
    const utilizacaoPct =
      titulares + dependentesCount > 0
        ...
        : 0;
    const mediaConsultasPessoa =
      titulares + dependentesCount > 0 ? usoTotal / (titulares + dependentesCount) : 0;
    void titularIds;
    void depIds;
    ...
    if (which === "titulares") {
      setDrill({
        ...
        columns: [
          ...
          { key: "valor", label: "Mensal", align: "right" },
        ],
        rows: contratos.map((c) => ({
          nome: c.paciente_nome,
          plano: planoNome.get(c.convenio_id) ?? "—",
          status: c.status,
          valor: BRL(c.valor_mensal),
        })),
      });
      ...
      const rows = [
        ...contratos.map((c) => ({
          nome: c.paciente_nome,
          tipo: "Titular",
          vinculo: planoNome.get(c.convenio_id) ?? "—",
        })),
        ...deps.map((d) => ({
          nome: d.paciente_nome,
          tipo: "Dependente",
          vinculo: `Titular: ${tituPorContrato.get(d.contrato_id) ?? "—"}`,
        })),
      ];
      setDrill({
        ...
        columns: [
          ...
          { key: "tipo", label: "Tipo" },
          { key: "vinculo", label: "Plano / Titular" },
        ],
        rows,
      });
      ...
      setDrill({
        ...
        rows: lista.map((m) => ({
          venc: fmtDate(m.vencimento),
          titular: contratoNome.get(m.contrato_id) ?? "—",
          status: m.status,
          valor: BRL(m.valor),
        })),
      });
      ...
      mens
        .filter((m) => m.status === "pago" && m.pago_em && m.pago_em >= from && m.pago_em <= to)
        .forEach((m) =>
          totalPorContrato.set(
            m.contrato_id,
            (totalPorContrato.get(m.contrato_id) ?? 0) + Number(m.valor),
          ),
        );
      contratos.forEach((c) => {
      ...
      setDrill({
        ...
        columns: [
          { key: "nome", label: "Titular" },
          { key: "plano", label: "Plano" },
          { key: "total", label: "Total no período", align: "right" },
        ],
        rows: lista.map((c) => ({
          nome: c.paciente_nome,
          plano: planoNome.get(c.convenio_id) ?? "—",
          total: BRL(totalPorContrato.get(c.id) ?? 0),
        })),
      });
      ...
      const usoPorPac = new Map<string, number>();
      atends.forEach((a) => {
        if (a.paciente_id) usoPorPac.set(a.paciente_id, (usoPorPac.get(a.paciente_id) ?? 0) + 1);
      });
      const todas = [
        ...contratos.map((c) => ({
          nome: c.paciente_nome,
          tipo: "Titular",
          vinculo: planoNome.get(c.convenio_id) ?? "—",
          usou: (usoPorPac.get(c.paciente_id) ?? 0) > 0 ? "Sim" : "Não",
          consultas: usoPorPac.get(c.paciente_id) ?? 0,
        })),
        ...deps.map((d) => ({
          nome: d.paciente_nome,
          tipo: "Dependente",
          vinculo: `Titular: ${tituPorContrato.get(d.contrato_id) ?? "—"}`,
          usou: (usoPorPac.get(d.paciente_id) ?? 0) > 0 ? "Sim" : "Não",
          consultas: usoPorPac.get(d.paciente_id) ?? 0,
        })),
      ].sort((a, b) => b.consultas - a.consultas);
      setDrill({
      ...
      setDrill({
        ...
        rows: stats.consultasTitulares.map((t) => ({
          nome: t.nome,
          plano: t.plano,
      ...
      setDrill({
        ...
        rows: stats.consultasDependentes.map((d) => ({
          nome: d.nome,
          titular: d.titular,
          tipo: d.tipo,
    ...
    <div className="space-y-4">
      <Card>
        ...
        <CardContent className="flex flex-wrap gap-3 items-end">
          ...
          <Button variant="outline" onClick={exportarPlanos}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <KPI
          onClick={() => openDrill("titulares")}
          icon={<Users className="h-4 w-4" />}
          label="Titulares"
          value={stats.titulares}
        />
        <KPI
          onClick={() => openDrill("dependentes")}
          icon={<UserPlus className="h-4 w-4" />}
          label="Dependentes"
          value={stats.dependentesCount}
        />
        <KPI
          onClick={() => openDrill("totalPessoas")}
          icon={<Users className="h-4 w-4" />}
          label="Total pessoas"
          value={stats.totalPessoas}
        />
        <KPI
          onClick={() => openDrill("pagantes")}
          icon={<Activity className="h-4 w-4" />}
          label="Pagantes no período"
          value={stats.pagantes}
        />
        <KPI
          onClick={() => openDrill("receita")}
          icon={<TrendingUp className="h-4 w-4 text-green-600" />}
          label="Receita (mensal. + adesão)"
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <KPI
          ...
          value={`${stats.utilizacaoPct.toFixed(1)}%`}
        />
        <KPI
          onClick={() => openDrill("consTit")}
          icon={<Users className="h-4 w-4" />}
          label="Consultas / titulares"
          value={stats.usosTitulares}
        />
        <KPI
          onClick={() => openDrill("consDep")}
          icon={<UserPlus className="h-4 w-4" />}
          label="Consultas / dependentes"
          value={stats.usosDependentes}
        />
        <KPI
          onClick={() => openDrill("mediaConsultas")}
          icon={<Activity className="h-4 w-4" />}
          label="Média consultas / pessoa"
          value={stats.mediaConsultasPessoa.toFixed(2)}
        />
        <KPI
          onClick={() => openDrill("semVinculo")}
          icon={<Activity className="h-4 w-4" />}
          label="Consultas sem vínculo"
          value={stats.usosSemVinculo}
        />
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-xs text-muted-foreground">
            Todos os cartões da clínica, com totais acumulados (não dependem do período acima).
          </p>
        </CardHeader>
        <CardContent>
          <Table>
              ...
              {stats.porPlanoAll.length > 0 ? (
                <TableRow className="bg-muted/30 font-semibold">
                  ...
                  <TableCell className="text-right">
                    {stats.porPlanoAll.reduce((s, p) => s + p.titulares, 0)}
                  </TableCell>
                  <TableCell className="text-right">
                    {stats.porPlanoAll.reduce((s, p) => s + p.titularesAtivos, 0)}
                  </TableCell>
                  <TableCell className="text-right">
                    {stats.porPlanoAll.reduce((s, p) => s + p.dependentes, 0)}
                  </TableCell>
                  <TableCell className="text-right">
                    {stats.porPlanoAll.reduce((s, p) => s + p.pessoas, 0)}
                  </TableCell>
                  <TableCell className="text-right">
                    {BRL(stats.porPlanoAll.reduce((s, p) => s + p.mrr, 0))}
                  </TableCell>
                  <TableCell className="text-right">
                    {BRL(stats.porPlanoAll.reduce((s, p) => s + p.receita, 0))}
                  </TableCell>
                </TableRow>
              ) : null}
            </TableBody>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground">
              Atendimentos no período por cada titular de cartão.
            </p>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground">
              Atendimentos no período por cada dependente.
            </p>
          </CardHeader>
          <CardContent className="max-h-96 overflow-auto">
            <Table>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card>
          ...
          <CardContent className="space-y-2">
            ...
            {stats.semData > 0 ? (
              <p className="text-xs text-muted-foreground">
                {stats.semData} pessoa(s) sem data de nascimento.
              </p>
            ) : null}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Top 10 — quem mais utilizou</CardTitle>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
              ...
              <TableBody>
                ...
                {stats.topUso.map((u, i) => (
                  <TableRow key={i}>
                    ...
                    <TableCell className="text-right font-semibold">{u.qtd}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
