...
export function DependentesPage({ produto }: { produto: ProdutoCartao }) {
  ...
  const load = async () => {
    ...
    // Só os contratos deste produto (Benefícios ou Terapêutico).
    const { data: convData } = await supabase
      .from("cb_convenios")
      .select("id")
      .eq("clinica_id", cid)
      .eq("produto", produto);
    const idsDoProduto = new Set(((convData ?? []) as Array<{ id: string }>).map((c) => c.id));
    setContratos(cList.filter((c) => contratoDoProduto(produto, idsDoProduto, c.convenio_id)));
    setDeps(dList);
    setLoading(false);
  };
