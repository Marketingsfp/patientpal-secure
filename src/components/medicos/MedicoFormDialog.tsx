...
export function MedicoFormDialog({
      ...
      const mantidos = cs.filter((c) => {
        ...
        if (isServicoCadastrado && !nomesServicosSelecionados.has(key)) {
          const vazio =
            ...
            !c.convenio_valor &&
            !c.cartao_consulta_valor &&
            !c.cartao_desconto_valor;
    ...
    void (async () => {
          ...
          convs.map((c) => {
            ...
            return {
              ...
              convenio_tipo_repasse:
                (r.convenio_tipo_repasse as "percentual" | "valor") ?? "percentual",
              convenio_percentual:
                r.convenio_percentual != null ? String(r.convenio_percentual) : "",
      ...
      try {
        const { data: cardios } = await supabase
          .from("medicos")
          .select(
            "id, nome, crm, crm_uf, ativo, medico_especialidades!inner(especialidade:especialidades!inner(nome))",
          )
          .eq("clinica_id", cliId)
          .eq("ativo", true)
        ...
        const catalog: LaudadorOption[] = ((cardios as any[]) ?? []).map((c) => ({
          ...
          nome: c.nome,
          crm: c.crm ?? null,
          crm_uf: c.crm_uf ?? null,
        }));
        // dedup (join pode duplicar se médico tiver múltiplas linhas em cardio)
        const seen = new Set<string>();
        setLaudadoresCatalog(
          catalog.filter((c) => (seen.has(c.id) ? false : (seen.add(c.id), true))),
        );
      } catch {
        setLaudadoresCatalog([]);
      }
      ...
      setForm({
        ...
        percentual:
          sens.percentual_repasse_padrao != null ? String(sens.percentual_repasse_padrao) : "",
        ...
        duracao_consulta_min:
          med.duracao_consulta_min != null ? String(med.duracao_consulta_min) : "15",
        usa_sistema: (med as { usa_sistema?: boolean }).usa_sistema !== false,
        cpf: sens.cpf ?? "",
        rg: sens.rg ?? "",
        data_nascimento: sens.data_nascimento ?? "",
        email: med.email ?? "",
        telefone: med.telefone ?? "",
        telefone2: med.telefone2 ?? "",
        nacionalidade: med.nacionalidade ?? "Brasileira",
        estado_civil: med.estado_civil ?? "",
        sexo: med.sexo ?? "nao_informar",
        cep: med.cep ?? "",
        logradouro: med.logradouro ?? "",
        numero: med.numero ?? "",
        complemento: med.complemento ?? "",
        bairro: med.bairro ?? "",
        cidade: med.cidade ?? "",
        estado: med.estado ?? "",
        banco: sens.banco ?? "",
        agencia: sens.agencia ?? "",
        conta: sens.conta ?? "",
        pix_chave: sens.pix_chave ?? "",
        criarUsuario: false,
        senhaUsuario: "",
        roleUsuario: "medico",
      ...
      if (med.user_id) {
        if (podeGerenciarEquipe) {
          try {
            const res = await getLoginFn({
              data: { clinicaId: med.clinica_id ?? clinicaId, userId: med.user_id },
            });
        ...
        setPacienteVinculado(
          pac
            ? {
                ...
                data_nascimento:
                  (pac as { data_nascimento: string | null }).data_nascimento ?? null,
  ...
  async function salvarNovaSenha() {
    ...
    setSavingSenha(true);
    try {
      await definirSenhaFn({
        data: { clinicaId: activeClinicaId, userId: medicoUserId, novaSenha },
      });
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (editId) {
      ...
      const { error: delEsp } = await supabase
        .from("medico_especialidades")
        .delete()
        .eq("medico_id", editId);
      if (delEsp) {
        setSaving(false);
        mostrarErro(delEsp, "erro ao limpar especialidades");
        return;
      }
      const { error: delProc } = await supabase
        .from("medico_procedimentos")
        .delete()
        .eq("medico_id", editId);
      if (delProc) {
        setSaving(false);
        mostrarErro(delProc, "erro ao limpar procedimentos");
        return;
      }
    } else {
    ...
    if (medicoId) {
      ...
      const laudoRows = laudadores
        .filter(
          (l) =>
            l.laudador_medico_id &&
            ((l.tipo_repasse === "percentual" &&
              l.percentual.trim() !== "" &&
              Number(l.percentual) > 0) ||
  ...
  const inner = loading ? (
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <Tabs defaultValue="dados">
        <TabsList
          className={
            asPage ? "grid grid-cols-7 w-full" : "grid grid-cols-7 w-full sticky top-[3.25rem] z-10"
          }
        >
          <TabsTrigger value="dados">Dados</TabsTrigger>
          <TabsTrigger value="especialidades">Especialidades</TabsTrigger>
          <TabsTrigger value="agendas" disabled={!editingMedicoId}>
            Agendas
          </TabsTrigger>
          <TabsTrigger value="banco">Banco</TabsTrigger>
          <TabsTrigger value="repasse">Repasse</TabsTrigger>
          <TabsTrigger value="convenio" disabled={!editingMedicoId}>
            Convênio
          </TabsTrigger>
            ...
            <div className="border-t pt-4 space-y-2">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Vincule o médico ao próprio cadastro de paciente da clínica. Necessário para
                  habilitar o <b>Convênio Funcionário</b> na aba <b>Convênio</b>.
                </p>
              </div>
              <PatientSearchInput
              ...
              {pacienteVinculado && (
                <div className="text-xs text-muted-foreground">
                  ...
                  <button
                    type="button"
                    className="underline"
                    onClick={() => setPacienteVinculado(null)}
                  >
                    remover vínculo
                  </button>
                </div>
          ...
          <div className="pt-2">
            <div className="rounded-md border p-3 flex items-start gap-3 bg-muted/30">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">
                  Quando <b>desmarcado</b>, o médico faz prontuário em papel. Na agenda, em vez de
                  abrir o prontuário, aparece o botão <b>"Concluir atendimento"</b> (1 clique) para
                  finalizar a consulta e liberar o repasse normalmente.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="border rounded-md p-3 space-y-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Adicione uma ou mais especialidades. Marque "Tem RQE" e informe o número quando
                  aplicável.
                </p>
              </div>
              <Button
                        ...
                        onChange={(v) => {
                          if (
                            ...
                            form.especialidades.some(
                              (x, i) => i !== idx && x.especialidade_id === v,
                            )
                          ) {
          ...
          <div className="border rounded-md p-3 space-y-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Adicione os serviços que o médico realiza. A lista mostra apenas serviços das
                        ...
                        return (
                          <DropdownMenuItem
                            ...
                            onSelect={() => {
                              ...
                              if (novos.length === 0) {
                                toast.info(
                                  "Todos os serviços dessa especialidade já estão adicionados.",
                                );
                                return;
                              }
                              setForm({
            ...
            {especialidadesSelecionadasNomes.size === 0 ? (
              <p className="text-xs text-muted-foreground">
                Selecione ao menos uma especialidade na seção acima para ver os serviços
                disponíveis.
              </p>
            ) : procsFiltradosPorEspecialidade.length === 0 ? (
              <p className="text-xs text-muted-foreground">
                Nenhum serviço cadastrado para as especialidades selecionadas.
              </p>
            ) : procs.length === 0 ? (
                        ...
                        options={(() => {
                          ...
                          const addProcOpts = (p: Procedimento) => {
                            ...
                            if (choices.length === 0) {
                              ...
                              if (!pushed.has(v) && !selecionados.has(v)) {
                                pushed.add(v);
                                opts.push({ value: v, label: p.nome });
                              }
                            } else {
                              for (const c of choices) {
                                ...
                                opts.push({
                                  value: v,
                                  label: `${p.nome} (${c.nome.toUpperCase()})`,
                                });
                              }
                            }
                          };
            ...
            {form.procedimentos.length > 0 && (
              <div className="border-t pt-3 mt-2 space-y-1">
                <Label className="text-sm font-medium">
                  Procedimento padrão (pré-selecionado na agenda)
                </Label>
                <p className="text-xs text-muted-foreground">
                  Ao agendar com este médico, o sistema já preenche este serviço — agiliza o
                  atendimento. Ex.: Ortopedista → CONSULTA. Médico que só faz USG → o exame mais
                  comum.
                </p>
                <SearchableSelect
                  value={
                  ...
                  options={[
                    ...
                    ...Array.from(
                      new Set(form.procedimentos.map((it) => splitItem(it).pid).filter(Boolean)),
                    )
          ...
          {editingMedicoId && (
            <MedicoAgendasTab
              ...
              procedimentoIds={Array.from(
                new Set(
                  form.procedimentos.map((it) => splitItem(it).pid).filter((x): x is string => !!x),
                ),
              )}
            />
          )}
        ...
        <TabsContent value="repasse" className="space-y-4 pt-4 pb-16">
            ...
            {form.aceita_cartao_beneficios && (
              <div className="grid grid-cols-[1fr_1fr] gap-2">
                <select
                  ...
                  onChange={(e) =>
                    setForm({ ...form, cb_tipo_repasse: e.target.value as "percentual" | "valor" })
                  }
                >
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-[1fr_1fr] gap-2">
              <select
                ...
                onChange={(e) =>
                  setForm({ ...form, tipo_repasse: e.target.value as "percentual" | "valor" })
                }
              >
                <option value="percentual">% Percentual</option>
                <option value="valor">R$ Valor</option>
          ...
          <div className="pt-4 border-t space-y-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Escolha o <b>serviço</b> e defina o repasse em cada forma de atendimento:{" "}
                  <b>Particular</b>, <b>Convênio</b>, <b>Cartão Consulta</b> e{" "}
                  <b>Cartão Desconto</b>. As <b>categorias</b> dos serviços selecionados na aba{" "}
                  <b>Especialidades</b> aparecem aqui automaticamente (Consulta, Exame,
                  Procedimento) e valem para todos os serviços daquela categoria; a linha do{" "}
                  <b>serviço específico</b> prevalece sobre a categoria. Campo em branco = usa o{" "}
            ...
            {convenios.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-4">
                Nenhum repasse individual. Clique em "Manual" para sobrescrever o repasse de um
                serviço específico.
              </p>
            ) : (
          ...
          {(() => {
            const _n = form.nome.trim().toUpperCase();
            return _n === "ELETROCARDIOGRAMA" || _n === "ITB";
          })() && (
            <div className="space-y-3 pt-4 border-t mt-4">
              <div>
                <Label>REPASSE LAUDO TERCEIRO</Label>
                <p className="text-xs text-muted-foreground">
                  Use quando este cadastro representa uma <b>agenda de exame</b> (ex.:
                  ELETROCARDIOGRAMA, ITB) e o laudo é feito por <b>outro médico</b>. Liste abaixo os
                  cardiologistas ativos da clínica e defina o repasse (percentual ou valor fixo por
                  exame) que cada um recebe pelo laudo. O financeiro vincula este repasse em{" "}
                  <b>Financeiro → Atendimentos</b>, filtrando pelo nome do médico da agenda.
                </p>
              </div>
              {!editingMedicoId ? (
                <p className="text-sm text-muted-foreground text-center py-6">
                  Salve o médico antes de configurar o repasse de laudo.
                </p>
              ) : laudadoresCatalog.length === 0 ? (
                <div className="rounded-md border border-dashed p-4 text-sm text-muted-foreground text-center">
                  Nenhum cardiologista ativo encontrado nesta clínica. Cadastre médicos com a
                  especialidade <b>Cardiologia</b> para poder configurar o laudo.
                </div>
              ) : (
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {laudadoresCatalog.map((cardio) => {
                        ...
                        const value =
                          tipo === "percentual" ? (row?.percentual ?? "") : (row?.valor ?? "");
                        return (
                          <tr key={cardio.id} className="border-t align-middle">
                            ...
                            <td className="px-2 py-1">
                              <select
                                ...
                                value={tipo}
                                onChange={(e) =>
                                  setRow({ tipo_repasse: e.target.value as "percentual" | "valor" })
                                }
                              >
                                <option value="percentual">% Percentual</option>
                                <option value="valor">R$ Valor</option>
                            ...
                            <td className="px-2 py-1 text-right">
                              <Button
                                ...
                                onClick={() => {
                                  setLaudadores((rows) =>
                                    rows.filter((l) => l.laudador_medico_id !== cardio.id),
                                  );
                                  setLaudadoresCatalog((cat) =>
                                    cat.filter((c) => c.id !== cardio.id),
                                  );
                                }}
                              >
                                <Trash2 className="h-4 w-4" />
              ...
              <p className="text-xs text-muted-foreground">
                Deixe o valor em branco / 0 para o médico que <b>não recebe laudo</b>. Este cadastro
                não gera lançamento automático — o financeiro decide quando lançar.
              </p>
            </div>
          )}
        </TabsContent>
          ...
          {hasLogin ? (
            <div className="space-y-4 py-2 text-sm">
              {existingEmail ? (
                ...
                <p className="text-muted-foreground">
                  Não foi possível recuperar o e-mail de login deste médico.
                </p>
              )}
            ...
            <>
              <div className="rounded-md border p-3 space-y-3">
                ...
                <p className="text-xs text-muted-foreground">
                  Cria um usuário com o e-mail informado na seção <b>Contato</b> (aba Dados) e
                  vincula este médico à equipe da clínica. Se já existir usuário com este e-mail,
                  ele será apenas adicionado à equipe.
                </p>
                {form.criarUsuario && (
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-2 col-span-2">
                      ...
                      {!form.email && (
                        <p className="text-xs text-amber-600">
                          Informe um e-mail (também na seção Contato da aba Dados).
                        </p>
                      )}
                    </div>
                    <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(v) =>
                          setForm({ ...form, roleUsuario: v as typeof form.roleUsuario })
                        }
                      >
                        <SelectTrigger>
                          <SelectValue />
              ...
              {!editId && (
                <p className="text-xs text-muted-foreground">
                  Ao salvar, também será criado automaticamente um cadastro de <b>paciente</b> com o
                  mesmo nome, CPF, e-mail e telefone — caso ainda não exista nesta clínica.
                </p>
              )}
            </>
          )}
      ...
      <Dialog open={bulkOpen} onOpenChange={setBulkOpen}>
        <DialogContent className="sm:max-w-2xl w-[calc(100vw-2rem)] max-h-[85vh] overflow-hidden flex flex-col">
          ...
          {(() => {
            ...
            const allChecked =
              filtered.length > 0 && filtered.every((o) => bulkSelected.has(o.value));
            return (
