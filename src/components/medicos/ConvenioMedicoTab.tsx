...
/**
 * Aba "Convênio" no cadastro do médico. Igual à aba de RH: ao habilitar,
 * cria um contrato-sombra em `contratos_assinatura` vinculado ao "Convênio
 * Funcionário" da clínica — sem taxa e sem mensalidade —, para que médico e
 * dependentes sejam reconhecidos como associados na agenda.
 */
export function ConvenioMedicoTab({ medicoId, clinicaId, pacienteId, pacienteNome, podeEscrever }: Props) {
  ...
  const carregar = useCallback(async () => {
    ...
    const { data: m } = await supabase
      ...
      .select("convenio_contrato_id" as any)
      .eq("id", medicoId)
      .maybeSingle();
    const contratoId = (m as { convenio_contrato_id: string | null } | null)?.convenio_contrato_id ?? null;
    if (!contratoId) {
      setContrato(null);
      setDependentes([]);
      setLoading(false);
  ...
  async function habilitar() {
    if (!pacienteId) { toast.error("Vincule o médico a um cliente na aba \"Dados\" antes de habilitar o convênio."); return; }
    setBusy(true);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  async function adicionarDependente() {
    ...
    setBusy(true);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error } = await (supabase.rpc as any)("medico_convenio_add_dependente", {
      _medico_id: medicoId,
    ...
    <div className="space-y-4">
      <Card className="p-4 space-y-3">
        ...
        <p className="text-xs text-muted-foreground">
          Ao habilitar, o médico e seus dependentes passam a usar o convênio de funcionários da clínica na agenda —
        ...
        {!habilitado ? (
          <div className="space-y-3">
            {pacienteId ? (
              ...
              <div className="text-sm rounded-md border border-dashed p-3 flex items-start gap-2 text-muted-foreground">
                ...
                <span>
                  Vincule o médico a um cliente na aba <b>Dados</b> para habilitar o convênio.
                </span>
              </div>
            )}
            {podeEscrever && (
              <Button type="button" onClick={habilitar} disabled={busy || !pacienteId}>
          ...
          <div className="space-y-2 text-sm">
            ...
            {podeEscrever && (
              <Button type="button" variant="outline" size="sm" onClick={desabilitar} disabled={busy}>
                Desligar convênio
              </Button>
            )}
          </div>
          ...
          <div className="border rounded-md divide-y">
            {dependentes.map((d) => (
              <div key={d.id} className="flex items-center justify-between p-2 text-sm">
                ...
                {podeEscrever && (
                  <Button type="button" variant="ghost" size="sm" onClick={() => removerDependente(d.id)} disabled={busy}>
        ...
        {podeEscrever && habilitado && (
          <div className="border-t pt-3 space-y-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-[1fr,200px,auto] gap-2 items-end">
              ...
              <Button type="button" onClick={adicionarDependente} disabled={busy || !novoDep || !novoParentesco.trim()}>
