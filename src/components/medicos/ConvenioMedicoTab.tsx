 ...
 */
export function ConvenioMedicoTab({
  medicoId,
  clinicaId,
  pacienteId,
  pacienteNome,
  ...
  const carregar = useCallback(async () => {
    ...
    const { data: m } = await supabase
      ...
      .maybeSingle();
    const contratoId =
      (m as { convenio_contrato_id: string | null } | null)?.convenio_contrato_id ?? null;
    if (!contratoId) {
      setContrato(null);
      setDependentes([]);
      setLoading(false);
  ...
  async function habilitar() {
    if (!pacienteId) {
      toast.error('Vincule o médico a um cliente na aba "Dados" antes de habilitar o convênio.');
      return;
    }
    setBusy(true);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  async function desabilitar() {
    if (
      !(await confirmDialog(
        "Deseja realmente desligar o Convênio Funcionário deste médico? Os dependentes também serão desativados.",
      ))
    )
      return;
    setBusy(true);
  ...
  async function removerDependente(depId: string) {
    ...
    setBusy(true);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error } = await (supabase.rpc as any)("medico_convenio_remove_dependente", {
    ...
    <div className="space-y-4">
      <Card className="p-4 space-y-3">
        ...
        <p className="text-xs text-muted-foreground">
          Ao habilitar, o médico e seus dependentes passam a usar o convênio de funcionários da
