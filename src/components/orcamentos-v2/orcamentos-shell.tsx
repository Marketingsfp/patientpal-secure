...
type TabV = "todos" | "abertos" | "convertidos" | "pendencia";
...
export function OrcamentosShellV2({ compactPref, onToggleCompact }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from("orcamentos")
      .select("id, numero, paciente_nome, paciente_telefone, medico_nome, forma_pagamento, valor_total, status, created_at, categoria")
  ...
  const matchTab = (o: OrcV2, t: TabV): boolean => {
    ...
    if (t === "abertos") return !convertido;
    if (t === "convertidos") return convertido;
    if (t === "pendencia") return convertido && (o.itens_consumidos ?? 0) < (o.itens_total ?? 0);
    return true;
  };
  ...
  const counts = useMemo(() => {
    ...
    return {
      todos: base.length,
      abertos: base.filter((o) => matchTab(o, "abertos")).length,
      convertidos: base.filter((o) => matchTab(o, "convertidos")).length,
      pendencia: base.filter((o) => matchTab(o, "pendencia")).length,
    };
  }, [list, applyBase]);
  ...
  const tabs: ReadonlyArray<StatusTab<TabV>> = [
    ...
    { value: "pendencia", label: "Com pendência", count: counts.pendencia },
  ];
  ...
  return (
    <div className="h-full p-3 sm:p-4">
      <ListShell<TabV>
        ...
        searchValue={q}
        onSearchChange={setQ}
        searchPlaceholder="Buscar por paciente, número ou médico…"
        tabs={tabs}
        tabValue={tab}
        onTabChange={setTab}
        chips={
