...
export function PacienteCartoesBeneficios({
  ...
  const titularesHistorico = useMemo(
    () => titulares.filter((c) => c.status !== "ativo"),
    [titulares],
  );
  const dependeDeAtivos = useMemo(() => dependeDe.filter((c) => c.status === "ativo"), [dependeDe]);
  const dependeDeHistorico = useMemo(
    () => dependeDe.filter((c) => c.status !== "ativo"),
    [dependeDe],
  );
        ...
        <div className="flex items-center gap-2 font-semibold">
          ...
          {titularesAtivos.length + dependeDeAtivos.length > 0 && (
            <Badge variant="outline">
              {titularesAtivos.length + dependeDeAtivos.length} contrato(s) ativo(s)
            </Badge>
          )}
        </div>
      </div>
                ...
                {histOpen && (
                  <div className="mt-2 space-y-2 opacity-70">
                    {titularesHistorico.map((c) => (
                      <div
                        key={`tit-hist-${c.id}`}
                        className="rounded-md border border-dashed p-3 space-y-1"
                      >
                        <div className="flex items-center justify-between gap-2 flex-wrap">
                          <div className="flex items-center gap-2">
