...
export function ClienteForm({
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      sexo: ["masculino", "feminino", "outro", "nao_informar"].includes(
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const { data: tit } = await supabase
        ...
        .select(
          "id, numero, status, paciente_id, paciente_nome, data_inicio, data_fim, dia_vencimento, valor_mensal, num_parcelas, forma_pagamento, convenio_id",
        )
        .eq("paciente_id", editing.id);
  ...
  async function revogarBiometria() {
    ...
    if (
      !(await confirmDialog(
        `Remover a biometria facial de ${editing.nome}? (direito de exclusão — LGPD)`,
      ))
    )
      return;
    setBioLoading(true);
  ...
  const fieldProps = (field: keyof FormState) => {
    ...
    return {
      ...
      ...(ehCPF
        ...
        : {}),
      ...(ehTelefone
        ? { inputMode: "numeric" as const, maxLength: 15, placeholder: "(00) 00000-0000" }
        : {}),
      onChange: (v: string) => {
        if (somenteLetras) {
  ...
  return (
    <>
            ...
            <fieldset disabled={readOnly} className="contents">
              <TabsContent value="dados" className="space-y-4 pt-4 pb-16">
                ...
                <div className="space-y-1">
                  <Label>
                    E-mail{" "}
                    <span className="text-xs text-muted-foreground">(usado em nota fiscal)</span>
                  </Label>
                      ...
                      <div className="space-y-1">
                        {hasBiometria ? (
                          <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-500/40 bg-emerald-500/10 px-2.5 py-0.5 text-xs font-medium text-emerald-700 dark:text-emerald-400">
                            <span className="h-2 w-2 rounded-full bg-emerald-500" /> Biometria
                            facial cadastrada
                          </span>
                        ) : (
                      ...
                      <div className="flex items-center gap-3">
                        <div className="h-24 w-24 overflow-hidden rounded-lg border border-border bg-muted">
                          {fotoPreview ? (
                            <img
                              src={fotoPreview}
                              alt="Foto biométrica do paciente"
                              className="h-full w-full object-cover"
                            />
                          ) : (
                  ...
                  <div className="space-y-4">
                    {convList.map((c) => {
                      ...
                      const dataFimCalc = c.data_fim
                        ...
                        : (() => {
                            const meses =
                              c.vigencia_meses || c.parcelas.length || c.num_parcelas || 12;
      ...
      <FaceCaptureDialog
        ...
        onCaptured={async (d, foto) => {
          await salvarBiometria(d, foto);
          setFaceOpen(false);
        }}
