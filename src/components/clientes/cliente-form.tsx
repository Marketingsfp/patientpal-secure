...
export function ClienteForm({
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      telefone2: editing.telefone2 ?? "",
      email: editing.email ?? "",
      data_nascimento: editing.data_nascimento ?? "",
      sexo: editing.sexo ?? "nao_informar",
      ativo: editing.ativo,
      cep: editing.cep ?? "",
  ...
  useEffect(() => {
    ...
    void supabase
      .from("prontuarios")
      .select(
        "id, data, medico_id, queixa_principal, hipotese_diagnostica, conduta, prescricao, historia_doenca, exame_fisico, observacoes",
      )
      .eq("paciente_id", editing.id)
      ...
      .then(async ({ data, error }) => {
        ...
        if (medicoIds.length > 0) {
          ...
          medEspMap = Object.fromEntries(
            (meds ?? []).map((m: any) => [m.id, m.especialidade_id ?? null]),
          );
          const espIds = Array.from(
            new Set((meds ?? []).map((m: any) => m.especialidade_id).filter(Boolean)),
          );
          if (espIds.length > 0) {
        ...
        const findProc = (dataIso: string, medicoId: string | null): string | null => {
          ...
          sameDay.sort(
            (a, b) =>
              Math.abs(new Date(a.inicio).getTime() - d.getTime()) -
              Math.abs(new Date(b.inicio).getTime() - d.getTime()),
          );
        ...
        setProntList(
          rows.map((r: any) => ({
            ...
            especialidade:
              r.medico_id && medEspMap[r.medico_id]
                ? (espNomeMap[medEspMap[r.medico_id] as string] ?? null)
                : null,
    ...
    void (async () => {
      ...
      if (medicoIds.length > 0) {
        ...
        medMap = Object.fromEntries(
          (meds ?? []).map((m: any) => [
            m.id,
            { nome: m.nome, especialidade_id: m.especialidade_id },
          ]),
        );
        const espIds = Array.from(
          new Set((meds ?? []).map((m: any) => m.especialidade_id).filter(Boolean)),
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const { data: tit } = await supabase
        ...
        .select(
          "id, numero, status, paciente_id, paciente_nome, data_inicio, data_fim, dia_vencimento, valor_mensal, num_parcelas, forma_pagamento, plano_id",
        )
        .eq("paciente_id", editing.id);
      ...
      const planoMap: Record<string, { nome: string; vigencia_meses: number | null }> =
        Object.fromEntries(
          ((planosRes.data ?? []) as any[]).map((p) => [
            p.id,
            { nome: p.nome, vigencia_meses: p.vigencia_meses },
          ]),
        );
      ...
      const lista: ConvContrato[] = todos.map((c) => {
        ...
        return {
          ...
          vigencia_meses: plano?.vigencia_meses ?? null,
          papel: c.papel,
    ...
    void supabase
      ...
      .then(({ data }) => {
        const nomes = Array.from(
          new Set((data ?? []).map((p: any) => (p.nome ?? "").trim()).filter(Boolean)),
        );
  ...
  async function revogarBiometria() {
    if (!editing) return;
    if (!confirm(`Remover a biometria facial de ${editing.nome}? (direito de exclusão — LGPD)`))
      return;
    setBioLoading(true);
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    if (!/\p{L}/u.test(nomeTrim)) {
      toast.error("Nome deve conter letras.");
      return;
    }
    ...
    if (anosDiff > 120) {
      toast.error("Data de nascimento inválida (idade acima de 120 anos).");
      return;
    }
    if (dn.getFullYear() < 1900) {
      toast.error("Data de nascimento inválida (ano anterior a 1900).");
      return;
    }
    ...
    if (fotoFile && pacienteId) {
      const ext =
        (fotoFile.name.split(".").pop() || "jpg").toLowerCase().replace(/[^a-z0-9]/g, "") || "jpg";
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        {!editing && baseImportada === false && (
          <div className="rounded-md border border-amber-300 bg-amber-50 dark:bg-amber-950/30 dark:border-amber-800 px-3 py-2 text-sm text-amber-900 dark:text-amber-200">
            ...
            Antes de cadastrar, verifique se o paciente já não existe (CPF, telefone ou nome) ou
            encaminhe para uma atendente. Você pode continuar o cadastro manual mesmo assim.
          </div>
        )}
        <Tabs value={tab} onValueChange={setTab}>
          ...
          <TabsContent value="dados" className="space-y-4 pt-4 pb-16">
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                <p className="text-xs text-muted-foreground">
                  JPG, PNG ou WebP até 5 MB. Acesso restrito à clínica.
                </p>
              </div>
            </div>
            <div className="space-y-1">
              <Label>Nome *</Label>
              <InputVoz {...fieldProps("nome")} required maxLength={120} />
            </div>
            <div className="space-y-1">
            ...
            <div className="space-y-1">
              <Label>
                Telefone 2 <span className="text-xs text-muted-foreground">(opcional)</span>
              </Label>
              <InputVoz {...fieldProps("telefone2")} />
            </div>
            <div className="space-y-1">
              <Label>
                E-mail <span className="text-xs text-muted-foreground">(usado em nota fiscal)</span>
              </Label>
            ...
            {!editing ? (
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <ScanFace
                    className={`h-6 w-6 ${hasBiometria ? "text-emerald-600" : "text-muted-foreground"}`}
                  />
                  <div>
              ...
              <>
                <div className="border rounded-lg p-3 bg-muted/30 grid gap-3 md:grid-cols-[1fr_1fr_1fr_1fr_auto] items-end">
                  <div className="space-y-1">
                    ...
                    <Input
                      type="date"
                      value={histFiltroDataDe}
                      onChange={(e) => setHistFiltroDataDe(e.target.value)}
                    />
                  </div>
                  <div className="space-y-1">
                    <Label className="text-xs">Data até</Label>
                    <Input
                      type="date"
                      value={histFiltroDataAte}
                      onChange={(e) => setHistFiltroDataAte(e.target.value)}
                    />
                  </div>
                  <div className="space-y-1">
                    <Label className="text-xs">Médico</Label>
                        ...
                        {histFiltered.map((h) => (
                          <tr key={h.id} className="border-t hover:bg-muted/30">
                            <td className="px-3 py-2 tabular-nums">
                              {new Date(h.inicio).toLocaleString("pt-BR", {
                                dateStyle: "short",
                                timeStyle: "short",
                              })}
                            </td>
              ...
              <div className="space-y-4">
                {convList.map((c) => {
                  ...
                  const isAtraso = (p: ConvParcela) =>
                    ["pendente", "aberto", "atrasado"].includes(p.status) &&
                    new Date(p.vencimento + "T00:00:00") < hoje;
                  ...
                  const destacar = (nome: string, pid: string) =>
                    pid === editing.id ? (
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Enquadre o rosto do paciente e clique em Capturar.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={consentOpen} onOpenChange={setConsentOpen}>
        <DialogContent className="max-w-lg">
          ...
          <div className="text-sm space-y-2 max-h-72 overflow-auto rounded-md border bg-muted/30 p-3">
            <p>
              <strong>Paciente:</strong> {editing?.nome}
            </p>
            <p>
              <strong>Finalidade:</strong> identificação na recepção, totem de auto-atendimento e
              confirmação de identidade em atendimentos, evitando troca de prontuários.
            </p>
            <p>
              <strong>O que é armazenado:</strong> apenas um vetor matemático (descritor) do seu
              rosto — <em>não</em> guardamos a foto. O vetor não permite reconstruir a imagem
              original.
            </p>
            <p>
              <strong>Compartilhamento:</strong> os dados ficam restritos à clínica e não são
              compartilhados com terceiros.
            </p>
            <p>
              <strong>Direitos do titular:</strong> você pode revogar o consentimento e solicitar a
              exclusão da biometria a qualquer momento, pela equipe da recepção.
            </p>
            <p>
              <strong>Base legal:</strong> consentimento específico e destacado (art. 11, I).
            </p>
          </div>
