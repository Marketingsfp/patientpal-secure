...
const navRows: ReadonlyArray<{ label: string; items: ReadonlyArray<NavItem> }> = [
  ...
  {
    label: "Marketing",
    items: [{ to: "/app/mkt-leads", label: "Marketing", icon: Megaphone }],
  },
  {
    label: "Cadastros",
    items: [
      { to: "/app/equipe", label: "Equipe", icon: Users },
      {
        ...
        label: "Serviços",
        icon: Stethoscope,
        aliases: ["/app/tipos-servico", "/app/procedimentos", "/app/enfermagem-recursos"],
      },
  ...
  {
    label: "RH",
    items: [{ to: "/app/hr-ponto", label: "RH", icon: GraduationCap }],
  },
  {
    label: "Gestão",
    items: [
...
export function AppShell() {
  ...
  const flatNavLeaves = useMemo(() => {
    ...
    for (const row of visibleNavRows) {
      ...
      const open =
        collapsed || hideLabel || row.label === "Operação"
          ? true
          : (openGroups[row.label] ?? false);
      if (!open) continue;
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      const isUsingSidebar = !!(
        navRoot &&
        ((tgt && navRoot.contains(tgt)) ||
          (activeElement instanceof HTMLElement && navRoot.contains(activeElement)))
      );
      if (!isUsingSidebar) return;
      if (flatNavLeaves.length === 0) return;
      const path = location.pathname;
      const idx = flatNavLeaves.reduce((best, to, i) => {
      ...
      if (idx < 0) return;
      const next =
        e.key === "ArrowDown" ? Math.min(flatNavLeaves.length - 1, idx + 1) : Math.max(0, idx - 1);
      if (next === idx) return;
      const now = Date.now();
      ...
      window.setTimeout(() => {
        const nextLink = Array.from(
          navRoot?.querySelectorAll<HTMLElement>("[data-nav-to]") ?? [],
                  ...
                  {!collapsed && !hideLabel && (
                    <button
                      ...
                      onClick={() =>
                        setOpenGroups((prev) => ({
                          ...prev,
                          [row.label]: !(prev[row.label] ?? false),
                        }))
                      }
