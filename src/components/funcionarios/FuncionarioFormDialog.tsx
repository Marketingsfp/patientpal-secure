...
export function FuncionarioFormDialog({
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const contrato =
        (contratos ?? []).find((c) => c.status === "ativo") ?? (contratos ?? [])[0] ?? null;
      const nome =
  ...
  async function salvar() {
    ...
    toast.success(
      editingContratoId || prefillUserId ? "Funcionário atualizado" : "Funcionário cadastrado",
    );
    onOpenChange(false);
    onSaved?.();
  }
              ...
              <div className="space-y-4 py-2 text-sm">
                {existingEmail ? (
                  ...
                  <p className="text-muted-foreground">
                    Não foi possível recuperar o e-mail de login deste funcionário.
                  </p>
                )}
                {membershipId && (
