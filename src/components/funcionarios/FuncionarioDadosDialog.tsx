...
export function FuncionarioDadosDialog({
  ...
  async function salvar() {
    ...
    if (!form.regime) {
      toast.error("Selecione o regime");
      return;
    }
    if (form.salario === "" || isNaN(Number(form.salario))) {
      toast.error("Informe o salário");
      return;
    }
      ...
      <DialogContent className="max-w-2xl rounded-2xl backdrop-blur-xl bg-background/95 border border-border/60">
        ...
        <DialogFooter>
          ...
          <Button onClick={salvar} disabled={saving || loading}>
            {saving ? "Salvando…" : editingContratoId ? "Salvar alterações" : "Salvar"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
