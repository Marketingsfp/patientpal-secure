    ...
    <div className="sticky top-0 z-10 bg-background/95 backdrop-blur-sm border-b pb-2 pt-1">
      <div className="flex gap-2 overflow-x-auto scrollbar-thin -mx-1 px-1">
        ...
        >
          ...
          <div className="text-base font-semibold tabular-nums" data-testid="kpi-hoje">
            {brl(data.recebidoHoje)}
          </div>
        </div>
        <Card
          label="Estornos no filtro"
          value={`${brl(data.estornos)}${data.estornosQtd ? ` (${data.estornosQtd})` : ""}`}
          tone="danger"
          testId="kpi-estornos"
        />
