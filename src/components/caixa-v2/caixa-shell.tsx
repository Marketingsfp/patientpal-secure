...
export function CaixaShellV2({
  ...
  const loadMovs = useCallback(async () => {
    ...
    if (error) {
      notify.error("Erro ao carregar movimentos");
      setMovs([]);
      setHasMore(false);
    } else {
  ...
  const resumoData = useMemo<ResumoData>(() => {
    ...
    const ehMensalidade = (m: AggRow) => {
      ...
      return (
        d.includes("MENSALIDADE") || d.includes("TAXA DE ADESAO") || d.includes("TAXA DE ADESÃO")
      );
    };
    ...
    const associado = soma(recebimentos.filter(ehAssoc)) - soma(estornos.filter(ehAssoc));
    return {
