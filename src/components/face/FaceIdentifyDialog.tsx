...
export interface FaceCandidate { id: string; nome: string; descriptor: number[]; extra?: any; }

interface Props { open: boolean; onClose: () => void; candidates: FaceCandidate[]; onMatched: (m: FaceCandidate) => void; titulo?: string; }

export function FaceIdentifyDialog({ open, onClose, candidates, onMatched, titulo = "Identificar por rosto" }: Props) {
  ...
  async function tentar() {
    ...
    setBusy(true); setMsg("Detectando rosto…");
    ...
    if (!desc) { setMsg("Rosto não detectado. Tente novamente."); setBusy(false); return; }
    ...
    if (melhor && melhor.dist <= FACE_MATCH_THRESHOLD) {
      setMsg(`Olá, ${melhor.c.nome}!`); stop();
      setTimeout(() => { onMatched(melhor!.c); onClose(); }, 400);
    } else { setMsg("Não foi possível reconhecer. Tente novamente."); setBusy(false); }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) { stop(); onClose(); } }}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><ScanFace className="h-5 w-5" /> {titulo}</DialogTitle>
        ...
        <DialogFooter>
          ...
          <Button onClick={tentar} disabled={busy}>{busy ? <Loader2 className="h-4 w-4 mr-1 animate-spin" /> : null} Tentar novamente</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
