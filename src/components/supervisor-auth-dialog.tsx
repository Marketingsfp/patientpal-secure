 ...
 */
export function SupervisorAuthDialog({ open, onOpenChange, acao = "esta ação", rolesPermitidos, onAuthorized }: Props) {
  ...
  useEffect(() => {
    if (!open) { setEmail(""); setSenha(""); setLoading(false); }
  }, [open]);
  ...
  async function validar() {
    ...
    if (!atual?.refresh_token) { setLoading(false); return toast.error("Sessão atual inválida."); }
    const { data: sup, error } = await supabase.auth.signInWithPassword({ email: email.trim(), password: senha });
    if (error || !sup?.user) {
      setLoading(false);
      // Restaura sessão original caso tenha sido apagada parcialmente.
      await supabase.auth.setSession({ access_token: atual.access_token, refresh_token: atual.refresh_token });
    ...
    if (ok) {
      const { data: prof } = await supabase
        .from("profiles").select("nome").eq("id", sup.user.id).maybeSingle();
      if (prof?.nome) nome = prof.nome;
    }
    // SEMPRE restaura a sessão original.
    await supabase.auth.setSession({ access_token: atual.access_token, refresh_token: atual.refresh_token });
    setLoading(false);
    if (!ok) return toast.error("Este usuário não tem permissão para autorizar (precisa ser admin, gestor ou financeiro).");
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-primary" /> Autorização da supervisão</DialogTitle>
          <DialogDescription>Para {acao}, peça ao supervisor (admin, gestor ou financeiro) que informe e-mail e senha.</DialogDescription>
        </DialogHeader>
        <div className="space-y-3">
          <div className="space-y-1">
            <Label htmlFor="sup-email">E-mail do supervisor</Label>
            <Input id="sup-email" type="email" autoComplete="off" value={email} onChange={(e) => setEmail(e.target.value)} />
          </div>
          <div className="space-y-1">
            <Label htmlFor="sup-senha">Senha</Label>
            <Input id="sup-senha" type="password" autoComplete="off" value={senha} onChange={(e) => setSenha(e.target.value)}
              onKeyDown={(e) => { if (e.key === "Enter") void validar(); }} />
          </div>
        </div>
        <DialogFooter>
          <Button variant="ghost" onClick={() => onOpenChange(false)} disabled={loading}>Cancelar</Button>
          <Button onClick={() => void validar()} disabled={loading}>{loading ? "Validando…" : "Autorizar"}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
