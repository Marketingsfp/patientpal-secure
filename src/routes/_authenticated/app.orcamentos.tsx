...
function OrcamentosPage() {
  ...
  const [filtroRealizacao, setFiltroRealizacao] = useState<
    "todos" | "realizados" | "nao_realizados"
  >("todos");
  ...
  const load = async () => {
    ...
    const { data, error } = await supabase
      .from("orcamentos")
      .select(
        "id, numero, paciente_nome, paciente_telefone, medico_nome, forma_pagamento, valor_total, valores_pagamento, status, created_at, categoria",
      )
  ...
  return (
    <div
      className={
        embedCompact ? "flex flex-col h-full min-h-0" : "space-y-4 flex flex-col h-full min-h-0"
      }
    >
      {!embedded && (
      ...
      <div
        className={`flex items-center gap-3 flex-wrap${embedded ? " px-4 py-2.5 border-b bg-card" : ""}`}
      >
              ...
              <tbody className="[&>tr:first-child>td]:pt-3">
                {loading ? (
                  ...
                  filtered.map((o) => (
                    <tr
                      key={o.id}
                      className="h-14 border-b border-border/40 hover:bg-muted/30 transition-colors"
                    >
                      ...
                      <td className="px-3">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          {podeEscrever && (
                            <IconAction
                              label="Converter itens (vender, agendar, cancelar, NFS-e)"
                              onClick={() => setConversaoId(o.id)}
                            >
                              <Workflow className="h-4 w-4 text-primary" />
                            </IconAction>
                          )}
                          <IconAction label="Imprimir" onClick={() => void imprimir(o.id)}>
                            <Printer className="h-4 w-4" />
                          </IconAction>
                          {podeVerHistorico && (
  ...
  useEffect(() => {
    ...
    const idsFaltando = Array.from(
      new Set(
        itens
          .filter(
            (i) => i.procedimento_id && formasPagamento.some((f) => i.valores_formas?.[f] == null),
          )
    ...
    (async () => {
      const { data } = await supabase
        .from("procedimentos")
        .select(
          "id, valor_dinheiro_pix, valor_cartao, valor_dinheiro, valor_pix, valor_cartao_credito, valor_cartao_debito, valor_padrao, preparo",
        )
        .in("id", idsFaltando);
  ...
  const salvar = async () => {
    ...
    for (let i = 0; i < itens.length; i++) {
      ...
      if (!Number.isFinite(vu) || vu <= 0) {
        return toast.error(
          `Item ${i + 1} (${it.descricao}): valor unitário deve ser maior que zero`,
        );
      }
    }
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">
                  Ou preencha manualmente abaixo para paciente novo.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    onChange={(e) => setPacienteTelefone(e.target.value.replace(/[<>]/g, ""))}
                  />
                </div>
                  ...
                  {medicoParticular ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <Input
                          value={clinicaSolicitante}
                          onChange={(e) => setClinicaSolicitante(e.target.value)}
                          placeholder="Nome da clínica/local de origem"
                        />
                      </div>
                    </div>
                  )}
                      ...
                      {procResults.map((p) => (
                        ...
                        >
                          ...
                          <span className="text-sm font-semibold whitespace-nowrap flex gap-2">
                            {(formasPagamento.length ? formasPagamento : ["Dinheiro"]).map(
                              (f, i) => (
                                <span
                                  key={f}
                  ...
                  {itens.some((i) => i.preparo && i.preparo.trim()) && (
                    <div className="bg-amber-50 dark:bg-amber-950/30 border-b border-amber-200 dark:border-amber-900 px-3 py-2 space-y-1">
                      <div className="flex items-center gap-2 text-sm font-semibold text-amber-800 dark:text-amber-200">
                        <AlertTriangle className="h-4 w-4" /> Atenção: este orçamento contém
                        exame(s) com preparo
                      </div>
                      <ul className="text-xs text-amber-900 dark:text-amber-100 space-y-0.5 pl-6 list-disc">
                        {itens
                          .filter((i) => i.preparo && i.preparo.trim())
                          .map((i, idx) => (
                            <li key={idx}>
                              <b>{i.descricao}:</b> {i.preparo}
                            </li>
                          ))}
                      </ul>
                    </div>
                          ...
                          <td className="px-2 py-1">
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                setItens((a) =>
                                  a.map((x, i) => (i === idx ? { ...x, quantidade: n } : x)),
                                );
                              }}
                            />
                          </td>
                ...
                <div className="space-y-2">
                  ...
                  {Number(desconto) > subtotal && (
                    <p className="text-xs text-destructive text-right">
                      Desconto não pode ser maior que o subtotal ({BRL(subtotal)}).
                    </p>
                  )}
