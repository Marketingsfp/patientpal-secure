...
function EquipePage() {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      const medicosUserIds = new Set(
        medicosRaw.map((x) => x.user_id).filter((x): x is string => !!x),
      );

      // Buscar nomes dos usuários pendentes
      const idsMedicoPendente = Array.from(
        new Set(
          mems
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button size="icon" variant="ghost" asChild className="h-8 w-8">
                                  <Link
                                    to="/app/medico/$medicoId"
                                    params={{ medicoId: m.id }}
                                    aria-label="Ver agenda do médico"
                                  >
                                ...
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Button
                                      ...
                                      className={cn(
                                        "h-8 w-8",
                                        m.ativo ? "text-destructive" : "text-emerald-600",
                                      )}
                                  ...
                                  <TooltipContent>
                                    {m.ativo ? "Inativar médico" : "Reativar médico"}
                                  </TooltipContent>
                                </Tooltip>
                              </>
                            ))}
