...
function FluxoPage() {
  ...
  const carregar = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from("agendamentos")
      .select(
        "id, paciente_id, paciente_nome, procedimento, inicio, fluxo_etapa, fluxo_atualizado_em, prioridade, medicos(nome)",
      )
    ...
    const pacIds = Array.from(
      new Set(reais.map((a) => a.paciente_id).filter((x): x is string => !!x)),
    );
    if (pacIds.length) {
    ...
    if (reais.length) {
      ...
      status.forEach((s: StatusPagamento, id: string) => {
        if (s.pago) idsPagos.add(id);
      });
      setPagos(idsPagos);
      ...
      if (presosNoCaixa.length) {
        await supabase
          ...
          .in(
            "id",
            presosNoCaixa.map((a) => a.id),
          );
        setAgs(
          reais.map((a) =>
            idsPagos.has(a.id) && a.fluxo_etapa === "caixa"
              ? { ...a, fluxo_etapa: "triagem" as Etapa }
              : a,
          ),
        );
      }
  ...
  async function setEtapa(id: string, etapa: Etapa, opts?: { silencioso?: boolean }) {
    ...
    setAgs((prev) =>
      prev.map((a) =>
        a.id === id ? { ...a, fluxo_etapa: etapa, fluxo_atualizado_em: agoraISO } : a,
      ),
    );
  ...
  const agsFiltrados = useMemo(() => {
    ...
    return ags.filter((a) => {
      ...
      return (
        a.paciente_nome.toLowerCase().includes(termo) || (pront ?? "").toLowerCase().includes(termo)
      );
    });
        ...
        <div className="flex items-start gap-2.5 min-w-0">
          ...
          <div className="min-w-0 space-y-1.5">
            ...
            <div className="hidden sm:flex flex-wrap items-center gap-1">
              {["Recepção", "Caixa", "Triagem", "Atendimento ou Exame", "Finalizado"].map(
                (etapa, i, arr) => (
            ...
            >
              ...
              <div className="space-y-2">
                ...
                {items.map((a) => {
                  ...
                  const next =
                    nextBruto === "caixa" && pagos.has(a.id) ? ("triagem" as Etapa) : nextBruto;
                    ...
                    >
                                ...
                                {a.paciente_id && (
                                  <DropdownMenuItem asChild>
                                    <Link
                                      ...
                                      params={{ pacienteId: a.paciente_id }}
                                    >
                      ...
                      {a.paciente_id && (
                        <BadgePacienteDistante
                          cidade={cidades.get(a.paciente_id) ?? null}
                          compact
                          className="mt-1.5"
                        />
                      )}
                          ...
                          <>
                            <Button
                              ...
                              className={cn(
                                acaoBtnCls,
                                acaoTxtCls,
                                "ml-auto gap-1 border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100 hover:text-blue-800",
                              )}
