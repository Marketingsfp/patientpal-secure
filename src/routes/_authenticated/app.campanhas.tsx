...
interface Row { id: string; nome: string; tipo: string; mensagem: string; segmento: string | null; status: string; agendada_para: string | null; }
...
function CampanhasPage() {
  ...
  return (
    <SimpleCrud<Row, Form>
      ...
      emptyForm={{ nome: "", tipo: "whatsapp", mensagem: "", segmento: "", status: "rascunho", agendada_para: "" }}
      toForm={r => ({ nome: r.nome, tipo: r.tipo, mensagem: r.mensagem, segmento: r.segmento ?? "", status: r.status, agendada_para: r.agendada_para ? r.agendada_para.slice(0,16) : "" })}
      ...
      renderForm={(f, set) => (
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1"><Label>Segmento</Label><Input value={f.segmento} onChange={e => set({ ...f, segmento: e.target.value })} placeholder="Ex: Aniversariantes" /></div>
            <div className="space-y-1"><Label>Agendar para</Label><Input type="datetime-local" value={f.agendada_para} onChange={e => set({ ...f, agendada_para: e.target.value })} /></div>
          </div>
          <div className="space-y-1"><Label>Mensagem *</Label><Textarea rows={5} required value={f.mensagem} onChange={e => set({ ...f, mensagem: e.target.value })} placeholder="Olá {{nome}}, ..." /></div>
