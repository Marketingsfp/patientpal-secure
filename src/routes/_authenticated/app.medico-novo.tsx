...
export const Route = createFileRoute("/_authenticated/app/medico-novo")({
  component: NovoMedicoPage,
  head: () => ({ meta: [{ title: "Novo médico — ClinicaOS" }] }),
});

function NovoMedicoPage() {
  ...
  return (
    <div className="space-y-4 max-w-5xl mx-auto p-4">
      ...
      <MedicoFormDialog
        ...
        onSaved={() => void navigate({ to: "/app/equipe" })}
        onClose={() => void navigate({ to: "/app/equipe" })}
      />
    </div>
  );
}