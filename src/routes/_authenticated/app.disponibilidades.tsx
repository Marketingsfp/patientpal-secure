...
function Page() {
  ...
  const [bloqueiosAtivos, setBloqueiosAtivos] = useState<
    Array<{ id: string; inicio: string; fim: string; observacoes: string | null }>
  >([]);
  ...
  const vigenciasSeSobrepoem = (
    ...
    const aI = aIni ?? "0000-01-01",
      aF = aFim ?? "9999-12-31";
    const bI = bIni ?? "0000-01-01",
      bF = bFim ?? "9999-12-31";
    return aI <= bF && bI <= aF;
  };
  ...
  const salvarEdicao = async () => {
    ...
    if (
      editRow.vigencia_inicio &&
      editRow.vigencia_fim &&
      editRow.vigencia_fim < editRow.vigencia_inicio
    ) {
      toast.error("A vigência final não pode ser antes da vigência inicial.");
      return;
    }
  ...
  const resumoGeracao = useMemo(() => {
    const medicosSelecionados =
      gerar.medico_id === "all" ? medicos.length : gerar.medico_id ? 1 : 0;
    let diasNoPeriodo = 0;
    ...
    const porMedico =
      medicosSelecionados > 0 ? Math.round(slotsPreview.length / medicosSelecionados) : 0;
  ...
  const aplicarBloqueio = async () => {
    ...
    try {
      ...
      if (livres.length === 0) {
        toast.info(
          "Nenhum horário livre encontrado nesse período para bloquear. Gere a agenda desse período antes, se ainda não existir.",
        );
        return;
      }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-1">
          Total de médicos: <strong>{medicos.length}</strong> · Total de agendas:{" "}
          <strong>{agendas.length}</strong> · Total de disponibilidades:{" "}
          <strong>{disps.length}</strong>
        </p>
      </div>
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  {slotsPreview.length > 0 ? (
                    <>
                      ...
                      <strong>
                        {slotsPreview[0].inicio}–{slotsPreview[0].fim}
                      </strong>
                      .
                    </>
                  ) : (
                              ...
                              <div className="flex items-center justify-end gap-1">
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    ...
                                    if (!primeira) {
                                      toast.warning(
                                        "Este médico não possui agenda ativa. Crie uma agenda primeiro.",
                                      );
                                    }
                                  }}
      ...
      >
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Bloqueia horários já livres nesse período sem mexer na disponibilidade recorrente da
              semana. Horários que já têm paciente marcado não são alterados.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            ...
            <div className="pt-2 border-t">
              ...
              {carregandoBloqueios ? (
                ...
                <p className="text-xs text-muted-foreground">
                  Nenhum bloqueio ativo para este médico.
                </p>
              ) : (
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setBloqueioMedico(null)}>
              Fechar
            </Button>
          </DialogFooter>
        </DialogContent>
