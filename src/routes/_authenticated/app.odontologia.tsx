...
function OdontologiaPage() {
  ...
  const [itensPorDente, setItensPorDente] = useState<
    Array<{
      id: string;
      descricao: string;
      valor_total: number;
  ...
  async function removerDenteDoItem(itemId: string, dente: number) {
    if (!podeEscrever) {
      toast.error("Sem permissão de edição.");
      return;
    }
                ...
                <TabsContent value="odontograma" className="space-y-4">
                            ...
                            {itensDoDenteSelecionado.map((it) => (
                              <li key={it.id} className="flex items-center justify-between gap-2">
                                <span>
                                  ...
                                  {Number(it.valor_total).toFixed(2)}
                                </span>
                                {podeEscrever && (
                                  <button
                                    type="button"
                        ...
                        <div className="border rounded-md p-4 space-y-3">
                          <div className="flex items-center justify-between gap-2 flex-wrap">
                            <p className="font-medium">
                              Dente {selecionado} ·{" "}
                              <span className="text-primary">{FACE_LABEL[faceSelecionada]}</span>
                            </p>
                            <Button
                              size="sm"
                              variant="outline"
                          ...
                          <div className="grid md:grid-cols-4 gap-3">
                            <div>
                              ...
                              <Select
                                ...
                                onValueChange={(v) => setFaceSelecionada(v as OdontoFace)}
                              >
                                <SelectTrigger>
                                  <SelectValue />
                                </SelectTrigger>
                      ...
                      {selecionado && historicoDenteSelecionado.length > 0 && (
                        <div className="border rounded-md p-3 space-y-2">
                          ...
                          <Table>
                            ...
                            <TableBody>
                              {historicoDenteSelecionado.map((r) => (
                                <TableRow key={r.id}>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      {planoPorDente.length === 0 ? (
                        <p className="text-sm text-muted-foreground">
                          Nenhum item em orçamento aberto vinculado a dentes. Selecione um dente no
                          odontograma e clique em "Incluir em orçamento".
                        </p>
                      ) : (
              ...
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Todos os orçamentos com procedimentos da especialidade Odontologia. Use a
                    pesquisa para filtrar por paciente.
                  </p>
                </div>
                <Button
                  size="sm"
