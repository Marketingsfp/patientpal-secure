...
function FeriasPage() {
  ...
  async function load() {
    ...
    const [f, c] = await Promise.all([
      ...
      supabase
        .from("hr_contratos")
        .select("id,funcionario_nome,data_admissao")
        .eq("clinica_id", clinicaAtual.clinica_id)
        .eq("status", "ativo")
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground">
            Solicitações e gestão de períodos de férias.
          </p>
        </div>
              ...
              rows.map((r) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell className="text-sm">
                    {formatDatePura(r.periodo_aquisitivo_inicio)} →{" "}
                    {formatDatePura(r.periodo_aquisitivo_fim)}
                  </TableCell>
                  <TableCell className="text-sm">
                    {r.inicio ? `${formatDatePura(r.inicio)} → ${formatDatePura(r.fim)}` : "—"}
                  </TableCell>
              ...
              <div className="flex items-end">
                <label className="flex items-center gap-2 text-sm">
                  <input
                    ...
                    onChange={(e) => setForm({ ...form, abono_pecuniario: e.target.checked })}
                  />
