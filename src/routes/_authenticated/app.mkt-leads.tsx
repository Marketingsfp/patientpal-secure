...
interface Row {
  ...
  origem: string | null; status: string; mensagem: string | null; created_at: string;
}
interface Form { nome: string; telefone: string; email: string; mensagem: string; origem: string; status: string; }
  ...
  return (
    <SimpleCrud<Row, Form>
      ...
      columns={[
        ...
        { key: "origem", header: "Origem", className: "w-32", render: r => <span className="text-xs capitalize">{r.origem ?? "—"}</span> },
        { key: "status", header: "Status", className: "w-28", render: r => (
          <span className={`text-xs px-2 py-0.5 rounded-full capitalize ${statusCor[r.status] ?? "bg-muted text-muted-foreground"}`}>{r.status}</span>
        ) },
        { key: "data", header: "Capturado em", className: "w-40", render: r => (
          <span className="text-xs text-muted-foreground">{new Date(r.created_at).toLocaleString("pt-BR")}</span>
        ) },
      ]}
      emptyForm={{ nome: "", telefone: "", email: "", mensagem: "", origem: "manual", status: "novo" }}
      toForm={r => ({
        nome: r.nome, telefone: r.telefone ?? "", email: r.email ?? "",
        mensagem: r.mensagem ?? "", origem: r.origem ?? "manual", status: r.status,
      })}
      toPayload={f => ({
        nome: f.nome.trim(),
      ...
      renderForm={(f, set) => (
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1"><Label>E-mail</Label><Input type="email" value={f.email} onChange={e => set({ ...f, email: e.target.value })} /></div>
          </div>
          <div className="grid grid-cols-2 gap-3">
          ...
          <div className="space-y-1"><Label>Mensagem</Label><Textarea rows={3} value={f.mensagem} onChange={e => set({ ...f, mensagem: e.target.value })} /></div>
        </div>
      )}
    />
  );
