...
function PontoPage() {
  ...
  async function load() {
    ...
    const [p, u] = await Promise.all([
      supabase
        .from("hr_pontos")
        .select("id,tipo,marcado_em,latitude,longitude,unidade_id,dentro_raio")
    ...
    <div className="p-6 space-y-4 max-w-3xl mx-auto">
      ...
      <Card className="p-6 text-center">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-6">
          <Button onClick={() => bater("entrada")} disabled={marcando}>
            <LogIn className="h-4 w-4 mr-1" /> Entrada
          </Button>
          <Button variant="outline" onClick={() => bater("intervalo_inicio")} disabled={marcando}>
            <Coffee className="h-4 w-4 mr-1" /> Início intervalo
          </Button>
          <Button variant="outline" onClick={() => bater("intervalo_fim")} disabled={marcando}>
            <Coffee className="h-4 w-4 mr-1" /> Fim intervalo
          </Button>
          <Button variant="destructive" onClick={() => bater("saida")} disabled={marcando}>
            <LogOut className="h-4 w-4 mr-1" /> Saída
          </Button>
        </div>
        {unidades.length === 0 && (
          <p className="text-xs text-muted-foreground mt-3">
            Cadastre unidades com geolocalização para validar a presença.
          </p>
        )}
      </Card>
                  ...
                  <TableCell>
                    {p.dentro_raio === true ? (
                      <Badge variant="default">
                        ...
                        {unidades.find((u) => u.id === p.unidade_id)?.nome ?? "Na unidade"}
                      </Badge>
                    ) : p.dentro_raio === false ? (
