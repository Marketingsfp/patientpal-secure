  ...
  return (
    <SimpleCrud<Row, Form>
      ...
      validate={(f) => {
        ...
        const telDigits = f.telefone.replace(/\D/g, "");
        if (telDigits && (telDigits.length < 10 || telDigits.length > 11)) return "Telefone deve ter 10 ou 11 dígitos (DDD + número).";
      ...
      renderForm={(f, set) => (
        <div className="space-y-3">
          <div className="space-y-1">
            ...
            <Input required maxLength={120} value={f.nome_lead} onChange={e => set({ ...f, nome_lead: e.target.value })} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <Input
                ...
                value={f.telefone}
                onChange={e => set({ ...f, telefone: e.target.value.replace(/[^\d()\-\s+]/g, "") })}
              />
            </div>
            <div className="space-y-1">
              <Label>Email</Label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="space-y-1">
              ...
              <p className="text-[11px] text-muted-foreground">Digite os centavos: ex. 150000 = R$ 1.500,00</p>
            </div>
            <div className="space-y-1"><Label>Origem</Label><Input maxLength={60} value={f.origem} onChange={e => set({ ...f, origem: e.target.value })} placeholder="Ex: Instagram" /></div>
          ...
          <div className="space-y-1"><Label>Observações</Label><Textarea rows={3} maxLength={2000} value={f.observacoes} onChange={e => set({ ...f, observacoes: e.target.value })} /></div>
        </div>
      )}
    />
  );
