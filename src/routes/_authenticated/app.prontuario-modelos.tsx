...
type Modelo = { id: string; nome: string; prompt_ia: string | null; ativo: boolean; secoes: Secao[] };
...
function ModelosPage() {
  ...
  return (
    <SimpleCrud<Modelo, Form>
      ...
      emptyForm={EMPTY}
      toForm={(r) => ({ nome: r.nome, prompt_ia: r.prompt_ia ?? "", ativo: r.ativo, secoes: r.secoes ?? [] })}
      toPayload={(f) => ({ nome: f.nome, prompt_ia: f.prompt_ia || null, ativo: f.ativo, secoes: f.secoes })}
      renderForm={(f, set) => (
        <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-1">
          <div className="space-y-1">
            <Label>Nome do modelo / Especialidade *</Label>
            <Input value={f.nome} onChange={(e) => set({ ...f, nome: e.target.value })} placeholder="Ex: Cardiologia" />
          </div>
          <div className="space-y-1">
            <Label>Instrução extra para a IA (opcional)</Label>
            <Textarea rows={3} value={f.prompt_ia ?? ""} onChange={(e) => set({ ...f, prompt_ia: e.target.value })}
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Button type="button" size="sm" variant="outline"
                onClick={() => set({ ...f, secoes: [...f.secoes, { chave: `campo_${f.secoes.length + 1}`, titulo: "Novo campo", placeholder: "" }] })}>
