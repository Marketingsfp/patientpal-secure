...
function Page() {
  ...
  const load = async () => {
    ...
    const { data, error } = await supabase
      .from("fin_contas")
      .select("id, nome, tipo, banco, agencia, conta, saldo_inicial, ativo, bandeira")
      .eq("clinica_id", clinicaAtual.clinica_id)
      .eq("ativo", true)
      .order("nome");
    if (error) mostrarErro(error);
    else setItems((data ?? []) as Conta[]);
    setLoading(false);
  };
  useEffect(() => {
    void load();
  ...
  const submit = async (e: FormEvent) => {
    ...
    const payload = {
      ...
      nome: form.nome.trim(),
      tipo: form.tipo as "banco" | "caixa" | "cartao" | "maquininha" | "outro",
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Contas bancárias, caixa e cartões</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{editing ? "Editar" : "Nova"} conta</DialogTitle>
            </DialogHeader>
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setForm({
                        ...form,
                        tipo: v,
                        bandeira: tipoUsaBandeira(v) ? form.bandeira : "",
                      })
                    }
                  >
                    <SelectTrigger>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm mt-2">
                      Saldo inicial: <strong>{fmt(Number(c.saldo_inicial))}</strong>
                    </p>
                    {podeEscrever && (
