...
function ProcedimentosPage() {
  ...
  const loadRegras = async () => {
    ...
    const { data, error } = await (supabase as any)
      ...
      .select(
        "id,convenio_id,especialidade_id,procedimento_id,tipo,modo,valor,valor_cartao,percentual,percentual_cartao,prioridade,ativo",
      )
      .eq("clinica_id", clinicaAtual.clinica_id)
      .eq("ativo", true);
    if (error) {
          ...
          <form onSubmit={onSubmit} className="flex flex-col min-h-0 flex-1 overflow-hidden">
            <div className="space-y-4 overflow-y-auto overflow-x-hidden px-6 py-4 flex-1 min-h-0">
              ...
              <div className="rounded-lg border border-border bg-muted/30 p-3 space-y-3">
                ...
                <div
                  className={`grid grid-cols-2 gap-3 ${form.valor_variavel ? "opacity-50 pointer-events-none" : ""}`}
                >
                  <div className="space-y-1">
                    <Label>Dinheiro (R$)</Label>
                    <CurrencyInput
                      value={form.valor_variavel ? "0" : form.valor_dinheiro}
                  ...
                  <div className="space-y-1">
                    ...
                    <CurrencyInput
                      value={form.valor_variavel ? "0" : form.valor_pix_cartao}
                      disabled={form.valor_variavel}
