...
export const Route = createFileRoute("/_authenticated/app/estoque")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Controle de insumos, lotes, validade, estoque mínimo e reposição da clínica.",
      },
...
function EstoquePage() {
  ...
  const carregar = useCallback(async () => {
    ...
    const [p, l] = await Promise.all([
      supabase
        ...
        .select(
          "id, nome, codigo, unidade, estoque_atual, estoque_minimo, custo_unitario, categoria, fornecedor, observacoes, ativo",
        )
  ...
  const loteVigente = useCallback(
    (produtoId: string) =>
      (lotesPorProduto.get(produtoId) ?? []).find((l) => Number(l.quantidade) > 0) ?? null,
    [lotesPorProduto],
  );
  ...
  const kpis = useMemo(
    () => ({
      ...
      valor: enriquecidos.reduce(
        (s, p) => s + Number(p.estoque_atual) * Number(p.custo_unitario || 0),
        0,
      ),
    }),
    [enriquecidos],
  ...
  const gerarPedido = () => {
    ...
    if (itens.length === 0) {
      toast.info("Nenhum item abaixo do mínimo no momento.");
      return;
    }
    printPedidoCompra({
    ...
    <div className="space-y-5 p-4 sm:p-6">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <KpiCard
          ...
          hint={fmtBRL(kpis.valor) + " em estoque"}
        />
        <KpiCard
        ...
        {loading ? (
          ...
          <div className="space-y-1 py-14 text-center">
            ...
            <p className="text-xs text-muted-foreground">
              Ajuste os filtros ou registre uma entrada de estoque.
            </p>
          </div>
        ) : (
          <Table>
                ...
                return (
                  <TableRow key={p.id} className={cn(!p.ativo && "opacity-60")}>
                    ...
                    <TableCell>
                      ...
                      {p.fornecedor && (
                        <div className="text-xs text-muted-foreground">{p.fornecedor}</div>
                      )}
                    </TableCell>
                    <TableCell>
                      ...
                      {!p.lote ? (
                        ...
                        <div className="space-y-1">
                          ...
                          {sv === "vencido" ? (
                            <Badge
                              variant="outline"
                              className="animate-pulse border border-rose-500/30 bg-rose-500/20 text-rose-700"
                            >
                            ...
                            <Badge
                              variant="outline"
                              className="border-amber-500/25 bg-amber-500/15 text-amber-700"
                            >
                              Vence em {dias}d · {fmtValidade(p.lote.validade)}
                            </Badge>
                          ) : (
                            <span className="text-xs text-muted-foreground">
                              {fmtValidade(p.lote.validade)}
                            </span>
                          )}
                        </div>
                      )}
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button variant="ghost" size="sm" aria-label={`Ações de ${p.nome}`}>
                            ···
                          </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem
                            onClick={() => setOpenProduto(p)}
      ...
      {clinicaId && (
        <>
          ...
          <ProdutoDialog
            alvo={openProduto}
            onOpenChange={(v) => !v && setOpenProduto(null)}
            clinicaId={clinicaId}
            onSaved={carregar}
          />
          <HistoricoDialog
            produto={historicoDe}
            onOpenChange={(v) => !v && setHistoricoDe(null)}
            lotes={lotes}
          />
        </>
      )}
...
function EntradaDialog({
  ...
  const salvar = async (e: React.FormEvent) => {
    ...
    if (!produtoId && !novoNome.trim()) {
      toast.error("Selecione um item ou informe o nome do novo item.");
      return;
    }
    setSaving(true);
    try {
      ...
      const { error: erroMov } = await supabase.from("estoque_movimentos").insert({
        ...
        quantidade,
        custo_unitario: Number(custo) || 0,
        lote_id: (loteRow as { id: string }).id,
        motivo: "compra",
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Registre a compra ou recebimento informando lote e validade.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={salvar} className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-1">
              ...
              <Select
                value={produtoId || "novo"}
                onValueChange={(v) => setProdutoId(v === "novo" ? "" : v)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
...
function BaixaDialog({
  ...
  onOpenChange: (v: boolean) => void;
  produtos: Produto[];
  lotesPorProduto: Map<string, Lote[]>;
  clinicaId: string;
  onSaved: () => Promise<void>;
}) {
  const [produtoId, setProdutoId] = useState("");
  ...
  const previa = useMemo(() => alocarFefo(lotesDoProduto, Number(qtd) || 0), [lotesDoProduto, qtd]);
      ...
      const registros: MovInsert[] =
        previa.alocacoes.length > 0
          ? previa.alocacoes.map((a) => ({
              ...
              motivo,
              observacoes:
                [obs, a.lote ? `Lote ${a.lote}` : null].filter(Boolean).join(" · ") || null,
            }))
          : [
              {
                clinica_id: clinicaId,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Os lotes com validade mais próxima são consumidos primeiro (FEFO).
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={salvar} className="space-y-4">
          <div className="space-y-1">
            ...
            <Select value={produtoId} onValueChange={setProdutoId}>
              ...
              <SelectContent>
                {produtos.map((p) => (
                  <SelectItem key={p.id} value={p.id}>
                    {p.nome} — saldo {Number(p.estoque_atual)} {p.unidade}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          {produtoId && (
            <div className="rounded-lg border bg-muted/40 p-3 text-sm">
              ...
              {previa.alocacoes.length === 0 ? (
                <p className="text-xs text-muted-foreground">
                  Nenhum lote com saldo — a baixa sai do saldo geral do item.
                </p>
              ) : (
                <ul className="space-y-1">
                  {previa.alocacoes.map((a) => (
                    <li key={a.lote_id} className="flex justify-between gap-3">
                      <span>
                        {a.lote || "Lote —"} · venc. {fmtValidade(a.validade)}
                      </span>
              ...
              {previa.restante > 0 && previa.alocacoes.length > 0 && (
                <p className="mt-2 text-xs font-medium text-amber-700">
                  {previa.restante} unidade(s) sem lote correspondente serão baixadas do saldo
                  geral.
                </p>
              )}
            </div>
    ...
    setForm(
      editando
        ? {
            ...
            categoria: editando.categoria ?? "insumos",
  ...
  const salvar = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = editando
        ...
        : await supabase
            .from("estoque_produtos")
            .insert({ ...payload, clinica_id: clinicaId } as never);
      if (error) throw error;
...
function HistoricoDialog({
  ...
  useEffect(() => {
    ...
    void supabase
      ...
      .limit(100)
      .then(({ data }) => {
        setMovs((data ?? []) as unknown as Movimento[]);
        setLoading(false);
      });
  }, [produto]);
                ...
                {movs.map((m) => (
                  <TableRow key={m.id}>
                    ...
                    <TableCell className="text-xs">
                      ...
                      {m.observacoes && (
                        <span className="text-muted-foreground"> · {m.observacoes}</span>
                      )}
                    </TableCell>
                  </TableRow>
                ))}
