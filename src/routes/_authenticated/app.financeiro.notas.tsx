...
function Page() {
  ...
  const [emitDialog, setEmitDialog] = useState<{ open: boolean; nota: Nota | null }>({
    open: false,
    nota: null,
  });
  ...
  const remove = async (n: Nota) => {
    ...
    if (!(await confirmDialog(`Excluir nota ${n.numero ?? ""}?`))) return;
  ...
  const openEmit = async (n: Nota) => {
    ...
    setDescricao(
      n.observacoes ||
        `Serviços médicos prestados${n.paciente_id ? ` ao paciente ${pacMap.get(n.paciente_id) ?? ""}` : ""}`.trim(),
    );
    setTomadorCpf("");
    if (n.paciente_id) {
  ...
  const doEmit = async () => {
    ...
    if (!n.paciente_id) {
      toast.error("Vincule um paciente à nota antes de emitir.");
      return;
    }
    setEmitting(true);
    try {
      ...
      if (nfseId) {
        ...
        const cons = (await consultarFn({ data: { id: nfseId } })) as {
          focus?: { url_danfse?: string; status?: string };
        };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold">Notas dos pacientes</h1>
          <p className="text-sm text-muted-foreground">Registro e controle de NFs emitidas</p>
        </div>
              ...
              <TableBody>
                {items.map((n) => (
                  <TableRow key={n.id}>
                    ...
                    <TableCell className="text-sm">
                      {n.paciente_id ? (pacMap.get(n.paciente_id) ?? "—") : "—"}
                    </TableCell>
                    <TableCell>
                      <Badge variant={n.status === "emitida" ? "default" : "secondary"}>
                        {n.status}
                      </Badge>
                    </TableCell>
      ...
      <Dialog
        ...
        onOpenChange={(o) => setEmitDialog({ open: o, nota: o ? emitDialog.nota : null })}
      >
        <DialogContent>
          <DialogHeader>
          ...
          <div className="space-y-3">
            ...
            <div className="text-sm text-muted-foreground">
              Tomador:{" "}
              <b>{emitDialog.nota?.paciente_id ? pacMap.get(emitDialog.nota.paciente_id) : "—"}</b>
              <br />
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setEmitDialog({ open: false, nota: null })}
              disabled={emitting}
            >
              Cancelar
            </Button>
            <Button
              ...
              disabled={
                ...
                !emitenteId ||
                !(
                  (tomadorCpf || "").replace(/\D/g, "").length === 11 ||
                  (tomadorCpf || "").replace(/\D/g, "").length === 14
                )
              }
            >
              {emitting ? (
