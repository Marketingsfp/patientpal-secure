  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data: med } = await supabase
        .from("medicos")
        .select(
          "id, nome, crm, crm_uf, ativo, email, telefone, nacionalidade, estado_civil, cep, logradouro, numero, complemento, bairro, cidade, estado, tipo_repasse, percentual_repasse_padrao, valor_repasse_padrao, clinica_id, medico_especialidades(especialidade:especialidades(nome))",
        )
        .eq("id", medicoId)
        .maybeSingle();
      if (cancel) return;
      ...
      setMedico(
        med
          ? ({
              ...
              banco: sens.banco ?? null,
              agencia: sens.agencia ?? null,
              conta: sens.conta ?? null,
              pix_chave: sens.pix_chave ?? null,
            } as MedicoFull)
          : null,
      );
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-muted-foreground">
                CRM {medico.crm}/{medico.crm_uf} • {especialidades}
              </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        >
          <CardContent className="pt-6">
            ...
            <p className="text-2xl font-bold mt-1">{fmtMoeda(totalFaturado)}</p>
          </CardContent>
        </Card>
        <Card
          className="cursor-pointer hover:bg-muted/50 transition-colors"
        ...
        >
          <CardContent className="pt-6">
            ...
            <p className="text-2xl font-bold mt-1">{fmtMoeda(totalRepasse)}</p>
          </CardContent>
        </Card>
      </div>
          ...
          <Card>
            <CardContent className="pt-6 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              ...
              <Info
                ...
                value={
                  medico.data_nascimento
                    ? `${formatDatePura(medico.data_nascimento)}${idade !== null ? ` (${idade} anos)` : ""}`
                    : null
                }
              />
              ...
              <Info
                label="Endereço"
                value={
                  [medico.logradouro, medico.numero, medico.complemento, medico.bairro]
                    .filter(Boolean)
                    .join(", ") || null
                }
              />
              <Info
                label="Cidade/UF"
                value={medico.cidade ? `${medico.cidade}/${medico.estado ?? ""}` : null}
              />
          ...
          <Card>
            <CardContent className="pt-6 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <Info
                ...
                value={
                  medico.tipo_repasse === "percentual"
                    ? `${medico.percentual_repasse_padrao}%`
                    : fmtMoeda(medico.valor_repasse_padrao)
                }
              />
              ...
              {agendamentos.length === 0 ? (
                ...
                <Table>
                  ...
                  <TableBody>
                    {agendamentos.map((a) => (
                      <TableRow key={a.id}>
                        <TableCell>{formatDateTime(a.inicio)}</TableCell>
              ...
              {atendimentos.length === 0 ? (
                ...
                <Table>
                  ...
                  <TableBody>
                    {atendimentos.map((a) => (
                      <TableRow key={a.id}>
                        <TableCell>{formatDatePura(a.data)}</TableCell>
                        <TableCell>{a.procedimento ?? "—"}</TableCell>
                        <TableCell>{fmtMoeda(a.valor_total)}</TableCell>
                        <TableCell>{fmtMoeda(a.valor_medico)}</TableCell>
                        <TableCell>
                          <Badge variant="outline">{a.status}</Badge>
                        </TableCell>
                      </TableRow>
                ...
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right">
                      {drill === "faturado" ? "Valor total" : "Repasse"}
                    </TableHead>
                    <TableHead>Status</TableHead>
                  </TableRow>
                </TableHeader>
                  ...
                  {atendimentos.map((a) => (
                    <TableRow key={a.id}>
                      ...
                      <TableCell className="text-right font-semibold">
                        {fmtMoeda(drill === "faturado" ? a.valor_total : a.valor_medico)}
                      </TableCell>
                      <TableCell>
                        <Badge variant="outline">{a.status}</Badge>
                      </TableCell>
