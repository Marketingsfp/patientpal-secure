  ...
  const load = async () => {
    ...
    const [cs, ps, ds, ls] = await Promise.all([
      supabase
        ...
        .select(
          "id, numero, paciente_id, paciente_nome, plano_id, valor_mensal, taxa_adesao, status, data_inicio, assinado_em",
        )
        .eq("clinica_id", cid)
      ...
      supabase
        ...
        .select("id, nome, tipo, valor_mensal")
        .eq("clinica_id", cid),
    ...
    const allMensRes = allCIds.length
      ? await supabase
          ...
          .select("id, contrato_id, valor, status, pago_em, vencimento")
          .in("contrato_id", allCIds)
          .eq("status", "pago")
          .limit(50000)
      : { data: [] as Mens[] };
    ...
    if (which === "titulares") {
      setDrill({
        ...
        rows: contratos.map((c) => ({
          nome: c.paciente_nome,
          plano: planoNome.get(c.plano_id) ?? "—",
          status: c.status,
      ...
      const rows = [
        ...contratos.map((c) => ({
          nome: c.paciente_nome,
          tipo: "Titular",
          vinculo: planoNome.get(c.plano_id) ?? "—",
        })),
        ...deps.map((d) => ({
          nome: d.paciente_nome,
          tipo: "Dependente",
      ...
      const rows = [
        ...pagas.map((m) => ({
          ...
          descricao: `Mensalidade — ${contratoNome.get(m.contrato_id) ?? "—"}`,
          valor: BRL(m.valor),
        })),
        ...contratos
          .filter((c) => Number(c.taxa_adesao || 0) > 0)
          .map((c) => ({
