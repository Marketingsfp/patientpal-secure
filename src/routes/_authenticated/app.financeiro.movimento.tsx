...
function Page() {
  ...
  useEffect(() => {
    void load();
    void loadResumo();
  }, [clinicaAtual?.clinica_id, filterTipo, fromDate, toDate, filterStatus]);
  useEffect(() => {
    void loadOpts();
            ...
            <>
              {resumo.totalRows > items.length ? (
                <div className="px-4 py-2 text-xs text-muted-foreground bg-muted/30 border-b">
                  Exibindo os {items.length.toLocaleString("pt-BR")} lançamentos mais recentes de{" "}
                  {resumo.totalRows.toLocaleString("pt-BR")} no período. Os totais acima consideram
                  todos.
                </div>
              ) : null}
              <Table>
