...
export const Route = createFileRoute("/_authenticated/app/financeiro/atendimentos-externos")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Relatório de atendimentos faturados em outras clínicas parceiras.",
      },
    ],
  }),
});
...
function Page() {
  ...
  const carregar = async () => {
    ...
    const { data, error } = await supabase
      .from("agendamentos")
      .select(
        "id,inicio,paciente_nome,procedimento,origem_clinica_nome,origem_valor,medico:medicos(nome)",
      )
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <p className="text-sm text-slate-600">
        Pacientes atendidos aqui mas faturados em outra clínica. Não geram caixa — alimentam o
        repasse do médico local e o acerto entre clínicas.
      </p>
              ...
              {rows.length === 0 && (
                <TableRow>
                  <TableCell colSpan={6} className="text-center text-sm text-slate-500 py-8">
                    Nenhum atendimento externo no período.
                  </TableCell>
                </TableRow>
              )}
              {rows.map((r) => (
                <TableRow key={r.id}>
                  <TableCell className="tabular-nums">
                    {format(new Date(r.inicio), "dd/MM/yyyy HH:mm")}
                  </TableCell>
                  ...
                  <TableCell className="text-right tabular-nums">
                    {r.origem_valor != null ? `R$ ${Number(r.origem_valor).toFixed(2)}` : "—"}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
