...
function Page() {
  ...
  const load = async () => {
    ...
    const [r, c] = await Promise.all([
      supabase
        .from("fin_regras_ia")
        .select("id, nome, padrao_descricao, categoria_id, prioridade, ativo")
    ...
    if (r.error) mostrarErro(r.error);
    else setItems((r.data ?? []) as Regra[]);
  ...
  const openEdit = (r: Regra) => {
    ...
    setForm({
      nome: r.nome,
      padrao_descricao: r.padrao_descricao ?? "",
  ...
  const remove = async (r: Regra) => {
    ...
    if (!(await confirmDialog(`Excluir "${r.nome}"?`))) return;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Categorização automática de lançamentos por padrão
          </p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{editing ? "Editar" : "Nova"} regra</DialogTitle>
            </DialogHeader>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {r.padrao_descricao && (
                    <p className="text-xs text-muted-foreground">
                      Quando contém: "{r.padrao_descricao}"
                    </p>
                  )}
                </div>
