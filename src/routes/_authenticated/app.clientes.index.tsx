          ...
          <Button
            ...
            onClick={async () => {
              ...
              const { data, error } = await supabase
                .from("pacientes")
                .select(
                  "nome,cpf,telefone,email,data_nascimento,cidade,estado,bairro,logradouro,numero,cep,ativo",
                )
                .eq("clinica_id", clinicaAtual.clinica_id)
                .order("nome");
              if (error) {
            ...
            {loading ? (
              ...
              filtrados.map((p) => (
                <TableRow key={p.id}>
                  ...
                  <TableCell>
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full ${p.ativo ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300" : "bg-muted text-muted-foreground"}`}
                    >
