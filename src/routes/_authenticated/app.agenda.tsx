...
import {
  aplicarDesconto,
  aplicarDescontoPorForma,
  memoriaDescontoPorForma,
  hojeLocalISODate,
  primeiroValorValido,
...
import {
  ...
  type DividirItem,
} from "@/components/agenda/dividir-orcamento-dialog";
import {
  SelecionarItensOrcamentoDialog,
  type SelectItemOrc,
} from "@/components/agenda/selecionar-itens-orcamento-dialog";
 ...
 */
const agendaSnapshotCache = new Map<string, { items: Agendamento[]; ficha: Agendamento[] }>();
 ...
 * duplicado. Tira acento, caixa e o sufixo "(ESPECIALIDADE)" que a tela
 * acrescenta ao rótulo — assim "CONSULTA" e "CONSULTA (ORTOPEDIA)" contam
 * como o mesmo atendimento, que é como a recepção enxerga.
 */
const chaveProcedimento = (s: string | null | undefined) =>
  normalizar(s ?? "")
    .replace(/\s*\([^()]*\)\s*$/, "")
    .replace(/\s+/g, " ")
    .trim();
...
/**
 * Procedimento cobrado por unidade — "(CADA)", "(CADA LADO)". Repetir no mesmo
 * dia é o uso normal dele (infiltração por articulação, RX por lado), então
 * esses nunca acionam a trava de duplicado. Conferido na produção: só a
 * infiltração do Dr. Paulo Roberto responde por 75 dos casos de repetição em
 * 20 dias, todos legítimos.
 */
const procedimentoPorUnidade = (s: string | null | undefined) =>
  /\(cada\b/.test(normalizar(s ?? ""));
...
async function buscarProcedimentoPorNome(
  ...
  for (const tentativa of [nomeCompleto, nomeSemSufixo].filter(
 ...
 * legados da Agenda.
 */
function aplicarAcrescimoCartaoAgenda(valor: number, _forma: string, _acr: unknown): number {
  return valor;
}
...
/**
 * Calcula as opções de pagamento (dinheiro/Pix/débito/crédito) de uma
 * cobrança com MAIS DE UM procedimento (multi-exame laboratório/imagem),
 * aplicando a regra de convênio de CADA procedimento individualmente.
 *
 * Antes, a cobrança de multi-exame buscava o desconto UMA VEZ usando o nome
 * concatenado ("HEMOGRAMA + TOMOGRAFIA") — que não bate com nenhum
 * procedimento cadastrado, então a regra encontrada (quando encontrava
 * alguma) era aplicada por engano sobre a SOMA de todos os itens (ex.:
 * Hemograma tem 10% de desconto e Tomografia 5%, mas o sistema aplicava um
 * dos dois sobre o total dos dois). Esta função resolve o convênio por
 * item e soma os valores já com o desconto correto de cada um.
 *
 * Gratuidade não é aplicada automaticamente aqui (seria preciso perguntar
 * "usar agora/depois" por item) — mesma regra já adotada pela cobrança em
 * lote: o item de gratuidade sai pelo valor particular nesta cobrança.
 */
async function calcularOpcoesMultiExame(params: {
  clinicaId: string;
  pacienteId: string | null | undefined;
  ...
  let vDinheiro = 0,
    ...
    vDebito = 0,
    vCredito = 0;
  let algumDesconto = false;
  let convenioNome = "";
  const avisos = new Set<string>();
  let algumAtraso: { convenioNome: string; parcelasAtrasadas: number } | null = null;
  let algumBloqueio: string | null = null;
  infos.forEach((info, idx) => {
    const p = procs[idx] as any;
    const baseCartao = valorCartaoProcedimento(p);
    const baseDin = primeiroValorValido(p?.valor_dinheiro, p?.valor_dinheiro_pix, p?.valor_padrao);
    if (info) convenioNome = info.convenioNome;
    if (info && !info.emDia && !algumAtraso) {
      algumAtraso = { convenioNome: info.convenioNome, parcelasAtrasadas: info.parcelasAtrasadas };
    }
    if (info && info.bloquear && !algumBloqueio) {
      algumBloqueio = info.avisoLimite ?? "Limite do convênio atingido — agendamento bloqueado.";
    }
    const desc =
      info && info.emDia && !info.bloquear && info.desconto && info.desconto.tipo !== "gratuidade"
        ? info.desconto
        : null;
    const acr = acrescimoParaDesconto(desc, info?.acrescimoCartao);
    if (desc) algumDesconto = true;
    if (info?.avisoLimite) avisos.add(info.avisoLimite);
    vDinheiro += desc ? aplicarDescontoPorForma(baseDin, "dinheiro", desc) : baseDin;
    vPix += desc
      ? aplicarAcrescimoCartaoAgenda(aplicarDescontoPorForma(baseCartao, "pix", desc), "pix", acr)
      : baseCartao;
    vDebito += desc
      ? aplicarAcrescimoCartaoAgenda(
          aplicarDescontoPorForma(baseCartao, "cartao_debito", desc),
          "cartao_debito",
          acr,
        )
      : baseCartao;
    vCredito += desc
      ? aplicarAcrescimoCartaoAgenda(
          aplicarDescontoPorForma(baseCartao, "cartao_credito", desc),
          "cartao_credito",
          acr,
        )
      : baseCartao;
  ...
  let descSuffix = "";
  let avisoLimite: { tom: "warning" | "error"; mensagem: string } | undefined;
  if (algumAtraso) {
    const at = algumAtraso as { convenioNome: string; parcelasAtrasadas: number };
    descSuffix = ` — ${at.convenioNome} EM ATRASO`;
    avisoLimite = {
      tom: "error",
      mensagem: `Convênio ${at.convenioNome} em atraso (${at.parcelasAtrasadas} parcela(s)). Cobrando valor cheio.`,
    };
  } else if (algumBloqueio) {
    descSuffix = ` — ${convenioNome} BLOQUEADO`;
    avisoLimite = { tom: "error", mensagem: algumBloqueio };
  } else if (algumDesconto) {
    descSuffix = ` — Convênio ${convenioNome} (desconto aplicado por item)`;
    if (avisos.size > 0) avisoLimite = { tom: "warning", mensagem: Array.from(avisos).join(" ") };
  } else if (avisos.size > 0) {
    descSuffix = ` — ${convenioNome} (limite atingido)`;
    avisoLimite = { tom: "warning", mensagem: Array.from(avisos).join(" ") };
  }
  return { opcoes, descSuffix, avisoLimite };
}
...
function AgendaPage() {
  // Agenda "seca": enquanto esta tela está montada, nenhuma animação ou
  // transição roda. Ver o bloco `agenda-sem-animacao` em src/styles.css — a
  // classe precisa ir no <html> porque diálogos, selects e menus são
  // renderizados em portal, fora desta árvore. Sai no desmonte, então as
  // outras telas continuam iguais.
  useEffect(() => {
    if (typeof document === "undefined") return;
    document.documentElement.classList.add("agenda-sem-animacao");
    return () => {
      document.documentElement.classList.remove("agenda-sem-animacao");
    };
  }, []);
  ...
  const [procOpcoesPorMedico, setProcOpcoesPorMedico] = useState<
  ...
  const labEspecialidadeIds = useMemo(
    () =>
      new Set(
        especialidades.filter((e) => normalizar(e.nome ?? "").includes("laborat")).map((e) => e.id),
      ),
    [especialidades],
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      //    ("Particular") quando alguma parcela passou de 5 dias vencida.
      ...
      const diasAtrasoLista = lista.map((m) =>
        Math.max(
          0,
          Math.floor((hojeMs - new Date(String(m.vencimento) + "T00:00:00").getTime()) / 86400000),
        ),
      );
      const atrasadas = lista.filter((_, i) => diasAtrasoLista[i] > DIAS_TOLERANCIA);
      const emCarenciaLista = diasAtrasoLista.filter((d) => d >= 0 && d <= DIAS_TOLERANCIA);
      const totalAberto = atrasadas.reduce((s, m) => s + (Number(m.valor) || 0), 0);
      setContratoPacienteInfo({
        convenioNome: contrato.convenioNome,
        totalAberto,
        qtdAtrasadas: atrasadas.length,
        qtdEmCarencia: emCarenciaLista.length,
      ...
      // força "Particular" quando há parcela de fato bloqueada. Aplica
      // também na edição de agendamentos antigos gravados incorretamente
      // como "particular" mesmo com o cartão em dia.
      setForm((f) => ({
        ...f,
   ...
   *
   * A equipe do telefone marca o paciente com antecedência; quando ele chega à
   * clínica, a recepção costumava abrir uma ficha nova do zero, criando uma
   * segunda vaga para o mesmo paciente no mesmo dia e bagunçando a fila.
   *
   * Assim que o paciente é escolhido no modal, procuramos o que ele já tem
   * marcado naquela data (qualquer profissional — a recepção precisa ver o
   * horário e o médico para julgar se é a mesma consulta) e oferecemos abrir a
   * ficha original em vez de criar outra. Cancelado e falta ficam de fora:
   * nesses casos remarcar é o certo.
   */
  const [dupDoDia, setDupDoDia] = useState<Agendamento[]>([]);
  /** A recepção confirmou que é outro atendimento — o aviso para de aparecer. */
  const [dupDispensado, setDupDispensado] = useState(false);
  const dupReqId = useRef(0);
  const diaDoForm = form.inicio ? form.inicio.slice(0, 10) : "";
  useEffect(() => {
    setDupDispensado(false);
  }, [open, form.paciente_id, diaDoForm]);
  useEffect(() => {
    if (!open || !clinicaAtual || !form.paciente_id || !diaDoForm) {
      setDupDoDia([]);
      return;
    }
    const reqId = ++dupReqId.current;
    const pacId = form.paciente_id;
    const clinicaId = clinicaAtual.clinica_id;
    // A própria ficha que está aberta (edição ou slot livre reservado) nunca
    // pode se acusar de duplicada.
    const editandoId = editing?.id ?? null;
    (async () => {
      const { data, error } = await supabase
        ...
        .eq("clinica_id", clinicaId)
        .eq("paciente_id", pacId)
        .gte("inicio", new Date(`${diaDoForm}T00:00:00`).toISOString())
        .lte("inicio", new Date(`${diaDoForm}T23:59:59`).toISOString())
        .in("status", ["agendado", "confirmado", "realizado"])
        .order("inicio", { ascending: true })
        .limit(10);
      if (reqId !== dupReqId.current) return;
      if (error) {
        setDupDoDia([]);
        return;
      }
      setDupDoDia(
        mapAgendaRows((data ?? []) as unknown as AgendaRowBruta[]).filter(
          (a) => a.id !== editandoId && !isSlotLivre(a.paciente_nome),
        ),
      );
    })();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open, form.paciente_id, diaDoForm, editing?.id, clinicaAtual?.clinica_id]);

  /**
   * Duplicata PROVÁVEL: mesmo profissional e mesmo procedimento no mesmo dia.
   * É o caso que a trava precisa pegar — a telefonista marcou a consulta e a
   * recepção abriu outra igual quando o paciente chegou.
   *
   * A regra é estreita de propósito. Na produção, 1 em cada 4 pacientes-dia
   * tem mais de um agendamento no dia, e a maioria é legítima: consulta +
   * preventivo com a mesma ginecologista, consulta + infiltração com o
   * ortopedista, vários exames de imagem. Avisar em todos esses casos viraria
   * ruído e a recepção passaria a clicar em "continuar" no automático.
   */
  const dupProvavel = useMemo(() => {
    if (!form.medico_id) return [];
    if (procedimentoPorUnidade(form.procedimento)) return [];
    const chave = chaveProcedimento(form.procedimento);
    return dupDoDia.filter(
      (d) =>
        d.medico_id === form.medico_id &&
        !procedimentoPorUnidade(d.procedimento) &&
        chaveProcedimento(d.procedimento) === chave,
    );
  ...
  const iniciarReagendamento = (a: Agendamento) => {
    ...
    if (a.status === "realizado") {
      toast.error(
        "Atendimento já realizado — peça ao financeiro para estornar antes de reagendar.",
      );
      return;
    }
    setReagendandoAg(a);
  ...
  const aplicarDescontoPendente = (valor: number): number => {
    ...
    if (n <= 0) return valor;
    const d =
      descontoPendente.tipo === "percentual"
      ...
      if (faltando.length > 0 && clinicaAtual) {
        try {
          ...
          let q = supabase
            .from("pacientes")
            .select(
              "id,cpf,nome,data_nascimento,telefone,email,cep,logradouro,numero,bairro,cidade,estado,foto_url",
            )
            .eq("clinica_id", clinicaAtual.clinica_id)
            .neq("id", base.id);
  ...
  const [novoPac, setNovoPac] = useState({
    nome: "",
    cpf: "",
    telefone: "",
    data_nascimento: "",
  ...
  const cadastrarPacienteRapido = async (e: FormEvent) => {
    ...
    setPacientes((prev) =>
      [...prev, { id: data.id, nome: data.nome }].sort((a, b) => a.nome.localeCompare(b.nome)),
    );
  ...
  const load = async () => {
    ...
    const agendaRows = (mapped ?? []) as unknown as Array<
      Agendamento & { fluxo_etapa?: string | null }
    >;
    setEtapaMap(
      new Map(
        agendaRows.map((r) => [r.id, r.fluxo_etapa ?? "aguardando_recepcao"] as [string, string]),
      ),
    );
    // Busca dados auxiliares dos pacientes em paralelo para não atrasar a agenda.
    const pacIds = Array.from(
      new Set(
        agendaRows.map((a) => a.paciente_id as string | null).filter((x): x is string => !!x),
      ),
    );
    if (pacIds.length) {
      const [{ data: nasc }, { data: contratos }, { data: deps }] = await Promise.all([
        ...
        supabase
          .from("contrato_dependentes")
          .select(
            "paciente_id,ativo,contratos_assinatura!inner(clinica_id,status,cb_convenios(nome))",
          )
          .in("paciente_id", pacIds)
          .eq("ativo", true)
    ...
    const idsComPaciente = new Set(
      agendaRows.filter((a) => !isSlotLivre(a.paciente_nome)).map((a) => a.id),
    );
        ...
        for (let i = 0; i < idsParaPagamento.length; i += CHUNK) {
          ...
          (
            ...
            if (!nMap.has(r.agendamento_id)) {
              nMap.set(r.agendamento_id, {
                id: r.nfse.id,
                status: r.nfse.status,
                url_pdf: r.nfse.url_pdf,
                numero: r.nfse.numero,
              });
            }
          });
        }
  ...
  const loadRef = async () => {
    ...
    const medicosBase = ((m ?? []) as unknown as RawMedicoAgenda[]).map((x) => ({
      ...
      procedimento_padrao_nome: x.procedimento_padrao_id
        ...
        : null,
      especialidade_nome: x.especialidade_id
        ? (especialidadesPorId.get(x.especialidade_id) ?? null)
        : null,
      __recurso: false,
    }));
    ...
    setProcedimentosList(
      todos.map(({ id, nome, tipo_procedimento }) => ({
        id,
        nome,
        tipo_procedimento: tipo_procedimento ?? null,
      })),
    );
    ...
    for (const r of (sr.data ?? []) as Array<{
      medico_id: string | null;
      procedimento_id: string;
    }>) {
  ...
  const procedimentoEfetivo = (
    ...
    if (padrao && (!atual || normalizar(atual) === normalizar(med?.especialidade_nome ?? "")))
      return padrao;
    return atual;
  };

  const procedimentoFormulario = (
    medicoId: string | null | undefined,
    procedimento: string | null | undefined,
  ) => {
    ...
    if (
      ...
      normalizar(atual) === normalizar(med.especialidade_nome)
    ) {
      ...
      const opts = opcoesProcedimentoMedico(
        medicoId ?? null,
        editing?.agenda_id ??
          (filtroAgenda !== "todos" && !filtroAgenda.startsWith("nome:") ? filtroAgenda : null),
      );
  ...
  const procedimentoEhImagem = (label: string) => {
    ...
    if (tp === "laboratorio" || tp === "consulta" || tp === "cirurgia" || tp === "procedimento")
      return false;
  ...
  const opcoesServicoFormulario = () => {
    ...
    const especialidadeMedico =
      medicos.find((m) => m.id === form.medico_id)?.especialidade_nome ?? "";
    const atualEhEspecialidadeSintetica =
      especialidadeMedico && normalizar(atual) === normalizar(especialidadeMedico);
    if (
      atual &&
      !atualEhEspecialidadeSintetica &&
      !opts.some((o) => normalizar(o.value) === normalizar(atual))
    ) {
  ...
  const atualizarProcedimento = async (ag: Agendamento, novoNome: string) => {
    ...
    setItems((prev) =>
      prev.map((x) => (x.id === ag.id ? { ...x, procedimento: limpar ? null : nomeFinal } : x)),
    );
  ...
  const cobrarSelecionados = async () => {
    ...
    try {
      ...
      let algumDescontoConvenio = false;
      (procsResolvidos as any[]).forEach((p, idx) => {
        ...
        const info = infosConvenio[idx];
        // Só aplica o benefício quando o convênio está em dia, não bloqueado e
        // com desconto de %/valor fixo (gratuidade fica para a cobrança avulsa).
        const desc =
          info &&
          info.emDia &&
          !info.bloquear &&
          ...
          info.desconto.tipo !== "gratuidade"
            ...
            : null;
        const acr = acrescimoParaDesconto(desc, info?.acrescimoCartao);
        if (desc) algumDescontoConvenio = true;
        const valorDin = desc ? aplicarDescontoPorForma(baseDin, "dinheiro", desc) : baseDin;
        const valorPix = desc
          ? aplicarAcrescimoCartaoAgenda(
              aplicarDescontoPorForma(baseCartao, "pix", desc),
              "pix",
              acr,
            )
          : baseCartao;
        const valorDebito = desc
          ? aplicarAcrescimoCartaoAgenda(
              aplicarDescontoPorForma(baseCartao, "cartao_debito", desc),
              "cartao_debito",
              acr,
        ...
        // Peso p/ rateio: prioriza valor de cartão (já com desconto); se 0, usa dinheiro.
        pesos[itens[idx].id] = valorCredito > 0 ? valorCredito : valorDin;
        rotulos[itens[idx].id] =
          itens[idx].procedimento ?? rotuloFallbackProc(itens[idx].medico_id);
      });
  ...
  const baixarLoteRealizado = async () => {
    ...
    const roleOk =
      usuarioEhMedico ||
      ["admin", "gestor", "financeiro", "recepcao"].includes(
  ...
  const reabrirAtendimento = async (a: Agendamento) => {
    ...
    if (!clinicaAtual) return;
    const roleOk = ["admin", "gestor", "financeiro"].includes(
      (clinicaAtual?.role ?? "").toLowerCase(),
    );
    if (!roleOk) {
    ...
    try {
      ...
      if (eAg) throw eAg;
      toast.success(
        temRepassePago ? "Atendimento reaberto e repasse estornado." : "Atendimento reaberto.",
      );
      await load();
    } catch (err: any) {
      mostrarErro(err);
  ...
  const abrirReagLote = () => {
    ...
    if (atendidos.length > 0) {
      toast.error(
        `${atendidos.length} paciente(s) já atendido(s) não podem ser reagendados. Desmarque-os.`,
      );
      return;
    }
    ...
    if (validos.length === 0) {
      toast.info(
        "Nenhum paciente válido para reagendar (todas as fichas selecionadas estão vazias).",
      );
      return;
    }
  ...
  const confirmarReagLoteNoSlot = async (slot: Agendamento) => {
    ...
    const { data: fontesRaw, error: eFontes } = await supabase
      .from("agendamentos")
      .select(
        "id,paciente_id,paciente_nome,inicio,fim,medico_id,status,procedimento,observacoes,data_pagamento",
      )
      .in("id", ids)
      .limit(1000);
    if (eFontes) {
    ...
    setSelecionados(new Set());
    toast.success(
      `${okCount} paciente(s) reagendado(s) a partir da ficha ${String(fichaInicial).padStart(3, "0")}.`,
    );
    await load();
  };
  ...
  const copiarPacienteSelecionado = () => {
    ...
    setSelecionados(new Set());
    toast.success(
      `Paciente copiado: ${ag.paciente_nome}. Clique em um horário livre para agendar.`,
    );
  };
  ...
  const buscarOrcamento = async (numeroOverride?: number) => {
    ...
    try {
      ...
      for (const cand of candidatos) {
        ...
        if (data) {
          orc = data as unknown as OrcBusca;
          break;
        }
      }
      if (!orc) {
      ...
      const { data: itens, error: e2 } = await supabase
        ...
        .select(
          "id, descricao, procedimento_id, valor_total, dentes, status_financeiro, sinal_valor, valor_pago",
        )
      ...
      const editingItemIdsLiberar = editing?.id
        ? new Set<string>(
            linkRows.filter((r) => r.agendamento_id === editing.id).map((r) => r.orcamento_item_id),
          )
        : new Set<string>();
      ...
      if (totalConsumidos > 0) {
        toast.info(
          `${totalConsumidos} de ${itsAll.length} itens já pagos. Restam ${its.length} para agendar.`,
        );
      }
      ...
      if (!todosLab && gruposDistintos.size > 1) {
        ...
        const inicioPadrao =
          form.inicio || toLocalInput(new Date(`${dataRef}T09:00:00`).toISOString());
        setDividirCtx({
          orcamento: {
            id: orc.id,
            numero: orc.numero,
  ...
  const liberarLockSlot = async () => {
    ...
    try {
      ...
      /* silencioso — a trava expira em 3 min de qualquer forma */
    }
  };
  ...
  const openSlot = async (a: Agendamento) => {
    ...
    if (isSlotLivre(a.paciente_nome)) {
      ...
      if (!res.ok) {
        if (res.reason === "locked") {
          toast.error(
            `Sendo agendado por ${res.by_nome ?? "outro usuário"} — tente novamente em instantes.`,
          );
        } else {
    ...
    setForm({
      ...
      forma_pagamento_prevista:
        (a as { forma_pagamento_prevista?: string | null }).forma_pagamento_prevista ?? "",
    });
  ...
  const openEdit = async (a: Agendamento) => {
    ...
    if (!resolvedPacId && a.paciente_nome && clinicaAtual) {
      ...
      const exato =
        lista.find((p) => norm(p.nome) === alvo) ?? (lista.length === 1 ? lista[0] : null);
      if (exato) {
  ...
  const submit = async (e: FormEvent, irParaPagamento = false, imprimirComprovanteApos = false) => {
    ...
    // Trava do agendamento duplicado: enquanto o aviso estiver na tela sem
    // resposta, salvar criaria a segunda vaga do mesmo paciente no dia. A
    // recepção precisa abrir a ficha existente ou dizer que é outro
    // atendimento.
    if (!dupDispensado && dupProvavel.length > 0) {
      toast.error(
        "Este paciente já tem este mesmo atendimento marcado no dia. Abra o agendamento existente ou confirme que é outro atendimento.",
      );
      return;
    }
    ...
    if (
      form.orcamento_id &&
      orcamentoLaboratorio &&
      form.medico_id &&
      !medicoEhLaboratorista(form.medico_id)
    ) {
      toast.error(
        "Orçamentos de Laboratório só podem ser agendados com médicos da especialidade Laboratório.",
      );
      return;
    }
    ...
    if (
      ...
      form.paciente_nome.trim() !== editing.paciente_nome
    ) {
    ...
    if (pendingOrcItemIds.length > 0) {
      ...
      const candidatos = ((dupLinks ?? []) as DupRow[])
        ...
        .filter(
          (r) =>
            r.agendamentos && r.agendamentos.status !== "cancelado" && !!r.agendamentos.paciente_id,
        )
      ...
      if (candidatos.length > 0) {
        ...
        if (naoPagos.length > 0) {
          ...
          const msg = `Este paciente já está agendado para o(s) mesmo(s) serviço(s) e ainda não pagou:\n\n${linhas.join("\n")}\n\nDeseja criar um novo agendamento mesmo assim?`;
      ...
      if (isMulti) {
        if (opcoesOrc) {
          ...
          const resultado = await calcularOpcoesMultiExame({
            ...
            pacienteId: payload.paciente_id,
            medicoId: payload.medico_id,
            tipoAtendimento: payload.tipo_atendimento,
            dataRef: payload.inicio ?? null,
            nomes: nomesParaValorar,
            agendamentoIds: [novoId, ...(result.sibling_ids ?? [])],
            procs: procsIndividuais,
          });
          opcoes = resultado.opcoes;
          descSuffix = resultado.descSuffix;
          if (resultado.avisoLimite) setAvisoConvenio(resultado.avisoLimite);
        }
      } else {
        let vDinheiro = 0,
          vPix = 0,
  ...
  const remove = async (a: Agendamento) => {
    ...
    if (
      !(await confirmDialog(
        `Liberar este horário? O cliente ${a.paciente_nome} será removido, mas a ficha continuará disponível.`,
      ))
    )
      return;
  ...
  const mudarStatus = async (a: Agendamento, status: Status) => {
    ...
    if (status === "cancelado" && a.pacote_id) {
      ...
      const outros = (
        (irmaos ?? []) as Array<{ id: string; inicio: string; procedimento: string | null }>
      ).filter((x) => x.id !== a.id);
      if (outros.length > 0) {
        const lista = outros
  ...
  const alternarAutorizacaoConvenio = async (a: Agendamento) => {
    ...
    if (error) mostrarErro(error);
      ...
      toast.success(
        novo ? "Autorização do convênio confirmada." : "Autorização do convênio removida.",
      );
      await load();
    }
  };
  ...
  const iniciarAtendimentoEnf = async (a: Agendamento) => {
    ...
    if (!(await confirmDialog(`Dar baixa como Realizado no atendimento de ${a.paciente_nome}?`)))
      return;
    const uid = (await supabase.auth.getUser()).data.user?.id;
    if (!uid) {
    ...
    if (agendamentoId) {
      ...
      if (ids.length) {
        const { data: itens } = await supabase
          .from("orcamento_itens")
          .select(
            "id, descricao, valor_total, quantidade, valor_unitario, valor_pago, valores_formas",
          )
          .in("id", ids);
  ...
  const aplicarEtapaSinal = async (
    ...
    return {
      opcoes: opcoes.map((o) => ({
        ...o,
        valor: (etapaPorForma.get(o.forma) ?? etapaSinal).valor,
      })),
      descSuffix: ` — ${rotulo}`,
    };
  };
  ...
  const cobrarAgendamento = async (a: Agendamento) => {
    ...
    try {
      ...
      const nomesParaValorar = (a.procedimento ?? rotuloFallbackProc(a.medico_id))
        ...
        .filter(Boolean);
      const nomesEfetivos =
        nomesParaValorar.length > 0 ? nomesParaValorar : [rotuloFallbackProc(a.medico_id)];
      const isMulti = nomesEfetivos.length > 1;
      const [{ data: jaPagos }, lista, infoInicial] = await Promise.all([
        supabase
          .from("fin_lancamentos")
          .select("id")
          .eq("clinica_id", clinicaAtual.clinica_id)
      ...
      if (!opcoesOrc && totalOpcoes <= 0 && (algumProcCasou || ehGratuidadeConvenio) && !ehLab) {
        ...
        if (errRpc) {
          mostrarErro(
            errRpc,
            isGrat
              ? "falha ao registrar gratuidade"
              : "falha ao registrar atendimento sem cobrança",
          );
          return;
        }
        setPagosSet((prev) => {
  ...
  const estornarCheckin = async (a: Agendamento) => {
    ...
    if (
      !(await confirmDialog(
        "Desfazer check-in deste paciente? Ele voltará para 'aguardando recepção'.",
      ))
    )
      return;
    const { error } = await supabase
  ...
  const verOuEmitirNota = async (a: Agendamento) => {
    ...
    if (nfseSel.size > 1 && nfseSel.has(a.id)) {
      toast.info(
        'Use o botão "Emitir NFS-e agrupada" na barra inferior para os itens selecionados.',
      );
      return;
    }
    if (ex) {
      if (ex.url_pdf) {
        ...
        toast.info(
          `NFS-e ${ex.numero ?? ""} — status: ${ex.status ?? "—"}. PDF ainda não disponível.`,
        );
  ...
  const emitirNfseAgrupada = async () => {
    ...
    if (ids.length < 2) {
      toast.error("Selecione ao menos 2 agendamentos.");
      return;
    }
    const selAgs = items.filter((a) => ids.includes(a.id));
    if (selAgs.length !== ids.length) {
      toast.error("Alguns agendamentos não foram encontrados na lista.");
      return;
    }
    // Todos precisam ser do mesmo paciente
    const pacIds = new Set(selAgs.map((a) => a.paciente_id).filter(Boolean) as string[]);
    if (pacIds.size !== 1) {
      toast.error("Selecione serviços de um único paciente.");
      return;
    }
    ...
    for (const a of selAgs) {
      if (!pagosSet.has(a.id)) {
        toast.error(`Agendamento das ${fmtHora(a.inicio)} ainda não foi pago.`);
        return;
      }
      if (nfseMap.has(a.id)) {
        toast.error(`Agendamento das ${fmtHora(a.inicio)} já possui NFS-e.`);
        return;
      }
    }
    try {
  ...
  const medicoNomeAgendamento = (a: Agendamento) => {
    ...
    const isExame = (n?: string | null) =>
      NOMES_EXAME_SEM_PREFIXO.has((n ?? "").trim().toUpperCase());
    if (m) {
    ...
    >
      ...
      <div className="flex shrink-0 items-end gap-1.5">
        ...
        <button
          ...
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-100 hover:text-slate-800"
        >
          <X className="h-4 w-4" />
        </button>
      </div>
    ...
    <div className="space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-3 text-sm">
            ...
            <div className="flex-1 min-w-0">
              <span className="font-semibold uppercase">
                Reagendando · {reagendandoAg.paciente_nome}
              </span>
              ...
              <span className="ml-2 opacity-90 italic">
                Clique em um horário disponível na agenda para confirmar.
              </span>
            </div>
            <Button
              size="sm"
        ...
        <div className="sticky top-0 z-30 -mx-4 px-4 py-2 border-b bg-primary text-primary-foreground shadow-sm">
          <div className="flex flex-wrap items-center gap-3 text-sm">
            ...
            <div className="flex-1 min-w-0">
              ...
              <span className="ml-2 opacity-90 italic">
                Clique em um horário DISPONÍVEL na agenda. Os pacientes serão alocados em sequência
                a partir dessa ficha.
              </span>
            </div>
            <Button
              size="sm"
        ...
        <div className="col-span-2 flex flex-wrap items-center gap-2 sm:col-span-1">
          ...
          <div className="hidden lg:inline-flex items-center gap-1">
            ...
            <button
              ...
              aria-label="Receber mensalidade do Cartão Benefícios"
                ...
                <div className="flex-1 min-h-0 overflow-y-auto scroll-hidden space-y-1.5 px-4 sm:px-6 py-2">
                  ...
                  {editing && pagosSet.has(editing.id) && (
                    <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50/70 text-amber-900 px-3 py-2 text-xs">
                      ...
                      Este agendamento já foi pago. Para alterações, estorne o pagamento no
                      Financeiro.
                    </div>
                  )}
                  <fieldset
                      ...
                      <div className="space-y-1 rounded-xl border border-primary/25 bg-primary/[0.04] p-2 text-xs">
                        ...
                        <div className="text-xs text-slate-600 space-y-1 pt-1 border-t border-primary/15">
                          <p className="font-medium text-slate-900">
                            ...
                            Pagamento continua pelo orçamento.
                          </p>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="flex gap-2">
                        <div className="flex-1">
                          <PatientSearchInput
                            clinicaIdsOverride={
                              clinicaAtual ? [clinicaAtual.clinica_id] : undefined
                            }
                            value={
                              form.paciente_id
                                ? {
                      ...
                      {form.paciente_nome && !form.paciente_id && (
                        <p className="text-xs text-amber-600 font-medium">
                          Paciente não cadastrado — use o botão ao lado para cadastrar antes de
                          salvar.
                        </p>
                      )}
                      {/* Aviso de agendamento duplicado: o paciente já tem ficha
                          nesta data. O caminho principal é ABRIR a ficha que já
                          existe; criar outra continua possível, mas exige que a
                          recepção diga que é outro atendimento. */}
                      {!dupDispensado && dupProvavel.length > 0 && (
                        <div className="space-y-2 rounded-xl border-2 border-amber-400 bg-amber-50 p-3">
                          <p className="text-sm font-bold text-amber-900">
                            ⚠️ Este paciente já possui agendamento para{" "}
                            {diaDoForm === hojeBR()
                              ? "hoje"
                              : new Intl.DateTimeFormat("pt-BR", {
                                  ...
                                  day: "2-digit",
                                  month: "2-digit",
                                }).format(new Date(`${diaDoForm}T00:00:00Z`))}
                            .
                          </p>
                          <ul className="space-y-1.5">
                            {dupProvavel.map((d) => {
                              const espec =
                                medicos.find((m) => m.id === d.medico_id)?.especialidade_nome ?? "";
                              return (
                                <li
                                  key={d.id}
                                ...
                                >
                                  <span className="min-w-0 text-xs text-amber-900">
                                    ...
                                    <strong>{d.medico_nome || "profissional não informado"}</strong>
                                    {espec ? ` — ${espec}` : ""}
                          ...
                          <div className="flex flex-wrap items-center justify-between gap-2">
                            <p className="text-[11px] font-medium text-amber-800">
                              Abrir o que já existe evita criar uma segunda vaga para o mesmo
                              paciente.
                            </p>
                            <Button
                              type="button"
                      ...
                      {/* Demais atendimentos do paciente no mesmo dia. Só
                          informa (não trava): consulta + preventivo, consulta +
                          exame e vários exames de imagem no mesmo dia são
                          rotina. Serve para a recepção enxergar o que a
                          telefonista já marcou antes de abrir mais uma ficha. */}
                      {dupOutros.length > 0 && (
                        <div className="rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-2">
                          <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">
                            Este paciente já tem no dia
                          </p>
                            ...
                            {dupOutros.map((d) => (
                              ...
                              >
                                <span className="min-w-0 text-xs text-slate-700">
                                  <strong>{fmtHora(d.inicio)}</strong> —{" "}
                                  {d.medico_nome || "profissional não informado"}
                                  {d.procedimento ? ` · ${d.procedimento}` : ""}
                                </span>
                                <button
                                  type="button"
                    ...
                    {contratoPacienteInfo && (
                      <div className="space-y-1.5">
                        ...
                        <Select
                          ...
                          onValueChange={(v) =>
                            setForm((f) => ({ ...f, tipo_atendimento: v as TipoAtendimento }))
                          }
                        >
                          <SelectTrigger>
                            <SelectValue />
                        ...
                        {contratoPacienteInfo.qtdAtrasadas > 0 &&
                          form.tipo_atendimento === "particular" && (
                            <p className="text-xs rounded-md border border-amber-300 bg-amber-50 text-amber-900 px-2 py-1.5">
                              Paciente tem <b>R$ {contratoPacienteInfo.totalAberto.toFixed(2)}</b>{" "}
                              em aberto no cartão ({contratoPacienteInfo.qtdAtrasadas} parcela(s)
                              vencida(s) há mais de 5 dias). Este atendimento será cobrado como
                              Particular.
                            </p>
                          )}
                        {contratoPacienteInfo.qtdAtrasadas > 0 &&
                          form.tipo_atendimento === "convenio" && (
                            <p className="text-xs rounded-md border border-destructive/40 bg-destructive/5 text-destructive px-2 py-1.5">
                              Convênio bloqueado: paciente tem{" "}
                              <b>R$ {contratoPacienteInfo.totalAberto.toFixed(2)}</b> em atraso há
                              mais de 5 dias. Para agendar, mude para <b>Particular</b> ou
                              regularize o débito.
                            </p>
                          )}
                        {contratoPacienteInfo.qtdAtrasadas === 0 &&
                          contratoPacienteInfo.qtdEmCarencia > 0 && (
                            <p className="text-xs rounded-md border border-amber-300 bg-amber-50 text-amber-900 px-2 py-1.5">
                              Mensalidade vencida, mas dentro da tolerância de 5 dias — convênio
                              segue liberado normalmente. Regularize em até{" "}
                              <b>{contratoPacienteInfo.diasCarenciaRestantes}</b> dia(s) para evitar
                              bloqueio.
                            </p>
                          )}
                      </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Select
                        ...
                        onValueChange={(v) =>
                          setForm((f) => ({
                            ...f,
                            forma_pagamento_prevista: v === "nao_informado" ? "" : v,
                          }))
                        }
                      >
                        <SelectTrigger>
                        ...
                        <SelectContent>
                          <SelectItem value="nao_informado">
                            Não informada (definir na cobrança)
                          </SelectItem>
                            ...
                            setForm((f) => {
                              ...
                              if (medico_id) {
                                ...
                                const deveAplicarPadrao =
                                  !procedimento ||
                                  normalizar(procedimento) ===
                                    normalizar(med?.especialidade_nome ?? "");
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <Input
                            ...
                            value={
                              form.data_pagamento
                                ? new Date(form.data_pagamento + "T00:00:00").toLocaleDateString(
                                    "pt-BR",
                                  )
                                : "—"
                            }
                    ...
                    <div className="space-y-1.5">
                      ...
                      {form.medico_id ? (
                        ...
                        procNomesPorMedico.get(form.medico_id)?.size ? (
                          ...
                          <p className="text-xs text-amber-600">
                            Este médico não possui serviços cadastrados. Configure-os no cadastro do
                            médico.
                          </p>
                        )
                      ) : (
                            ...
                            <p className="hidden sm:block text-[12px] text-slate-500">
                              ...
                              {form.procedimentos.length > 1 && (
                                <span className="text-emerald-700">
                                  {" "}
                                  {form.procedimentos.length} selecionados.
                                </span>
                              )}
                            </p>
                          </>
          ...
          const executarPagamento = async () => {
            ...
            if (pagamentoExtraIds.length > 0) {
              ...
              if (errPrincipal) {
                mostrarErro(
                  errPrincipal,
                  "pagamento salvo, mas falhou ao localizar o lançamento principal",
                );
              }
              ...
              const obsOriginal =
                (principalRow as { observacoes?: string | null } | null)?.observacoes ?? "";
              ...
              const { error: errRateio } = await supabase.rpc("finalizar_pagamento_agrupado", {
                ...
                _criado_por:
                  (principalRow as { criado_por?: string | null } | null)?.criado_por ??
                  currentUserId,
                _itens: itensRateio,
              } as never);
              if (errRateio) {
      ...
      >
        ...
        >
          <DialogHeader>
            <DialogTitle
              className={avisoConvenio?.tom === "error" ? "text-destructive" : "text-amber-600"}
            >
              Aviso do convênio
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-1">
                ...
                <Select
                  value={descForm.tipo}
                  onValueChange={(v) =>
                    setDescForm((f) => ({ ...f, tipo: v as "valor" | "percentual" }))
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setDescForm((f) => ({ ...f, input: e.target.value.replace(/[^\d.,]/g, "") }))
                  }
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                placeholder={
                  ehSupervisorDesc ? "Você (supervisor)" : "Será preenchido após autorização"
                }
              />
            </div>
            <div className="space-y-1">
            ...
            {!ehSupervisorDesc && (
              <p className="text-xs text-amber-700 dark:text-amber-400">
                Exige autorização do supervisor (admin, gestor ou financeiro). Ao confirmar, será
                solicitada a senha.
              </p>
            )}
          </div>
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => {
                ...
                if (ehSupervisorDesc) {
                  const autor =
                    descForm.autorizadoPor.trim() || (clinicaAtual?.role ?? "supervisor");
                  setDescontoPendente({
                    tipo: descForm.tipo,
      ...
      */}
          ...
          <form onSubmit={cadastrarPacienteRapido} className="space-y-3">
            ...
            <div className="flex items-center gap-2 pt-1">
              ...
              >
                {descritorFace ? (
                  <>
                    <CheckCircle2 className="h-4 w-4 mr-1 text-green-600" /> Foto capturada —
                    refazer
                  </>
                ) : (
                  <>
                ...
                for (const r of auditRows) {
                  ...
                  if (pacienteMudou && !antesLivre && depoisLivre) {
                    items.push({
                      ...
                      body: (
                        <>
                          Liberou o horário (paciente removido:{" "}
                          <b>{String(antes.paciente_nome ?? "—")}</b>).
                        </>
                      ),
                    });
                    continue;
      ...
      >
        ...
        <Sheet open={filtrosMobileOpen} onOpenChange={setFiltrosMobileOpen}>
          <SheetContent
            side="bottom"
            className="lg:hidden max-h-[85vh] overflow-y-auto rounded-t-2xl"
          >
        ...
        {/* O seletor "Lista / Por médico" troca o conteúdo desta área. Antes a
            lista corrida ficava sempre montada e a grade por profissional era
            desenhada depois dela, então as duas opções pareciam iguais. */}
        {viewMode === "dia" && (
          <>
      ...
      {fotoPreviewOpen && pacInfoFoto && (
        ...
        >
          <button
            ...
            className="absolute right-5 top-5 rounded-full p-2 text-white/90 hover:bg-white/10 hover:text-white pointer-events-auto"
          >
            <X className="h-6 w-6" />
          </button>
          <img
          ...
          <p className="text-center text-sm text-white/80">
            Visualização Ampliada da Foto do Paciente
          </p>
          <Button
            onClick={(e) => {
              e.stopPropagation();
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`relative h-14 w-14 shrink-0 overflow-hidden rounded-full border bg-muted ${
                    pacInfoFoto ? "cursor-zoom-in hover:ring-2 hover:ring-primary/40" : ""
                  }`}
                >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 rounded-lg border bg-muted/40 p-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-sm font-medium break-words">
                    {new Date(obsAg.inicio).toLocaleString("pt-BR", {
                      dateStyle: "short",
                      timeStyle: "short",
                    })}
                  </p>
                  ...
                  {intervaloDias.map((dia) => {
                    ...
                    return (
                      <FragmentDayCell
                        ...
                        ocultarPaciente={
                          !!(ag && estornoPendAgs.has(ag.id) && ocultarPacienteMedico)
                        }
                        procedimentoFallback={
                          ag?.procedimento ??
                          (ehLaboratorio?.(ag?.medico_id) ? "EXAMES LABORATORIAIS" : "CONSULTA")
                        }
                      />
                    );
                  })}
...
function FragmentDayHeader({
  dia,
  fmtCabecalho,
}: {
  dia: string;
  fmtCabecalho: (d: string) => string;
}) {
  return (
    <>
      <th
    ...
    <>
      ...
      <td className="px-1 py-1 border-r align-middle" style={{ minWidth: 180 }}>
        {!ag ? (
          <button
            type="button"
            className="w-full h-8 rounded-md text-xs text-muted-foreground/60 hover:bg-emerald-50 hover:text-emerald-700"
          ...
          <button
            type="button"
            onClick={() =>
              ehLivre ? onSlotClick(ag) : ocultarPaciente ? undefined : onAgClick(ag)
            }
            disabled={ocultarPaciente}
            className={`w-full text-left rounded-md px-2 py-1.5 text-xs leading-tight truncate hover:brightness-95 ${
              estornoPend
