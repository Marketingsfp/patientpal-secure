...
function Page() {
  ...
  const openEdit = (c: Categoria) => { setEditing(c); setForm({ nome: c.nome, tipo: c.tipo, cor: c.cor }); setOpen(true); };
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <Card><CardContent className="py-12 text-center text-muted-foreground">Nenhuma categoria.</CardContent></Card>
      ) : (
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          {items.map((c) => (
            <Card key={c.id}>
              <CardContent className="pt-6 flex items-center gap-3">
                <div className="h-10 w-10 rounded-lg flex items-center justify-center shrink-0" style={{ backgroundColor: c.cor + "22", color: c.cor }}>
