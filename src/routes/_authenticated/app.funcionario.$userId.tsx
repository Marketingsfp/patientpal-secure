  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [{ data: prof }, { data: mems }, { data: ct }, { data: pt }] = await Promise.all([
        supabase
          .from("profiles")
          .select("id,nome,telefone,telefone2,avatar_url")
          .eq("id", userId)
          .maybeSingle(),
        supabase
          .from("clinica_memberships")
          .select("clinica_id,role,ativo,clinica:clinicas(nome)")
          .eq("user_id", userId),
        supabase
          .from("hr_contratos")
          .select(
            "id,numero,regime,data_inicio,data_fim,status,salario,cargo:cargos(nome),setor:setores(nome)",
          )
          .eq("user_id", userId)
          .order("data_inicio", { ascending: false }),
        supabase
          .from("hr_pontos")
          .select("id,tipo,registrado_em,unidade:unidades(nome)")
          .eq("user_id", userId)
      ...
      if (contratoIds.length > 0) {
        const [{ data: fr }, { data: hl }] = await Promise.all([
          supabase
            .from("hr_ferias")
            .select("id,inicio,fim,status,periodo_aquisitivo_inicio,periodo_aquisitivo_fim")
    ...
    return (
      <div className="p-6 text-center space-y-4">
        <p className="text-muted-foreground">
          Funcionário não encontrado ou sem permissão de visualização.
        </p>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                {contratoAtivo?.cargo?.nome && (
                  <Badge variant="secondary">{contratoAtivo.cargo.nome}</Badge>
                )}
              </div>
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                {memberships.map((m) => (
                  <Badge key={m.clinica_id} variant="outline">
                    {m.clinica?.nome ?? "?"} • {m.role}
                  </Badge>
                ))}
              </div>
            </div>
      ...
      <Tabs defaultValue="contratos">
              ...
              <div className="grid sm:grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <div className="font-medium">
                    {email ?? <span className="text-muted-foreground">Sem login cadastrado</span>}
                  </div>
                </div>
                <div>
                  <div className="text-muted-foreground text-xs uppercase tracking-wide">
              ...
              {pontos.length === 0 ? (
                ...
                <Table>
                  ...
                  <TableBody>
                    {pontos.map((p) => (
                      <TableRow key={p.id}>
                        <TableCell>{formatDateTime(p.registrado_em)}</TableCell>
                        <TableCell>
                          <Badge variant="outline">{p.tipo}</Badge>
                        </TableCell>
                        <TableCell>{p.unidade?.nome ?? "—"}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
        ...
        <TabsContent value="ferias">
          <Card>
            <CardContent className="pt-6">
              {ferias.length === 0 ? (
                <p className="text-sm text-muted-foreground">Nenhum período de férias.</p>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableRow key={f.id}>
                        <TableCell>
                          {f.periodo_aquisitivo_inicio
                            ? `${formatDatePura(f.periodo_aquisitivo_inicio)} – ${formatDatePura(f.periodo_aquisitivo_fim)}`
                            : "—"}
                        </TableCell>
                  ...
                  <TableBody>
                    {holerites.map((h) => (
                      <TableRow key={h.id}>
                        ...
                        <TableCell>{fmtMoeda(h.liquido)}</TableCell>
                        <TableCell>
