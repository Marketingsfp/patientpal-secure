...
function EditarFuncionarioPage() {
  ...
  const [loginsDisponiveis, setLoginsDisponiveis] = useState<
    Array<{ user_id: string; nome: string; email: string | null }>
  >([]);
      ...
      if (isNovo) {
        ...
        if (c) {
          setForm({
            ...
            paciente_id:
              ((c as { paciente_id?: string | null }).paciente_id as string | null) ?? "",
          ...
          if (uid) {
            ...
            const [mem, emailRes] = await Promise.all([
              supabase
                ...
                .eq("user_id", uid)
                .maybeSingle(),
              getLoginFn({ data: { clinicaId: clinicaAtual.clinica_id, userId: uid } }).catch(
            ...
            if (livresIds.length) {
              ...
              const emails = await Promise.all(
                livresIds.map(async (uid) => {
                  try {
                    const r = await getLoginFn({
                      data: { clinicaId: clinicaAtual.clinica_id, userId: uid },
                    });
  ...
  async function salvar() {
    ...
    if (isNovo && !form.paciente_id) {
      toast.error("Selecione o cliente vinculado ao funcionário");
      return;
    }
  ...
  async function salvarNovaSenha() {
    ...
    try {
      await definirSenhaFn({
        data: { clinicaId: form.clinica_id, userId: linkedUserId, novaSenha },
      });
  ...
  async function vincularLoginExistente() {
    ...
    const [mem, emailRes] = await Promise.all([
      supabase
        ...
        .select("id, role, ativo")
        .eq("clinica_id", clinicaAtual.clinica_id)
        .eq("user_id", vincularUserId)
        .maybeSingle(),
      getLoginFn({ data: { clinicaId: clinicaAtual.clinica_id, userId: vincularUserId } }).catch(
        () => ({ email: null as string | null }),
      ),
    ]);
    setMembershipId((mem.data?.id as string | undefined) ?? null);
    ...
    <div className="p-6 space-y-4 max-w-4xl mx-auto">
      ...
      <div>
        <h1 className="text-xl font-bold">
          {isNovo
            ? "Novo funcionário"
            : `Editar funcionário${form.funcionario_nome ? ` — ${form.funcionario_nome}` : ""}`}
        </h1>
                ...
                linkedUserId ? (
                  <div className="space-y-5">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <Select
                          value={membershipAtivo ? "ativo" : "inativo"}
                          onValueChange={(v) => setMembershipAtivo(v === "ativo")}
                        >
                          <SelectTrigger>
                            <SelectValue />
                          </SelectTrigger>
                    ...
                    {podeEscrever && (
                      <div>
                        <Button
                          ...
                          disabled={savingAcesso || !membershipId}
                        >
                  ...
                  <div className="space-y-4">
                    <p className="text-sm text-muted-foreground">
                      Este funcionário ainda não tem login de acesso. Você pode vincular um login já
                      existente na clínica ou criar um novo abaixo.
                    </p>
                        ...
                        <div className="flex gap-2">
                          <Select value={vincularUserId} onValueChange={setVincularUserId}>
                            <SelectTrigger className="max-w-md">
                              <SelectValue placeholder="Selecione um login sem funcionário" />
                            </SelectTrigger>
                            <SelectContent>
                          ...
                          {podeEscrever && (
                            <Button
                              size="sm"
                              onClick={vincularLoginExistente}
                              disabled={!vincularUserId || savingAcesso}
                            >
                    ...
                    <div className="border-t pt-4 space-y-3">
                      ...
                      <p className="text-xs text-muted-foreground">
                        O novo login será criado e vinculado a este contrato ao clicar em "Salvar"
                        no rodapé.
                      </p>
                    </div>
                  </div>
