...
function HoleritesPage() {
  ...
  async function load() {
    ...
    const [h, c] = await Promise.all([
      ...
      supabase
        .from("hr_contratos")
        .select("id,funcionario_nome,salario")
        .eq("clinica_id", clinicaAtual.clinica_id)
        .eq("status", "ativo")
              ...
              rows.map((r) => (
                <TableRow key={r.id}>
                  <TableCell className="font-medium">
                    {contratos.find((c) => c.id === r.contrato_id)?.funcionario_nome ?? "—"}
                  </TableCell>
                  <TableCell>{formatDatePura(r.competencia).slice(3)}</TableCell>
                  <TableCell className="text-right">
                    {Number(r.total_proventos).toLocaleString("pt-BR", {
                      style: "currency",
                      currency: "BRL",
                    })}
                  </TableCell>
                  <TableCell className="text-right text-destructive">
                    -{" "}
                    {Number(r.total_descontos).toLocaleString("pt-BR", {
                      style: "currency",
                      currency: "BRL",
                    })}
                  </TableCell>
                  <TableCell className="text-right font-semibold">
                    {Number(r.liquido).toLocaleString("pt-BR", {
                      style: "currency",
                      currency: "BRL",
                    })}
                  </TableCell>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <Button
                  size="sm"
                  variant="ghost"
                  onClick={() => setProventos([...proventos, { descricao: "", valor: 0 }])}
                >
              ...
              {proventos.map((p, i) => (
                <div key={i} className="grid grid-cols-[1fr_140px_auto] gap-2 mb-1">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      const x = [...proventos];
                      x[i].valor = Number(e.target.value);
                      setProventos(x);
                    }}
                  />
                  <Button
                    size="icon"
                    variant="ghost"
                    onClick={() => setProventos(proventos.filter((_, j) => j !== i))}
                  >
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <Button
                  size="sm"
                  variant="ghost"
                  onClick={() => setDescontos([...descontos, { descricao: "", valor: 0 }])}
                >
                  <Plus className="h-3 w-3" />
                </Button>
              </div>
              {descontos.map((p, i) => (
                <div key={i} className="grid grid-cols-[1fr_140px_auto] gap-2 mb-1">
                  ...
                  <Button
                    size="icon"
                    variant="ghost"
                    onClick={() => setDescontos(descontos.filter((_, j) => j !== i))}
                  >
                    <Trash2 className="h-4 w-4" />
                  </Button>
                </div>
            ...
            <div className="bg-muted p-3 rounded grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
              <div>
                ...
                <div className="font-semibold">
                  {totalProv.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}
                </div>
              </div>
              <div>
                <div className="text-muted-foreground">Descontos</div>
                <div className="font-semibold text-destructive">
                  - {totalDesc.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}
                </div>
              </div>
              <div>
