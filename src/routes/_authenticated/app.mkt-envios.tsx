...
interface Row { id: string; canal: string; destinatario: string; status: string; erro: string | null; enviado_em: string | null; created_at: string; }
interface Form { canal: string; destinatario: string; status: string; }
    ...
    <SimpleCrud<Row, Form>
      ...
      columns={[
        ...
        { key: "status", header: "Status", className: "w-28", render: r => (
          <span className={`text-xs px-2 py-0.5 rounded-full capitalize ${cor[r.status] ?? "bg-muted text-muted-foreground"}`}>{r.status}</span>
        ) },
        { key: "erro", header: "Erro", render: r => r.erro ? <span className="text-xs text-rose-600 line-clamp-1">{r.erro}</span> : <span className="text-xs text-muted-foreground">—</span> },
        { key: "quando", header: "Data", className: "w-40", render: r => (
          <span className="text-xs text-muted-foreground">{new Date(r.enviado_em ?? r.created_at).toLocaleString("pt-BR")}</span>
        ) },
      ]}
      ...
      renderForm={(f, set) => (
        <div className="space-y-3">
          ...
          <div className="space-y-1"><Label>Destinatário *</Label><Input required value={f.destinatario} onChange={e => set({ ...f, destinatario: e.target.value })} placeholder="+55 11 99999-9999 ou email@dominio.com" /></div>
        </div>
      )}
    />
  );
