...
import { DateRangeFilter, computeRange, type DateRange, type DatePreset } from "@/components/date-range-filter";
  ...
  useEffect(() => {
    (async () => {
      ...
      const [resumoRes, atend, notas, lancRes, notasFull] = await Promise.all([
        ...
        supabase.from("fin_notas_pacientes").select("id", { count: "exact", head: true }).eq("clinica_id", clinicaAtual.clinica_id).gte("data_emissao", since).lte("data_emissao", hoje),
        ...
        supabase.from("fin_notas_pacientes").select("id, numero, serie, data_emissao, valor, status").eq("clinica_id", clinicaAtual.clinica_id).gte("data_emissao", since).lte("data_emissao", hoje).order("data_emissao", { ascending: false }).limit(1000),
      ]);
      let r = 0, d = 0, p = 0;
