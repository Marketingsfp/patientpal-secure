...
function Page() {
  ...
  const remove = async (l: Lemb) => {
    ...
    if (!(await confirmDialog(`Excluir "${l.titulo}"?`))) return;
  ...
  const prioColor = (p: string) =>
    p === "alta" ? "destructive" : p === "baixa" ? "secondary" : "default";
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Vencimentos e tarefas financeiras</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{editing ? "Editar" : "Novo"} lembrete</DialogTitle>
            </DialogHeader>
          ...
          {items.map((l) => (
            <Card key={l.id} className={l.concluido ? "opacity-60" : ""}>
              <CardContent className="pt-4 pb-4 flex items-center gap-3">
                ...
                >
                  <CheckCircle2
                    className={`h-5 w-5 ${l.concluido ? "text-green-600" : "text-muted-foreground"}`}
                  />
                </Button>
                ...
                <Badge variant={prioColor(l.prioridade) as "default" | "secondary" | "destructive"}>
                  {l.prioridade}
                </Badge>
