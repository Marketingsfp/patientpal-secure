...
function NfsePage() {
  ...
  const onAvancarRps = async (reenviarDepois: boolean) => {
    ...
    if (!Number.isFinite(novo) || novo < 1) {
    ...
    try {
      ...
      const r = await avancarRps({
        data: { emitente_id: erroDetalhe.emitente_id, novo_numero: novo },
      });
      if (!r.ok) {
    ...
    <div className="space-y-6">
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Ban className="h-4 w-4 text-red-600" /> Cancelar NFS-e
            </DialogTitle>
          </DialogHeader>
          {cancelarAlvo && (
            <div className="space-y-3 text-sm">
              <div className="rounded-md bg-muted p-3 space-y-1">
                ...
                <div>
                  ...
                  {Number(cancelarAlvo.valor_servicos).toLocaleString("pt-BR", {
                    style: "currency",
                    currency: "BRL",
                  })}
                </div>
          ...
          <DialogFooter>
            <Button variant="ghost" onClick={() => setCancelarAlvo(null)} disabled={cancelando}>
              Voltar
            </Button>
            <Button
              variant="destructive"
              ...
              onClick={async () => {
                ...
                try {
                  const r = await cancelar({
                    data: { id: cancelarAlvo.id, justificativa: cancelarJustificativa.trim() },
                  });
                  if (r.ok) {
            ...
            >
              {cancelando ? (
                <>
                  <Loader2 className="h-3.5 w-3.5 mr-2 animate-spin" /> Cancelando…
                </>
              ) : (
