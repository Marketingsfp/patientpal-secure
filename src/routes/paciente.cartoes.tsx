...
export const Route = createFileRoute("/paciente/cartoes")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Veja seu cartão benefícios digital, validade, dependentes e mensalidades." },
    ],
  }),
});

interface Mensalidade { id: string; parcela: number; vencimento: string; valor: number; status: string; pago_em: string | null; }
interface Dependente { id: string; nome: string; parentesco: string | null; tipo: string; }
interface Cartao {
  id: string; numero: number; data_inicio: string; vigencia_meses: number; status: string;
  paciente_nome: string; validade: string;
      ...
      <main className="mx-auto max-w-2xl px-4 py-4">
        ...
        {loading ? (
          ...
          <Card className="mt-6 p-6 text-center text-sm text-muted-foreground">
            Nenhum cartão benefícios encontrado para o seu e-mail.<br />
            ...
            {cartoes.map((c) => {
              ...
              return (
                <div key={c.id} className="space-y-3">
                  {/* Carteirinha digital */}
                  <div className={`rounded-2xl p-5 text-white shadow-lg ${isVerde ? "bg-gradient-to-br from-emerald-600 to-emerald-800" : "bg-gradient-to-br from-blue-600 to-blue-900"}`}>
                    <div className="flex items-center justify-between">
                      <span className="text-xs uppercase tracking-wider opacity-80">{c.plano_nome}</span>
                    ...
                    <div className="mt-4 flex items-end justify-between text-xs">
                      <div>
                        ...
                        <p className="font-mono font-semibold">Nº {String(c.numero).padStart(5, "0")}</p>
                      </div>
                    ...
                    {c.clinica_nome && <p className="mt-3 text-xs opacity-80 border-t border-white/20 pt-2">{c.clinica_nome}{c.clinica_telefone ? ` · ${c.clinica_telefone}` : ""}</p>}
                  </div>

                  {c.descricao_beneficios && (
                    <Card className="p-3 text-xs text-muted-foreground whitespace-pre-wrap">{c.descricao_beneficios}</Card>
                  )}

                  {c.dependentes.length > 0 && (
                    <Card className="p-3">
                      <p className="text-xs font-semibold flex items-center gap-1 mb-2"><Users className="h-3.5 w-3.5" /> Dependentes ({c.dependentes.length})</p>
                      <ul className="space-y-1 text-sm">
                        {c.dependentes.map((d) => (
                          <li key={d.id} className="flex justify-between">
                            <span>{d.nome}</span>
                            <span className="text-xs text-muted-foreground">{d.parentesco || d.tipo}</span>
                          </li>
                        ))}
                      </ul>
                    </Card>
                  ...
                  <Card className="p-3">
                    <p className="text-xs font-semibold flex items-center gap-1 mb-2"><Calendar className="h-3.5 w-3.5" /> Mensalidades</p>
                    {proxima ? (
                      <div className="mb-2 p-2 rounded-md bg-amber-50 dark:bg-amber-950/30 text-xs flex items-center gap-2">
                        <AlertCircle className="h-3.5 w-3.5 text-amber-600" />
                        <span>Próximo vencimento: <strong>{fmtData(proxima.vencimento)}</strong> — {fmtMoeda(Number(proxima.valor))}</span>
                      </div>
                    ) : (
                    ...
                    <p className="text-xs text-muted-foreground mb-2">{pagas} de {c.mensalidades.length} parcelas pagas</p>
                    <ul className="space-y-1 text-xs max-h-40 overflow-y-auto">
                      {c.mensalidades.map((m) => (
                        <li key={m.id} className="flex justify-between border-b border-border/40 py-1">
                          <span>#{m.parcela} · {fmtData(m.vencimento)}</span>
                          <span className="flex items-center gap-2">
                            {fmtMoeda(Number(m.valor))}
                            <span className={`px-1.5 py-0.5 rounded text-[10px] uppercase ${m.status === "pago" ? "bg-emerald-100 text-emerald-700" : m.status === "atrasado" ? "bg-red-100 text-red-700" : "bg-muted text-muted-foreground"}`}>{m.status}</span>
                          </span>
                        </li>
                      ))}
                    </ul>
        ...
        <Card className="mt-6 p-4 text-xs text-muted-foreground">
          O pagamento é feito diretamente na clínica. Em caso de dúvidas, entre em contato pelo telefone informado no cartão.
        </Card>
      </main>
    </div>
  );
