  ...
  [
    {
      ...
      Icon: Hash,
      classe: "from-primary/90 to-primary",
    },
    {
      tipo: "P",
      titulo: "Preferencial",
    ...
    {
      tipo: "E",
      titulo: "Prioridade médica",
      sub: "Urgência / encaixe",
      Icon: Stethoscope,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/40 flex flex-col">
      ...
      <main className="flex-1 flex items-center justify-center px-8 pb-12">
        ...
        {step === "consent" && tipo && (
          <div className="max-w-2xl w-full bg-card border rounded-2xl p-10 space-y-6 shadow-xl">
            ...
            <div className="space-y-3 text-muted-foreground">
              <p>
                Podemos usar o reconhecimento facial para identificar você mais rápido nas próximas
                visitas. A imagem <strong>não é armazenada</strong> — apenas um código matemático do
                rosto (vetor) fica salvo, criptografado, e pode ser removido a qualquer momento.
              </p>
              <p className="text-sm">
        ...
        {step === "manual" && tipo && (
          <form
            onSubmit={concluirManual}
            className="max-w-md w-full bg-card border rounded-2xl p-8 space-y-5 shadow-xl"
          >
            <h2 className="text-2xl font-bold">Seus dados</h2>
            <p className="text-sm text-muted-foreground">
              Preencha para personalizar seu atendimento (opcional, exceto o nome).
            </p>
            <div className="space-y-3">
              ...
              <input
                ...
                value={manual.cpf}
                onChange={(e) =>
                  setManual({ ...manual, cpf: e.target.value.replace(/\D/g, "").slice(0, 11) })
                }
                inputMode="numeric"
                className="w-full h-14 px-4 text-lg rounded-lg border bg-background"
              />
        ...
        {step === "ticket" && ticket && (
          <div className="text-center space-y-6 animate-in fade-in zoom-in duration-300">
            ...
            <div className="text-[12rem] leading-none font-black text-primary tabular-nums">
              {ticket.codigo}
            </div>
            <div className="text-2xl">
              {ticket.identificado && ticket.nome
                ? `Bem-vindo de volta, ${ticket.nome}!`
                : "Aguarde sua chamada"}
            </div>
            <p className="text-muted-foreground">
