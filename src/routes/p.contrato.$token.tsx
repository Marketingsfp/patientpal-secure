...
function AssinarContrato() {
  ...
  const assinar = async () => {
    ...
    try {
      ...
      // IP é metadado opcional da assinatura; sem ele a assinatura segue válida.
    }
    const { error } = await supabase.rpc("assinar_contrato_publico", {
      _token: token,
      _assinatura_svg: dataUrl,
      _ip: ip,
    });
    setSaving(false);
      ...
      <div className="max-w-3xl mx-auto bg-card rounded-lg border shadow-sm p-6 space-y-4">
        <div>
          ...
          <h1 className="text-xl font-bold">
            Contrato Nº {c.numero} — {c.plano_nome}
          </h1>
          <div className="text-sm text-muted-foreground">
            Titular: <strong>{c.paciente_nome}</strong>
          </div>
        </div>
          ...
          <div className="text-sm">
            ...
            <ul className="list-disc ml-5">
              {data.dependentes.map((d: any) => (
                <li key={d.id}>
                  {d.paciente_nome} — {d.parentesco ?? "—"} ({d.tipo})
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div className="rounded-md border p-2">
            ...
            <div className="font-bold">R$ {Number(c.valor_mensal).toFixed(2)}</div>
          </div>
          <div className="rounded-md border p-2">
            <div className="text-muted-foreground text-xs">Parcelas</div>
            <div className="font-bold">{c.num_parcelas}x</div>
          </div>
        </div>
        ...
        {assinado ? (
          <div className="rounded-md border-2 border-green-500 bg-green-50 p-4 text-center">
            ...
            <div className="font-semibold text-green-800">
              Contrato assinado em {new Date(c.assinado_em).toLocaleString("pt-BR")}
            </div>
            {c.assinatura_svg ? (
              <img
                src={c.assinatura_svg}
                alt="Assinatura digital do contratante"
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button size="sm" onClick={assinar} disabled={saving}>
                <Check className="h-4 w-4 mr-1" />
                Confirmar assinatura
              </Button>
            </div>
            <p className="text-xs text-muted-foreground">
              Ao assinar, declaro que li e concordo com o contrato. Assinatura registrada com
              data/hora e IP (Lei 14.063/2020).
            </p>
          </div>
        )}
      </div>
